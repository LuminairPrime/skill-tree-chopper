...
export const KNOWN_AGENT_PREFIXES = [
  '.adal',
  '.agent',
  '.agents',
  '.amazonq',
...
export async function hasSkillMd(folderUri: vscode.Uri): Promise<boolean> {
  try {
    ...
    return entries.some(
      ([name, type]) => (type & vscode.FileType.File) !== 0 && isSkillMarkdownFile(name),
    );
  } catch {
    return false;
  }
  ...
  constructor(
    ...
    if (this.resourceUri) {
      ...
      if (this.type === 'skill-folder') {
        this.checkboxState = toVsCodeCheckboxState(
          getSkillFolderCheckboxState(this.resourceUri.fsPath),
        );
...
export class SkillTreeProvider implements vscode.TreeDataProvider<SkillNode> {
  private _onDidChangeTreeData: vscode.EventEmitter<SkillNode | undefined | null | void> =
    new vscode.EventEmitter<SkillNode | undefined | null | void>();
  readonly onDidChangeTreeData: vscode.Event<SkillNode | undefined | null | void> =

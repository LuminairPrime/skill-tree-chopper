...
exports.TerminalFocusService = void 0;
const ManagerLogger_1 = require("../../utils/ManagerLogger");
const debugLog = (message, data) => {
    ManagerLogger_1.terminalLogger.debug(message, data);
};
/**
...
class TerminalFocusService {
     ...
     */
    ensureTerminalFocus(terminal, terminalId, terminalContent) {
        // Use requestAnimationFrame to ensure DOM is fully settled
        requestAnimationFrame(() => {
            try {
                // Find the xterm textarea
                const textarea = terminalContent.querySelector('.xterm-helper-textarea');
                if (!textarea) {
                    // Retry once after a short delay if textarea doesn't exist yet
                    setTimeout(() => {
                        const retryTextarea = terminalContent.querySelector('.xterm-helper-textarea');
                        if (retryTextarea) {
                            this.focusTerminalTextarea(terminal, retryTextarea, terminalId);
                        }
                        else {
                            ManagerLogger_1.terminalLogger.error(`xterm-helper-textarea never appeared for: ${terminalId}`);
                        }
                    }, 50);
                    return;
                }
                ...
                ManagerLogger_1.terminalLogger.error(`Failed to ensure terminal focus for ${terminalId}:`, error);
            }
        });
    }
    /**
     ...
     */
    focusTerminalTextarea(terminal, textarea, terminalId) {
        try {
            ...
            setTimeout(() => {
                ...
                if (hasFocus) {
                    ...
                    ManagerLogger_1.terminalLogger.info(`Terminal successfully focused and ready for input: ${terminalId}`);
                }
                else {
                    console.warn(`[FOCUS-DEBUG] Focus failed for ${terminalId}`);
                    console.warn(`   Active element: ${activeTag}.${activeClass}`);
                    ManagerLogger_1.terminalLogger.warn(`Terminal focus verification failed for: ${terminalId}`);
                    ManagerLogger_1.terminalLogger.warn(`   Active element: ${activeTag}.${activeClass}`);
            ...
            ManagerLogger_1.terminalLogger.error(`Failed to focus terminal textarea for ${terminalId}:`, error);
        }
    }
    /**
     * Setup container click handler for re-focusing terminal
     */
    setupContainerFocusHandler(terminal, terminalId, container, terminalContent) {
...
exports.TerminalFocusService = TerminalFocusService;
//# sourceMappingURL=TerminalFocusService.js.map
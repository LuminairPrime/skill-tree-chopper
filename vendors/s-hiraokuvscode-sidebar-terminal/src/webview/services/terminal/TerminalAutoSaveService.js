...
class TerminalAutoSaveService {
     ...
     */
    static clearPeriodicSaveTimer(terminalId) {
        ...
        if (timer) {
            ...
            ManagerLogger_1.terminalLogger.debug(`[AUTO-SAVE] Cleared periodic save timer for: ${terminalId}`);
        }
    ...
    static setupVisibilityChangeHandler() {
        ...
        document.addEventListener('visibilitychange', () => {
            ...
            if (document.visibilityState === 'hidden') {
                ...
                ManagerLogger_1.terminalLogger.debug('[AUTO-SAVE] Page hidden - saving all scrollback immediately');
            ...
            if (document.visibilityState === 'visible') {
                ...
                ManagerLogger_1.terminalLogger.debug(`[AUTO-SAVE] Page visible - hiddenDuration: ${timeHidden}ms, likelyWake: ${isLikelyWakeFromSleep}`);
                if (isLikelyWakeFromSleep) {
                    // Request scrollback restoration from Extension
                    ManagerLogger_1.terminalLogger.debug('[AUTO-SAVE] Likely wake from sleep - requesting scrollback refresh');
        ...
        ManagerLogger_1.terminalLogger.debug('[AUTO-SAVE] Visibility change handler setup complete');
    }
    /**
     * Save all terminal scrollback immediately (called before sleep)
     */
    static saveAllScrollbackImmediately() {
        ...
        for (const [terminalId, { serializeAddon }] of TerminalAutoSaveService.registeredTerminals) {
            ...
            try {
                ...
                ManagerLogger_1.terminalLogger.debug(`[AUTO-SAVE] Saved scrollback before sleep for ${terminalId}: ${lines.length} lines`);
            }
            catch (error) {
                // eslint-disable-next-line no-console
     ...
     */
    static requestScrollbackRefresh() {
        const windowWithApi = window;
        if (!windowWithApi.vscodeApi) {
            // eslint-disable-next-line no-console
        ...
        ManagerLogger_1.terminalLogger.debug(`[AUTO-SAVE] Requested scrollback refresh for ${TerminalAutoSaveService.registeredTerminals.size} terminals`);
    }
    static requestSessionSaveOnExit() {
        const windowWithApi = window;
        if (!windowWithApi.vscodeApi) {
            // eslint-disable-next-line no-console
        ...
        ManagerLogger_1.terminalLogger.debug('[AUTO-SAVE] Requested session save on webview unload');
    }
    /**
    ...
    static disposeAll() {
        ...
        ManagerLogger_1.terminalLogger.debug('[AUTO-SAVE] All static state disposed');
    }
    /**
    ...
    static markTerminalRestoring(terminalId) {
        ...
        ManagerLogger_1.terminalLogger.debug(`[AUTO-SAVE] Marked terminal as restoring: ${terminalId}`);
    }
    /**
    ...
    static markTerminalRestored(terminalId) {
        ...
        setTimeout(() => {
            ...
            ManagerLogger_1.terminalLogger.debug(`[AUTO-SAVE] Restoration protection ended: ${terminalId}`);
        }, 5000);
    }
    /**
     ...
     */
    setupScrollbackAutoSave(terminal, terminalId, serializeAddon) {
        let saveTimer = null;
        const pushScrollbackToExtension = () => {
            // Skip auto-save if terminal is currently being restored
            if (TerminalAutoSaveService.isTerminalRestoring(terminalId)) {
                ManagerLogger_1.terminalLogger.debug(`[AUTO-SAVE] Skipped save during restoration: ${terminalId}`);
                return;
            }
            if (saveTimer) {
            ...
            saveTimer = window.setTimeout(() => {
                ...
                if (TerminalAutoSaveService.isTerminalRestoring(terminalId)) {
                    ManagerLogger_1.terminalLogger.debug(`[AUTO-SAVE] Skipped delayed save during restoration: ${terminalId}`);
                    return;
                }
                try {
                    const serialized = serializeAddon.serialize({ scrollback: 1000 });
                    ...
                    if (windowWithApi.vscodeApi) {
                        ...
                        ManagerLogger_1.terminalLogger.info(`[AUTO-SAVE] Pushed scrollback via vscodeApi for terminal ${terminalId}: ${lines.length} lines`);
                    }
                    else {
                        if (this.coordinator && typeof this.coordinator.postMessageToExtension === 'function') {
                            ...
                            ManagerLogger_1.terminalLogger.info(`[AUTO-SAVE] Pushed scrollback via MessageManager for terminal ${terminalId}: ${lines.length} lines`);
                        }
                        else {
                            ManagerLogger_1.terminalLogger.error(`[AUTO-SAVE] No message transport available for terminal ${terminalId}`);
                        }
                    }
                }
                catch (error) {
                    ManagerLogger_1.terminalLogger.warn(`[AUTO-SAVE] Failed to push scrollback for terminal ${terminalId}:`, error);
                }
            }, 3000);
        };
        ...
        const periodicTimer = window.setInterval(() => {
            ...
            try {
                ...
                if (windowWithApi.vscodeApi) {
                    windowWithApi.vscodeApi.postMessage(message);
                    ManagerLogger_1.terminalLogger.debug(`[AUTO-SAVE] Periodic save for terminal ${terminalId}: ${lines.length} lines`);
                }
                else if (this.coordinator &&
                    typeof this.coordinator.postMessageToExtension === 'function') {
                    this.coordinator.postMessageToExtension(message);
                    ManagerLogger_1.terminalLogger.debug(`[AUTO-SAVE] Periodic save via coordinator for terminal ${terminalId}: ${lines.length} lines`);
                }
            }
            catch (error) {
                ManagerLogger_1.terminalLogger.warn(`[AUTO-SAVE] Periodic save failed for terminal ${terminalId}:`, error);
            }
        ...
        ManagerLogger_1.terminalLogger.info(`[AUTO-SAVE] Scrollback auto-save enabled for terminal: ${terminalId} (periodic: ${TerminalAutoSaveService.PERIODIC_SAVE_INTERVAL}ms)`);
    }
}
exports.TerminalAutoSaveService = TerminalAutoSaveService;
// Static Set to track terminals currently being restored (blocks auto-save)
TerminalAutoSaveService.restoringTerminals = new Set();
// Periodic save interval (30 seconds) - ensures scrollback is saved even without user activity
TerminalAutoSaveService.PERIODIC_SAVE_INTERVAL = 30000;
// Track periodic save timers for cleanup
TerminalAutoSaveService.periodicSaveTimers = new Map();
// Track registered terminals for visibility change recovery
TerminalAutoSaveService.registeredTerminals = new Map();
// Track xterm.js event listener disposables per terminal for proper cleanup
TerminalAutoSaveService.terminalListenerDisposables = new Map();
...
//# sourceMappingURL=TerminalAutoSaveService.js.map
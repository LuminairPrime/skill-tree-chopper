...
exports.DisplayModeManager = void 0;
 ...
 */
class DisplayModeManager extends BaseManager_1.BaseManager {
    constructor(coordinator) {
        ...
        // ターミナルの可視性マップ
        this.terminalVisibility = new Map();
     ...
     * ターミナルをフルスクリーン表示
     */
    showTerminalFullscreen(terminalId) {
        ...
        allContainers.forEach((container) => DOMUtils_1.DOMUtils.clearContainerHeightStyles(container));
    ...
    showAllTerminalsSplit() {
        ...
        const currentLocation = splitManager.getCurrentPanelLocation?.() || 'sidebar';
        ...
        const isGridLayout = document.getElementById('terminals-wrapper')?.classList.contains('terminal-grid-layout') ??
            false;
     ...
     * 指定ターミナル以外を非表示
     */
    hideAllTerminalsExcept(terminalId) {
        const containerManager = this.coordinator?.getTerminalContainerManager?.();
        if (!containerManager) {
            this.log('TerminalContainerManager not available', 'error');
            return;
    ...
    getSplitManager() {
        ...
        if (this.coordinator && 'splitManager' in this.coordinator) {
            return this.coordinator.splitManager;
        }
        return null;
    }
    /**
    ...
    notifyModeChanged(mode) {
        ...
        const uiManager = this.coordinator?.uiManager;
        if (uiManager?.setFullscreenMode) {
            uiManager.setFullscreenMode(mode === 'fullscreen');
        }
    }
}
exports.DisplayModeManager = DisplayModeManager;
//# sourceMappingURL=DisplayModeManager.js.map
...
exports.TerminalContainerManager = void 0;
 ...
 */
class TerminalContainerManager extends BaseManager_1.BaseManager {
    constructor(coordinator) {
        ...
        this.splitLayoutService = new container_1.SplitLayoutService();
        this.visibilityService = new container_1.ContainerVisibilityService();
    }
    /**
     * 初期化処理
     */
    doInitialize() {
        ...
        const resizeCoordinator = {
            updateSplitResizers: 'updateSplitResizers' in this.coordinator
     ...
     * コンテナを登録
     */
    registerContainer(terminalId, container) {
        this.containerCache.set(terminalId, container);
     ...
     * コンテナの登録を解除
     */
    unregisterContainer(terminalId) {
     ...
     * Splitレイアウト用のラッパーを登録
     */
    registerSplitWrapper(terminalId, wrapper) {
        if (!wrapper)
            return;
     ...
     * Splitラッパーの登録解除
     */
    unregisterSplitWrapper(terminalId) {
     ...
     * コンテナの表示/非表示を設定
     */
    setContainerVisibility(terminalId, visible) {
    ...
    applyDisplayState(state) {
        ...
        if (state.mode === 'split') {
            ...
            const panelLocation = splitDirection === 'horizontal' ? 'panel' : 'sidebar';
            const viewportArea = terminalBody.clientWidth * terminalBody.clientHeight;
            const isCompactPanelArea = panelLocation === 'panel' &&
                viewportArea <= webview_1.PANEL_LOCATION_CONSTANTS.COMPACT_VIEWPORT_AREA_THRESHOLD;
            if ((0, GridLayoutCalculator_1.shouldUseGrid)(orderedIds.length, panelLocation, true) && !isCompactPanelArea) {
                this.splitLayoutService.activateGridLayout(terminalBody, orderedIds, (id) => this.containerCache.get(id));
            }
            else {
                this.splitLayoutService.activateSplitLayout(terminalBody, orderedIds, splitDirection, (id) => this.containerCache.get(id));
            }
        }
        else {
            this.clearSplitArtifacts();
     ...
     */
    applyModeToContainers(state, orderedIds, terminalBody) {
    ...
    clearSplitArtifacts() {
        ...
        targetBody?.querySelectorAll('.split-resizer').forEach((resizer) => {
            resizer.remove();
        });
        ...
        if (targetBody) {
            ...
            if (this.coordinator) {
                const updated = this.coordinator.updatePanelLocationIfNeeded?.();
     ...
     * コンテナの表示モードを設定
     */
    setContainerMode(terminalId, mode) {
        const container = this.getContainer(terminalId);
        if (!container) {
     ...
     * コンテナのモードを取得
     */
    getContainerMode(terminalId) {
        return this.containerModes.get(terminalId) || null;
    }
    /**
     * すべてのコンテナを通常モードに戻す
     */
    resetAllToNormalMode() {
    ...
    reorderContainers(order) {
        ...
        const newContainerCache = new Map();
        const orderedIds = [];
...
exports.TerminalContainerManager = TerminalContainerManager;
//# sourceMappingURL=TerminalContainerManager.js.map
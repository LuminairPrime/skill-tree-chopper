...
exports.KeyboardShortcutSetupHandler = void 0;
/**
 ...
 */
class KeyboardShortcutSetupHandler {
    constructor(deps) {
        this.deps = deps;
    ...
    setupKeyboardShortcuts(manager) {
        ...
        const shortcutHandler = (event) => {
            ...
            this.deps.logger(`Keybinding: ${event.key}, Command: ${resolvedCommand}, Skip Shell: ${shouldSkip}, IME: ${this.deps.isIMEComposing()}`);
        ...
        this.deps.eventRegistry.register('keyboard-shortcuts', document, 'keydown', shortcutHandler);
    ...
    handleLegacyShortcuts(event, manager) {
        ...
        if (event.ctrlKey && event.altKey && event.key === 't') {
            ...
            if (manager.profileManager) {
                manager.profileManager.createTerminalWithDefaultProfile().catch((error) => {
        ...
        if (event.ctrlKey && event.shiftKey && /^[1-5]$/.test(event.key)) {
            ...
            this.deps.logger(`Ctrl+Shift+${event.key} shortcut detected, switching to profile index ${profileIndex}`);
            if (manager.profileManager) {
                manager.profileManager.switchToProfileByIndex(profileIndex).catch((error) => {
     ...
     */
    setAgentInteractionMode(_enabled) {
        ...
        if (this.agentInteractionMode !== actualEnabled) {
            ...
            this.deps.logger(`Agent interaction mode: ${actualEnabled} (VS Code standard - always disabled)`);
    ...
    setupGlobalKeyboardListener() {
        ...
        this.deps.eventRegistry.register('global-keyboard', document, 'keydown', globalKeyHandler, true);
    }
    /**
        ...
        const arrowKeyHandler = (event) => {
            ...
            if (['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight'].includes(event.key)) {
                ...
                if (activeTerminal) {
                    ...
                    if (terminalId) {
                        this.deps.logger(`Arrow key ${event.key} in agent mode for terminal ${terminalId} - letting xterm.js handle`);
                    }
                }
            }
        };
        this.deps.eventRegistry.register('agent-arrow-keys', document, 'keydown', arrowKeyHandler, true);
    }
    /**
...
exports.KeyboardShortcutSetupHandler = KeyboardShortcutSetupHandler;
//# sourceMappingURL=KeyboardShortcutSetupHandler.js.map
...
const ShellIntegrationAddon_1 = require("../addons/ShellIntegrationAddon");
class ShellIntegrationManager {
    constructor() {
        this.coordinator = null;
        this.statusMap = new Map();
        this.statusIndicators = new Map();
        this.cwdDisplays = new Map();
        this.shellAddons = new Map();
        ...
        this.onCommandStart = (command) => {
            ...
            if (!terminalId)
                return;
            (0, logger_1.webview)(`🚀 Command started in terminal ${terminalId}: ${command.command}`);
            // Update status
        ...
        this.onCommandEnd = (command, exitCode) => {
            const terminalId = this.findTerminalIdForCommand(command);
            if (!terminalId)
                return;
            (0, logger_1.webview)(`✅ Command finished in terminal ${terminalId}: "${command.command}" (exit code: ${exitCode})`);
            // Calculate duration
        ...
        this.onPromptStart = () => {
            (0, logger_1.webview)('💡 Shell prompt started');
     ...
     */
    initializeTerminalShellIntegration(terminal, terminalId) {
        try {
            // Create and load shell integration addon
            const addon = new ShellIntegrationAddon_1.ShellIntegrationAddon(this);
            ...
            (0, logger_1.webview)(`🐚 Shell Integration initialized for terminal: ${terminalId}`);
            this.updateStatusIndicator(terminalId, 'ready');
        }
        catch (error) {
            (0, logger_1.webview)(`Failed to initialize shell integration for terminal ${terminalId}:`, error);
        }
    }
    /**
     * Find terminal ID for a command (helper method)
     */
    findTerminalIdForCommand(command) {
     ...
     */
    showExitCodeNotification(terminalId, exitCode, command) {
        // This would integrate with NotificationManager in a full implementation
        (0, logger_1.webview)(`Command failed in terminal ${terminalId}: "${command}" (exit code: ${exitCode})`);
     ...
     */
    updateShellStatus(terminalId, status) {
     ...
     */
    updateStatusIndicator(terminalId, status) {
     ...
     */
    updateCwdDisplay(terminalId, cwd) {
     ...
     */
    addCommandGutter(terminalId, status) {
     ...
     */
    handleCommandLink(terminalId, command) {
        const terminalInstance = this.coordinator?.getAllTerminalInstances().get(terminalId);
        if (!terminalInstance)
            return;
     ...
     */
    showCommandHistory(terminalId, history) {
        // This would integrate with VS Code's QuickPick API
        // For now, we'll just log it
        (0, logger_1.webview)('Command history for terminal', terminalId, history);
    }
    /**
     * Add decorations to terminal output
     */
    decorateTerminalOutput(_terminal, _terminalId) {
        ...
        switch (message.command) {
            case 'updateShellStatus':
                if ('terminalId' in message && 'status' in message) {
                    this.updateShellStatus(message.terminalId, message.status);
                }
                break;
            case 'updateCwd':
                if ('terminalId' in message && 'cwd' in message) {
                    this.updateCwd(message.terminalId, message.cwd);
                }
                break;
            case 'commandHistory':
                if ('terminalId' in message && 'history' in message) {
                    this.showCommandHistory(message.terminalId, message.history);
                }
                break;
        }
    }
     ...
     */
    getShellIntegrationState(terminalId) {
        const addon = this.shellAddons.get(terminalId);
        if (!addon)
            return undefined;
        return {
     ...
     */
    getTerminalCommandHistory(terminalId) {
        const addon = this.shellAddons.get(terminalId);
     ...
     */
    clearTerminalCommandHistory(terminalId) {
        const addon = this.shellAddons.get(terminalId);
        addon?.clearHistory();
    }
}

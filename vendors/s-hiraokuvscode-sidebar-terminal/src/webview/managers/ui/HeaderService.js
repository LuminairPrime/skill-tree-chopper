...
class HeaderService {
    ...
    createTerminalHeader(terminalId, terminalName, options = {}) {
        ...
        const headerElements = HeaderFactory_1.HeaderFactory.createTerminalHeader({
            terminalId,
            terminalName,
        ...
        (0, logger_1.webview)(`🔍 [DEBUG] Header created successfully:`, {
        ...
        ManagerLogger_1.uiLogger.info(`Terminal header created using HeaderFactory for ${terminalId}`);
    ...
    updateTerminalHeader(terminalId, newName, indicatorColor) {
        ...
        if (headerElements) {
            if (typeof newName === 'string' && newName.trim().length > 0) {
                HeaderFactory_1.HeaderFactory.updateTerminalName(headerElements, newName);
            }
            if (typeof indicatorColor === 'string' && indicatorColor.trim().length > 0) {
                HeaderFactory_1.HeaderFactory.setIndicatorColor(headerElements, indicatorColor);
            }
        }
        else {
            ...
            if (typeof newName === 'string') {
                ...
                if (header) {
                    ...
                    ManagerLogger_1.uiLogger.info(`Updated terminal header (fallback) for ${terminalId}: ${newName}`);
                }
            }
            if (typeof indicatorColor === 'string') {
                const headerContainer = document.querySelector(`[data-terminal-id="${terminalId}"].terminal-header`);
    ...
    setTerminalProcessingIndicator(terminalId, isProcessing) {
        ...
        HeaderFactory_1.HeaderFactory.setProcessingIndicatorActive(headerElements, isProcessing);
    }
    setHeaderEnhancementsEnabled(enabled) {
        this.headerElementsCache.forEach((headerElements) => {
            HeaderFactory_1.HeaderFactory.setHeaderEnhancementsEnabled(headerElements, enabled);
        });
    }
    /**
    ...
    removeTerminalHeader(terminalId) {
        if (this.headerElementsCache.has(terminalId)) {
            ...
            ManagerLogger_1.uiLogger.debug(`Removed terminal header cache for ${terminalId}`);
        }
    }
    /**
    ...
    clearHeaderCache() {
        ...
        ManagerLogger_1.uiLogger.debug('Cleared all header cache');
    }
    /**
     * Find all terminal headers in the DOM
     */
    findTerminalHeaders() {
        const headers = Array.from(document.querySelectorAll(`.${HeaderCssClasses.TERMINAL_HEADER}`));
        ManagerLogger_1.uiLogger.debug(`Found ${headers.length} terminal headers`);
        return headers;
    }
    /**
     * Get cached header elements for a specific terminal
     */
    getHeaderElements(terminalId) {
        return this.headerElementsCache.get(terminalId);
    }
    /**
     * Check if a terminal header is cached
     */
    hasHeaderElements(terminalId) {
     ...
     */
    updateHeaderThemeColors(terminalId, backgroundColor, foregroundColor) {
        ...
        ManagerLogger_1.uiLogger.debug(`Updated header theme for terminal ${terminalId}`);
    }
    /**
     * Update all cached header theme colors
     */
    updateAllHeaderThemeColors(backgroundColor, foregroundColor) {
        this.headerElementsCache.forEach((_, terminalId) => {
            this.updateHeaderThemeColors(terminalId, backgroundColor, foregroundColor);
        });
        ManagerLogger_1.uiLogger.info(`Updated ${this.headerElementsCache.size} headers to match terminal theme`);
    }
    /**
     * Update all terminal headers using VS Code CSS variables (fallback for uncached)
     */
    updateHeadersFromCssVariables(headerBg, headerFg) {
        // Update from cache
        this.headerElementsCache.forEach((headerElements) => {
        ...
        (0, logger_1.webview)(`🎨 [UI] Updated ${allTerminalHeaders.length} terminal headers`);
    }
    /**

...
class ProfileSelector {
     ...
     */
    show(profiles, selectedProfileId, onProfileSelected, onClosed) {
        ...
        const filterInput = this._container.querySelector('.profile-filter');
        if (filterInput) {
            filterInput.focus();
        }
        (0, logger_1.webview)('🎯 [PROFILE-UI] Profile selector shown with', profiles.length, 'profiles');
    }
    /**
    ...
    hide() {
        ...
        (0, logger_1.webview)('🎯 [PROFILE-UI] Profile selector hidden');
    }
    /**
    ...
    _updateProfileList() {
        ...
        const filteredProfiles = this._profiles.filter((profile) => profile.name.toLowerCase().includes(this._filterText.toLowerCase()) ||
      ...
      `;
        ...
        (0, logger_1.webview)(`🎯 [PROFILE-UI] Updated profile list: ${filteredProfiles.length} profiles`);
    }
    _selectProfile(profileId) {
        ...
        (0, logger_1.webview)('🎯 [PROFILE-UI] Selected profile:', profileId);
    }
    _confirmSelection() {
    ...
    _navigateList(direction) {
        const profileItems = Array.from(this._container.querySelectorAll('.profile-item:not(.profile-no-results)'));

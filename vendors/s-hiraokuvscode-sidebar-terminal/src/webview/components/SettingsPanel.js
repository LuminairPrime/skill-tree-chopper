"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.SettingsPanel = void 0;
const DOMUtils_1 = require("../utils/DOMUtils");
const ErrorHandler_1 = require("../utils/ErrorHandler");
const logger_1 = require("../../utils/logger");
/**
 * 設定パネルコンポーネント
 */
class SettingsPanel {
    /**
     * コンストラクタ
     */
    constructor(options) {
        this.panelElement = null;
        this.isVisible = false;
        this.versionInfo = 'v0.1.104';
        this.onSettingsChange = options?.onSettingsChange;
        this.onClose = options?.onClose;
    }
    /**
     * バージョン情報を設定
     */
    setVersionInfo(version) {
        this.versionInfo = version;
    }
    /**
     * 設定パネルを表示
     */
    show(currentSettings) {
        (0, logger_1.webview)('⚙️ [SETTINGS] Starting to show settings panel, isVisible:', this.isVisible);
        try {
            if (this.isVisible) {
                (0, logger_1.webview)('⚙️ [SETTINGS] Panel already visible, hiding first');
                this.hide();
                return;
            }
            (0, logger_1.webview)('⚙️ [SETTINGS] Creating panel...');
            this.createPanel();
            (0, logger_1.webview)('⚙️ [SETTINGS] Populating settings...');
            this.populateSettings(currentSettings);
            (0, logger_1.webview)('⚙️ [SETTINGS] Setting up event listeners...');
            this.setupEventListeners();
            (0, logger_1.webview)('⚙️ [SETTINGS] Showing panel...');
            this.showPanel();
            (0, logger_1.webview)('⚙️ [SETTINGS] Settings panel opened successfully');
        }
        catch (error) {
            console.error('❌ [SETTINGS] Error in show():', error);
            ErrorHandler_1.ErrorHandler.handleOperationError('SettingsPanel.show', error);
        }
    }
    /**
     * 設定パネルを非表示
     */
    hide() {
        try {
            if (this.panelElement) {
                DOMUtils_1.DOMUtils.safeRemove(this.panelElement);
                this.panelElement = null;
            }
            this.isVisible = false;
            this.onClose?.();
            (0, logger_1.webview)('⚙️ [SETTINGS] Settings panel closed');
        }
        catch (error) {
            ErrorHandler_1.ErrorHandler.handleOperationError('SettingsPanel.hide', error);
        }
    }
    /**
     * パネルが表示されているかどうか
     */
    get visible() {
        return this.isVisible;
    }
    /**
     * 設定パネルを作成
     */
    createPanel() {
        this.panelElement = DOMUtils_1.DOMUtils.createElement('div', {
            position: 'fixed',
            top: '0',
            left: '0',
            right: '0',
            bottom: '0',
            background: 'rgba(0, 0, 0, 0.8)',
            zIndex: '10000',
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'center',
        }, {
            id: 'settings-panel',
            role: 'dialog',
            'aria-modal': 'true',
            'aria-labelledby': 'settings-panel-title',
        });
        const settingsContent = this.createSettingsContent();
        this.panelElement.appendChild(settingsContent);
    }
    /**
     * 設定コンテンツを作成
     */
    createSettingsContent() {
        const content = DOMUtils_1.DOMUtils.createElement('div', {
            background: 'var(--vscode-editor-background, #1e1e1e)',
            border: '1px solid var(--vscode-widget-border, #454545)',
            borderRadius: '6px',
            padding: '24px',
            minWidth: '400px',
            maxWidth: '600px',
            maxHeight: '80vh',
            overflow: 'auto',
            boxShadow: '0 8px 32px rgba(0, 0, 0, 0.3)',
        });
        content.innerHTML = `
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
        <h2 id="settings-panel-title" style="color: var(--vscode-foreground, #cccccc); margin: 0; font-size: 18px; font-weight: 600;">
          Terminal Settings
        </h2>
        <button id="close-settings" type="button" aria-label="Close settings dialog" style="
          background: transparent;
          border: none;
          color: var(--vscode-foreground, #cccccc);
          font-size: 16px;
          cursor: pointer;
          padding: 4px;
          border-radius: 3px;
        " title="Close">✕</button>
      </div>

      <div role="group" aria-label="Settings options" style="display: grid; gap: 16px;">
        ${this.createActiveBorderControl()}
        ${this.createClaudeCodeIntegrationControl()}
        ${this.createVersionInfoSection()}
      </div>

      <div role="group" aria-label="Settings actions" style="display: flex; gap: 12px; margin-top: 24px; justify-content: flex-end;">
        <button id="reset-settings" type="button" aria-label="Reset all settings to default values" style="
          background: var(--vscode-button-secondaryBackground, #5a5a5a);
          color: var(--vscode-button-secondaryForeground, #cccccc);
          border: 1px solid var(--vscode-widget-border, #454545);
          padding: 8px 16px;
          border-radius: 3px;
          cursor: pointer;
          font-size: 13px;
        ">Reset to Defaults</button>
        <button id="apply-settings" type="button" aria-label="Apply and save current settings" style="
          background: var(--vscode-button-background, #0e639c);
          color: var(--vscode-button-foreground, #ffffff);
          border: none;
          padding: 8px 16px;
          border-radius: 3px;
          cursor: pointer;
          font-size: 13px;
        ">Apply Settings</button>
      </div>
    `;
        return content;
    }
    /**
     * アクティブターミナルの枠表示設定を作成
     */
    createActiveBorderControl() {
        return `
      <div>
        <div style="
          color: var(--vscode-foreground, #cccccc);
          font-size: 13px;
          font-weight: 500;
          margin-bottom: 8px;
        ">
          Active Terminal Border
        </div>
        <select
          id="active-border-mode"
          style="
            width: 100%;
            padding: 6px 8px;
            background: var(--vscode-dropdown-background, #3c3c3c);
            color: var(--vscode-dropdown-foreground, #cccccc);
            border: 1px solid var(--vscode-dropdown-border, #3c3c3c);
            border-radius: 4px;
            font-size: 13px;
            cursor: pointer;
          "
        >
          <option value="none">Off</option>
          <option value="always">Always</option>
          <option value="multipleOnly" selected>Only with Multiple Terminals</option>
        </select>
        <div style="
          font-size: 11px;
          color: var(--vscode-descriptionForeground, #999999);
          margin-top: 6px;
          line-height: 1.4;
        ">
          When to show the blue border around the active terminal.
        </div>
      </div>
    `;
    }
    /**
     * バージョン情報セクションを作成
     */
    createVersionInfoSection() {
        return `
      <div style="border-top: 1px solid var(--vscode-widget-border, #454545); padding-top: 16px;">
        <h3 style="
          color: var(--vscode-foreground, #cccccc);
          font-size: 14px;
          font-weight: 600;
          margin: 0 0 12px 0;
        ">About</h3>
        <div style="
          color: var(--vscode-foreground, #cccccc);
          font-size: 13px;
          display: flex;
          align-items: center;
          gap: 8px;
          padding: 8px 0;
        ">
          <div style="
            background: var(--vscode-badge-background, #4d4d4d);
            color: var(--vscode-badge-foreground, #ffffff);
            padding: 2px 8px;
            border-radius: 3px;
            font-size: 11px;
            font-weight: 500;
          ">
            Secondary Terminal ${this.versionInfo}
          </div>
        </div>
        <div style="
          font-size: 11px;
          color: var(--vscode-descriptionForeground, #999999);
          line-height: 1.4;
        ">
          Production-ready VS Code extension with TypeScript-compliant terminal in sidebar,
          AI agent integration, and comprehensive session management.
        </div>
      </div>
    `;
    }
    /**
     * CLI Agent統合機能コントロールを作成
     */
    createClaudeCodeIntegrationControl() {
        return `
      <div style="border-top: 1px solid var(--vscode-widget-border, #454545); padding-top: 16px;">
        <h3 style="
          color: var(--vscode-foreground, #cccccc);
          font-size: 14px;
          font-weight: 600;
          margin: 0 0 12px 0;
        ">CLI Agent Integration</h3>
        <label style="
          color: var(--vscode-foreground, #cccccc);
          font-size: 13px;
          font-weight: 500;
          display: flex;
          align-items: flex-start;
          gap: 8px;
          cursor: pointer;
        ">
          <input
            type="checkbox"
            id="cli-agent-integration"
            checked
            style="
              width: 16px;
              height: 16px;
              cursor: pointer;
              margin-top: 2px;
            "
          />
          <div>
            <div>Enable File Reference Shortcuts</div>
            <div style="
              font-size: 11px;
              color: var(--vscode-descriptionForeground, #999999);
              margin-top: 4px;
              line-height: 1.4;
            ">
              Use Cmd+Option+L (Mac) or Alt+Ctrl+L (Linux/Windows) to insert file references
            </div>
          </div>
        </label>
      </div>
    `;
    }
    /**
     * イベントリスナーを設定
     */
    setupEventListeners() {
        if (!this.panelElement)
            return;
        // 閉じるボタン
        const closeBtn = this.panelElement.querySelector('#close-settings');
        DOMUtils_1.DOMUtils.addEventListenerSafe(closeBtn, 'click', () => {
            this.hide();
        });
        // ESCキーで閉じる
        DOMUtils_1.DOMUtils.addEventListenerSafe(document.documentElement, 'keydown', (e) => {
            if (e.key === 'Escape' && this.isVisible) {
                this.hide();
            }
        });
        // 背景クリックで閉じる
        DOMUtils_1.DOMUtils.addEventListenerSafe(this.panelElement, 'click', (e) => {
            if (e.target === this.panelElement) {
                this.hide();
            }
        });
        // 適用ボタン
        const applyBtn = this.panelElement.querySelector('#apply-settings');
        DOMUtils_1.DOMUtils.addEventListenerSafe(applyBtn, 'click', () => {
            this.applySettings();
        });
        // リセットボタン
        const resetBtn = this.panelElement.querySelector('#reset-settings');
        DOMUtils_1.DOMUtils.addEventListenerSafe(resetBtn, 'click', () => {
            this.resetSettings();
        });
    }
    /**
     * 設定を適用
     */
    applySettings() {
        try {
            const settings = this.collectSettings();
            this.onSettingsChange?.(settings);
            this.hide();
        }
        catch (error) {
            ErrorHandler_1.ErrorHandler.handleOperationError('SettingsPanel.applySettings', error);
        }
    }
    /**
     * 設定をリセット
     */
    resetSettings() {
        try {
            const defaultSettings = {
                enableCliAgentIntegration: true,
                activeBorderMode: 'multipleOnly',
            };
            this.populateSettings(defaultSettings);
        }
        catch (error) {
            ErrorHandler_1.ErrorHandler.handleOperationError('SettingsPanel.resetSettings', error);
        }
    }
    /**
     * 現在の設定値を収集
     */
    collectSettings() {
        if (!this.panelElement) {
            throw new Error('Settings panel not available');
        }
        const activeBorderModeSelect = this.panelElement.querySelector('#active-border-mode');
        const claudeCodeIntegrationCheckbox = this.panelElement.querySelector('#cli-agent-integration');
        return {
            activeBorderMode: activeBorderModeSelect?.value ?? 'multipleOnly',
            enableCliAgentIntegration: claudeCodeIntegrationCheckbox?.checked ?? true,
        };
    }
    /**
     * 設定値をフォームに反映
     */
    populateSettings(settings) {
        if (!settings || !this.panelElement)
            return;
        try {
            const activeBorderModeSelect = this.panelElement.querySelector('#active-border-mode');
            const claudeCodeIntegrationCheckbox = this.panelElement.querySelector('#cli-agent-integration');
            if (activeBorderModeSelect) {
                activeBorderModeSelect.value =
                    settings.activeBorderMode !== undefined ? settings.activeBorderMode : 'multipleOnly';
            }
            if (claudeCodeIntegrationCheckbox && settings.enableCliAgentIntegration !== undefined) {
                claudeCodeIntegrationCheckbox.checked = settings.enableCliAgentIntegration;
            }
        }
        catch (error) {
            ErrorHandler_1.ErrorHandler.handleOperationError('SettingsPanel.populateSettings', error);
        }
    }
    /**
     * パネルを表示
     */
    showPanel() {
        if (this.panelElement) {
            (0, logger_1.webview)('⚙️ [SETTINGS] Adding panel to document.body...');
            document.body.appendChild(this.panelElement);
            this.isVisible = true;
            (0, logger_1.webview)('⚙️ [SETTINGS] Panel added, isVisible set to true');
            // Ensure panel is visible immediately for debugging
            this.panelElement.style.zIndex = '10000';
            this.panelElement.style.position = 'fixed';
            this.panelElement.style.top = '0';
            this.panelElement.style.left = '0';
            this.panelElement.style.right = '0';
            this.panelElement.style.bottom = '0';
            this.panelElement.style.backgroundColor = 'rgba(0, 0, 0, 0.8)';
            // アニメーション用の初期スタイル
            this.panelElement.style.opacity = '0';
            requestAnimationFrame(() => {
                if (this.panelElement) {
                    this.panelElement.style.transition = 'opacity 0.2s ease';
                    this.panelElement.style.opacity = '1';
                    (0, logger_1.webview)('⚙️ [SETTINGS] Animation applied, panel should be visible');
                }
            });
        }
        else {
            console.error('❌ [SETTINGS] panelElement is null, cannot show panel');
        }
    }
    /**
     * クリーンアップ
     */
    dispose() {
        this.hide();
    }
}
exports.SettingsPanel = SettingsPanel;
//# sourceMappingURL=SettingsPanel.js.map
...
exports.TerminalTabList = void 0;
...
class TerminalTabList {
    ...
    `;
        ...
        this.modeIndicatorContainer.addEventListener('click', () => {
            (0, logger_1.webview)('🖥️ Mode indicator clicked - toggling display mode');
    ...
    setupKeyboardNavigation() {
        this.container.addEventListener('keydown', (e) => {
            if (e.key === 'ArrowLeft' || e.key === 'ArrowRight') {
                ...
                if (e.target.closest('.terminal-tab-rename-input'))
                    return;
                e.preventDefault();
    ...
    setupGlobalEventDelegation() {
        ...
        this.tabsContainer.addEventListener('click', (e) => {
            ...
            (0, logger_1.webview)('🗂️ Tab container clicked:', target.className);
                ...
                if (tabElement) {
                    ...
                    if (tabId) {
                        ...
                        if (closingTabs.has(tabId)) {
                            (0, logger_1.webview)('🗂️ Close already in progress, ignoring:', tabId);
                            return;
                        }
                        closingTabs.add(tabId);
                        (0, logger_1.webview)('🗂️ Close button clicked for tab:', tabId);
        ...
        this.tabsContainer.addEventListener('dblclick', (e) => {
            ...
            if (target.closest('.terminal-tab-close'))
                return;
    ...
    addTab(tab) {
        ...
        if (this.tabs.has(tab.id)) {
            (0, logger_1.webview)(`🗂️ [TAB-LIST] Tab already exists, updating: ${tab.id}`);
        ...
        (0, logger_1.webview)(`🗂️ [TAB-LIST] Tab added: ${tab.id}`);
    }
    removeTab(tabId) {
    ...
    reorderTabs(newOrder) {
        ...
        (0, logger_1.webview)(`🔄 [TAB-LIST] Tabs reordered successfully`);
    }
    ...
    updateTabElement(tabElement, tab) {
        ...
        if (tabElement.querySelector('.terminal-tab-rename-input'))
            return;
        ...
        tabElement.addEventListener('drop', (e) => {
            ...
            if (this.draggedTab && this.draggedTab !== tab.id) {
                ...
                if (fromIndex !== -1) {
                    ...
                    const dropInfo = this.dropTargetInfo && this.dropTargetInfo.id === tab.id
                        ? this.dropTargetInfo
                        : { id: tab.id, position: 'before' };
    ...
    showDropIndicator(tabElement, position) {
        const rect = tabElement.getBoundingClientRect();
    ...
    setFlexDirection(direction) {
        if (!this.container) {
            (0, logger_1.webview)('⚠️ [TAB-LIST] Cannot set flex direction: container not initialized');
            return;
        }
        (0, logger_1.webview)(`📐 [TAB-LIST] Setting tab container flex-direction to: ${direction} (${direction === 'row' ? 'horizontal tabs' : 'vertical tabs'})`);
        this.container.style.flexDirection = direction;
        // Verify the change was applied
        const computedStyle = window.getComputedStyle(this.container);
        (0, logger_1.webview)(`📐 [TAB-LIST] ✅ Verified flex-direction: ${computedStyle.flexDirection}`);
    }
    startRename(tabId) {
        const tab = this.tabs.get(tabId);
        const tabElement = this.container.querySelector(`[data-tab-id="${tabId}"]`);
    ...
    `;
        const menuItems = [
            { label: 'Duplicate', action: () => (0, logger_1.webview)('Duplicate tab') },
            { label: 'Move to New Window', action: () => (0, logger_1.webview)('Move to new window') },
        ];
    ...
    updateTheme(theme) {
        ...
        const tabElements = this.tabsContainer.querySelectorAll('.terminal-tab');
        ...
        (0, logger_1.webview)(`🎨 [TAB-LIST] Updated theme: bg=${theme.background}, fg=${theme.foreground}`);
    }
    /**
...
//# sourceMappingURL=TerminalTabList.js.map
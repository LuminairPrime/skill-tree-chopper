...
class TerminalContainerFactory {
    ...
    static createContainer(config, headerConfig = {}) {
        ...
        try {
            ...
            if (headerConfig.showHeader) {
                ...
                headerElements = HeaderFactory_1.HeaderFactory.createTerminalHeader({
                ...
                ManagerLogger_1.terminalLogger.info(`AI Agent-compatible header created for ${config.id}`);
            }
            ...
            if (headerConfig.onContainerClick) {
                if (header) {
                    ...
                    header.addEventListener('click', (event) => {
                        ...
                        ManagerLogger_1.terminalLogger.info(`🎯 Header area clicked, activating terminal: ${config.id}`);
                    });
                    ManagerLogger_1.terminalLogger.info(`Header click activation enabled for terminal: ${config.id}`);
                }
                else {
                    ...
                    ManagerLogger_1.terminalLogger.info(`Split terminal will use xterm hasSelection() logic: ${config.id}`);
                }
            }
            ...
            ManagerLogger_1.terminalLogger.info(`Container created successfully: ${config.id}`);
            return elements;
        }
        catch (error) {
            ManagerLogger_1.terminalLogger.error(`Failed to create container ${config.id}:`, error);
            throw error;
        }
    }
    /**
     ...
     */
    static createMainContainer(config) {
     ...
     */
    static createTerminalBody(_config) {
     ...
     */
    static setActiveState(container, isActive) {
        if (isActive) {
            this.applyStyles(container, this.DEFAULT_STYLES.active);
     ...
     */
    static configureSplitMode(container, height) {
        this.applyStyles(container, this.DEFAULT_STYLES.split);
     ...
     */
    static removeFromSplitMode(container) {
     ...
     */
    static applyTheme(container, theme) {
        if (theme.background) {
            container.style.background = theme.background;
            // Also update body background
            const body = container.querySelector('.terminal-content');
            if (body) {
     ...
     */
    static appendToMainContainer(container) {
     ...
     */
    static createSimpleContainer(id, name) {
        const container = document.createElement('div');
    ...
    static destroyContainer(container) {
        try {
            ...
            ManagerLogger_1.terminalLogger.info(`Container destroyed: ${terminalId}`);
        }
        catch (error) {
            ManagerLogger_1.terminalLogger.error('Failed to destroy container:', error);
        }
    }
}
exports.TerminalContainerFactory = TerminalContainerFactory;
TerminalContainerFactory.DEFAULT_STYLES = {
    container: {
        display: 'flex',
        flexDirection: 'column',
        width: '100%',
...
//# sourceMappingURL=TerminalContainerFactory.js.map
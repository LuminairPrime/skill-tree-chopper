...
(0, mocha_1.describe)('Split Mode Specification Tests', () => {
    ...
    (0, mocha_1.describe)('REQ-2: Adding Terminal in Split Mode', () => {
        (0, mocha_1.it)('TC-2.1: Adding terminal to 2 existing should result in 3 equal terminals', () => {
            // Initial: 2 terminals
            setupSplitLayout(2);
            (0, chai_1.expect)(terminalBody.querySelectorAll('.terminal-split-wrapper').length).to.equal(2);
            // Add 1 terminal
            clearTerminalBody();
            setupSplitLayout(3);
        ...
        (0, mocha_1.it)('TC-2.2: Adding terminal to 4 existing should result in 5 equal terminals', () => {
            setupSplitLayout(4);
            clearTerminalBody();
            setupSplitLayout(5);
            const wrappers = terminalBody.querySelectorAll('.terminal-split-wrapper');
        ...
        (0, mocha_1.it)('TC-2.4: All existing terminals remain visible after adding new', () => {
            ...
            wrappers.forEach((wrapper, index) => {
                (0, chai_1.expect)(wrapper.style.display).to.not.equal('none');
                (0, chai_1.expect)(wrapper.getAttribute('data-terminal-wrapper-id')).to.equal(`terminal-${index + 1}`);
            });
        });
    });
    (0, mocha_1.describe)('REQ-3: Adding Terminal in Fullscreen Mode', () => {
        ...
        (0, mocha_1.it)('TC-3.2: All existing terminals must be visible before new terminal creation', () => {
        ...
        (0, mocha_1.it)('TC-3.4: No terminal should be missing or hidden', () => {
            ...
            for (let i = 0; i < 5; i++) {
                ...
                (0, chai_1.expect)(wrapper.getAttribute('data-terminal-wrapper-id')).to.equal(`terminal-${i + 1}`);
                (0, chai_1.expect)(wrapper.style.display).to.not.equal('none');
                (0, chai_1.expect)(wrapper.style.visibility).to.not.equal('hidden');
            }
        });
    ...
    (0, mocha_1.describe)('REQ-4: Removing Terminal in Split Mode', () => {
        (0, mocha_1.it)('TC-4.1: Removing 1 from 4 terminals should result in 3 equal terminals', () => {
            setupSplitLayout(4);
            (0, chai_1.expect)(terminalBody.querySelectorAll('.terminal-split-wrapper').length).to.equal(4);
            // Remove terminal
            clearTerminalBody();
            setupSplitLayout(3);
            const wrappers = terminalBody.querySelectorAll('.terminal-split-wrapper');

...
describe('Session Persistence Performance Tests - Phase 2.5.3', () => {
    ...
    describe('Large Scrollback Restoration Performance', () => {
        it('should restore 2000-line scrollback in <1000ms', async () => {
            ...
            const scrollbackData = largeSession.scrollbackData['1'];
            ...
            (0, chai_1.expect)(processedLines).to.be.at.least(500);
        });
        it('should restore 5000-line scrollback with progressive loading in <1500ms', async () => {
            ...
            const scrollbackData = veryLargeSession.scrollbackData['1'];
            ...
            (0, chai_1.expect)(duration).to.be.lessThan(1500);
            (0, chai_1.expect)(processedLines).to.equal(500); // Progressive loading
        });
    });
    describe('Small Scrollback Restoration Performance', () => {
        it('should restore 500-line scrollback in <500ms', async () => {
            ...
            // When: Restoration
            const startTime = performance.now();
            const scrollbackData = smallSession.scrollbackData['1'];
            const processedLines = scrollbackData.length;
            const duration = performance.now() - startTime;
            // Then: Performance target met
            (0, chai_1.expect)(duration).to.be.lessThan(500);
            (0, chai_1.expect)(processedLines).to.equal(500);
        });
        it('should restore 100-line scrollback in <100ms', () => {
            ...
            // When: Restoration
            const startTime = performance.now();
            const scrollbackData = tinySession.scrollbackData['1'];
            const processedLines = scrollbackData.length;
            const duration = performance.now() - startTime;
            // Then: Very fast restoration
            (0, chai_1.expect)(duration).to.be.lessThan(100);
            (0, chai_1.expect)(processedLines).to.equal(100);
        });
    });
    describe('Storage Optimization Performance', () => {
        it('should calculate storage size in <10ms', () => {
            ...
            const startTime = performance.now();
            const size = session_types_1.SessionDataTransformer.calculateStorageSize(session);
            const duration = performance.now() - startTime;
            // Then: Fast calculation
            (0, chai_1.expect)(duration).to.be.lessThan(10);
            (0, chai_1.expect)(size).to.be.greaterThan(0);
        });
        it('should optimize oversized session in <100ms', () => {
            ...
            // When: Optimization
            const startTime = performance.now();
            const result = session_types_1.SessionDataTransformer.optimizeSessionStorage(largeSession, 5);
            const duration = performance.now() - startTime;
            // Then: Optimization completes quickly
            (0, chai_1.expect)(duration).to.be.lessThan(100);
            (0, chai_1.expect)(result.optimized).to.be.true;
            (0, chai_1.expect)(result.newSizeMB).to.be.lessThan(result.originalSizeMB);
        });
        it('should handle 100 consecutive storage checks efficiently', () => {
            ...
            const startTime = performance.now();
            const results = sessions.map((session) => session_types_1.SessionDataTransformer.isStorageLimitExceeded(session, 20));
            const duration = performance.now() - startTime;
            // Then: Batch processing efficient
            (0, chai_1.expect)(duration).to.be.lessThan(500); // <5ms per check
    ...
    describe('Memory Efficiency', () => {
        ...
        it('should efficiently handle array splicing for lazy loading', () => {
            ...
            (0, chai_1.expect)(chunks.length).to.equal(10);
            (0, chai_1.expect)(lines.length).to.equal(5000); // Half remaining
        });
    });
    describe('Concurrent Operations Performance', () => {
        it('should handle concurrent storage checks without degradation', async () => {
            ...
            const startTime = performance.now();
            const promises = sessions.map((session) => Promise.resolve(session_types_1.SessionDataTransformer.isStorageLimitExceeded(session, 20)));
            const results = await Promise.all(promises);
            const duration = performance.now() - startTime;
            // Then: Concurrent execution efficient
            (0, chai_1.expect)(duration).to.be.lessThan(200); // <10ms per concurrent check
            (0, chai_1.expect)(results).to.have.length(20);
        });
        it('should handle concurrent optimizations without blocking', async () => {
            ...
            const startTime = performance.now();
            const promises = sessions.map((session) => Promise.resolve(session_types_1.SessionDataTransformer.optimizeSessionStorage(session, 3)));
            const results = await Promise.all(promises);
            const duration = performance.now() - startTime;
            // Then: Reasonable concurrent performance
            (0, chai_1.expect)(duration).to.be.lessThan(500); // <100ms per concurrent optimization
            (0, chai_1.expect)(results.every((r) => r.optimized)).to.be.true;
        });
    });
    describe('Migration Performance', () => {
        it('should migrate old format session in <50ms', () => {
            ...
            // When: Migration
            const startTime = performance.now();
            const result = session_types_1.SessionDataTransformer.migrateSessionFormat(oldSession);
            const duration = performance.now() - startTime;
            // Then: Fast migration
            (0, chai_1.expect)(duration).to.be.lessThan(50);
            (0, chai_1.expect)(result.migrated).to.be.true;
        });
        it('should validate session in <20ms', () => {
            ...
            // When: Validation
            const startTime = performance.now();
            const result = session_types_1.SessionDataTransformer.validateSessionForRestore(session);
            const duration = performance.now() - startTime;
            // Then: Fast validation
            (0, chai_1.expect)(duration).to.be.lessThan(20);
    ...
    describe('Regression Prevention', () => {
        it('should maintain performance with ANSI color codes', async () => {
            ...
            const startTime = performance.now();
            const scrollbackData = coloredSession.scrollbackData['1'];
            let processedLines = 0;
            const batchSize = 100;
            for (let i = 0; i < Math.min(500, scrollbackData.length); i += batchSize) {
                const batch = scrollbackData.slice(i, i + batchSize);
        ...
        it('should maintain performance with wrapped lines', async () => {
            ...
            const startTime = performance.now();
            const scrollbackData = wrappedSession.scrollbackData['1'];
 ...
 */
function createSessionWithScrollback(lineCount, lineLength) {
    return {
        terminals: [
            {
                id: '1',
...
//# sourceMappingURL=PerformanceTests.test.js.map
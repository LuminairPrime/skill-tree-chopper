  ...
  describe('Storage Optimization Performance', () => {
    ...
    it('should handle 100 consecutive storage checks efficiently', () => {
      ...
      const results = sessions.map((session) =>
        session_types_1.SessionDataTransformer.isStorageLimitExceeded(session, 20)
      );
      const duration = performance.now() - startTime;
  ...
  describe('Concurrent Operations Performance', () => {
    it('should handle concurrent storage checks without degradation', async () => {
      ...
      const promises = sessions.map((session) =>
        Promise.resolve(session_types_1.SessionDataTransformer.isStorageLimitExceeded(session, 20))
      );
    ...
    it('should handle concurrent optimizations without blocking', async () => {
      ...
      const startTime = performance.now();
      const promises = sessions.map((session) =>
        Promise.resolve(session_types_1.SessionDataTransformer.optimizeSessionStorage(session, 3))
      );
      const results = await Promise.all(promises);
      const duration = performance.now() - startTime;

...
describe('Progressive Loading Integration - Phase 2.2', () => {
    ...
    describe('Chunk-based Loading (2.2.1)', () => {
        it('should load initial 500 lines for large scrollback (>500 lines)', async () => {
            ...
            const writtenLines = [];
            mockTerminal.write = sandbox.stub().callsFake((data) => {
            ...
            (0, chai_1.expect)(writtenLines.length).to.be.at.least(500);
            (0, chai_1.expect)(writtenLines.length).to.be.lessThan(1000);
        });
        it('should load all lines for small scrollback (<=500 lines)', () => {
            ...
            (0, chai_1.expect)(writtenLines.length).to.equal(300);
        });
        it('should use 100-line batches to avoid UI blocking', async () => {
            ...
            (0, chai_1.expect)(batches.length).to.equal(5); // 500 lines / 100 = 5 batches
            (0, chai_1.expect)(batches.every((size) => size <= 100)).to.be.true;
        });
    });
    describe('Performance Benchmarks (2.2.2)', () => {
        ...
        it('should log performance status with ✅ or ⚠️ indicators', () => {
            ...
            (0, chai_1.expect)(fastStatus).to.equal('✅');
            (0, chai_1.expect)(slowStatus).to.equal('⚠️');
        });
    });
    describe('Lazy Loading (2.2.3)', () => {
        it('should defer remaining lines for lazy loading', () => {
            ...
            (0, chai_1.expect)(lazyLoadSetup).to.be.true;
            (0, chai_1.expect)(remainingLines).to.equal(500);
        });
        it('should load next 500-line chunk on scroll-to-top', () => {
            ...
            // Then: Chunk loaded
            (0, chai_1.expect)(chunkLoaded).to.be.true;
        });
        it('should cleanup scroll listener when all history loaded', () => {
            ...
            (0, chai_1.expect)(listenerDisposed).to.be.true;
        });
        it('should prepend historical content to preserve order', () => {
            ...
            // Then: Correct order
            (0, chai_1.expect)(combined[0]).to.equal('Line 1');
            (0, chai_1.expect)(combined[combined.length - 1]).to.equal('Line 502');
        });
    });
    describe('Error Handling and Edge Cases', () => {
        it('should handle empty scrollback gracefully', () => {
            ...
            (0, chai_1.expect)(shouldLoad).to.be.false;
        });
        it('should handle exactly 500 lines without triggering progressive loading', () => {
            ...
            (0, chai_1.expect)(useProgressive).to.be.false;
        });
        it('should handle 501 lines with progressive loading', () => {
            ...
            (0, chai_1.expect)(useProgressive).to.be.true;
        });
        it('should preserve ANSI colors during progressive loading', () => {
            ...
            (0, chai_1.expect)(writtenData.some((line) => line.includes('\x1b[31m'))).to.be.true;
            (0, chai_1.expect)(writtenData.some((line) => line.includes('\x1b[32m'))).to.be.true;
            (0, chai_1.expect)(writtenData.some((line) => line.includes('\x1b[34m'))).to.be.true;
        });
    });
    describe('Memory Efficiency', () => {
        it('should free memory for loaded chunks (using array.splice)', () => {
            ...
            (0, chai_1.expect)(chunk.length).to.equal(500);
            (0, chai_1.expect)(remainingLines.length).to.equal(500);
            (0, chai_1.expect)(remainingLines.length).to.equal(initialLength - chunk.length);
        });
        it('should track remaining lines count accurately', () => {
            ...
            (0, chai_1.expect)(loadedLines).to.equal(1000);
            (0, chai_1.expect)(remainingLines).to.equal(1000);
        });
    });
});
/**
...
//# sourceMappingURL=ProgressiveLoading.test.js.map
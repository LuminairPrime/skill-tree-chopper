...
class MockSecondaryTerminalProvider {
    constructor(webview, terminalManager) {
    ...
    handleSendInput(data, terminalId) {
    ...
    describe('Step 1: WebView Ready → Extension', () => {
        it('should create initial terminal when WebView sends webviewReady', (done) => {
            ...
            setTimeout(() => {
                ...
                (0, chai_1.expect)(terminalCreatedMsg).to.exist;
                (0, chai_1.expect)(terminalCreatedMsg?.terminal).to.exist;
                (0, chai_1.expect)(terminalCreatedMsg?.terminal.id).to.be.a('string');
                (0, chai_1.expect)(terminalCreatedMsg?.terminal.name).to.match(/Terminal \d/);
                (0, chai_1.expect)(terminalCreatedMsg?.terminal.isActive).to.be.true;
                done();
            }, 100);
        });
        it('should restore existing terminals when WebView reconnects', () => {
            ...
            (0, chai_1.expect)(restoreMsg).to.exist;
            (0, chai_1.expect)(restoreMsg?.terminals).to.have.length(2);
            (0, chai_1.expect)(restoreMsg?.terminals.map((t) => t.id)).to.include(terminal1);
            (0, chai_1.expect)(restoreMsg?.terminals.map((t) => t.id)).to.include(terminal2);
        });
    ...
    describe('Step 2: Terminal State Synchronization', () => {
        it('should synchronize terminal ID between Extension and WebView', (done) => {
            ...
            setTimeout(() => {
                ...
                // Assert: IDs match
                (0, chai_1.expect)(terminal).to.exist;
                (0, chai_1.expect)(terminal?.id).to.equal(webViewTerminalId);
                done();
            }, 100);
        });
        ...
        it('should assign sequential terminal numbers', (done) => {
            ...
            setTimeout(() => {
                ...
                (0, chai_1.expect)(msg1?.terminal.name).to.equal('Terminal 1');
                // Create second terminal
                mockWebView.clearMessages();
                mockWebView.simulateWebViewMessage({ command: 'requestInitialTerminal' });
                setTimeout(() => {
                    const msg2 = mockWebView.findMessage('terminalCreated');
                    (0, chai_1.expect)(msg2?.terminal.name).to.equal('Terminal 2');
                    done();
                }, 100);
            }, 100);
        });
    ...
    describe('Step 3: PTY Process Initialization', () => {
        it('should initialize PTY process with event handlers', (done) => {
            ...
            setTimeout(() => {
                ...
                (0, chai_1.expect)(terminal?.ptyProcess).to.exist;
                (0, chai_1.expect)(terminal?.pty).to.exist;
                // Assert: Process state is initialized
                (0, chai_1.expect)(terminal?.processState).to.be.oneOf([shared_1.ProcessState.Launching, shared_1.ProcessState.Running]);
                done();
            }, 100);
        });
        it('should register exactly one event handler per event type', (done) => {
            ...
            setTimeout(() => {
                ...
                const terminalId = terminalCreatedMsg?.terminal.id;
                const terminal = terminalManager.getTerminal(terminalId);
                const ptyProcess = terminal?.ptyProcess;
                // Check event listener counts
                if (ptyProcess && ptyProcess.listenerCount) {
                    (0, chai_1.expect)(ptyProcess.listenerCount('data')).to.equal(1);
                    (0, chai_1.expect)(ptyProcess.listenerCount('exit')).to.equal(1);
                }
                done();
            }, 100);
        });
    ...
    describe('Step 4: User Input Flow', () => {
        it('should handle user input from WebView to PTY', (done) => {
            ...
            setTimeout(() => {
                ...
                setTimeout(() => {
                    (0, chai_1.expect)(sendInputSpy.calledOnce).to.be.true;
                    (0, chai_1.expect)(sendInputSpy.firstCall.args[0]).to.equal('echo "test"\n');
                    (0, chai_1.expect)(sendInputSpy.firstCall.args[1]).to.equal(terminalId);
        ...
        it('should handle input for active terminal when terminalId not specified', (done) => {
            ...
            setTimeout(() => {
                ...
                setTimeout(() => {
                    (0, chai_1.expect)(sendInputSpy.calledOnce).to.be.true;
                    (0, chai_1.expect)(sendInputSpy.firstCall.args[0]).to.equal('test command\n');
                    // Should use active terminal
                    (0, chai_1.expect)(sendInputSpy.firstCall.args[1]).to.be.undefined;
                    sendInputSpy.restore();
                    done();
                }, 50);
            }, 100);
    ...
    describe('Error Handling', () => {
        ...
        it('should handle rapid WebView ready messages', (done) => {
            ...
            setTimeout(() => {
                ...
                (0, chai_1.expect)(terminals.length).to.be.lessThanOrEqual(1);
                done();
            }, 200);
        });
    });
});
//# sourceMappingURL=WebViewTerminalCreationFlow.test.js.map
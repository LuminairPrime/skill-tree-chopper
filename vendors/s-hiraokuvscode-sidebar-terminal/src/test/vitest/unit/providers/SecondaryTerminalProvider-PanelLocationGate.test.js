...
(0, vitest_1.describe)('SecondaryTerminalProvider - Panel Location Response Gate', () => {
    ...
    (0, vitest_1.it)('should ignore reportPanelLocation when no detection was requested', async () => {
        const handleReportSpy = vitest_1.vi.spyOn(provider._panelLocationService, 'handlePanelLocationReport');
        await messageHandler?.({ command: 'reportPanelLocation', location: 'panel' });
        (0, vitest_1.expect)(handleReportSpy).not.toHaveBeenCalled();
    });
    (0, vitest_1.it)('should accept reportPanelLocation after explicit detection request', async () => {
        const handleReportSpy = vitest_1.vi.spyOn(provider._panelLocationService, 'handlePanelLocationReport');

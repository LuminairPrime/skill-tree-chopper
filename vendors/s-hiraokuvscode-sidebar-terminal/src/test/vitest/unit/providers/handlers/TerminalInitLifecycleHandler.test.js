...
function createMockDeps(overrides = {}) {
    return {
        ...
        getCurrentState: vitest_1.vi.fn().mockReturnValue({ terminals: [], activeTerminalId: 'term-1' }),
        ...
        getCurrentFontSettings: vitest_1.vi.fn().mockReturnValue({ fontSize: 14, fontFamily: 'monospace' }),
...
(0, vitest_1.describe)('TerminalInitLifecycleHandler', () => {
    ...
    (0, vitest_1.describe)('handleTerminalReady', () => {
        ...
        (0, vitest_1.it)('should advance state to ViewReady when state is below ViewReady', async () => {
            ...
            await handler.handleTerminalReady(msg);
            (0, vitest_1.expect)(deps.terminalInitStateMachine.markViewReady).toHaveBeenCalledWith('term-1', 'terminalReady');
            (0, vitest_1.expect)(deps.watchdogCoordinator.startForTerminal).toHaveBeenCalledWith('term-1', 'prompt', 'terminalReady');
        });
    ...
    (0, vitest_1.describe)('handleTerminalInitializationComplete', () => {
        ...
        (0, vitest_1.it)('should ignore duplicate when prompt phase already active and past ViewReady', async () => {
        ...
        (0, vitest_1.it)('should initialize shell and start pty output on success', async () => {
            ...
            await handler.handleTerminalInitializationComplete(msg);
            (0, vitest_1.expect)(deps.watchdogCoordinator.stopForTerminal).toHaveBeenCalledWith('term-1', 'webviewAck');
            (0, vitest_1.expect)(deps.terminalInitStateMachine.markViewReady).toHaveBeenCalledWith('term-1', 'webviewAck');
            (0, vitest_1.expect)(deps.initializeShellForTerminal).toHaveBeenCalledWith('term-1', {}, false);
            ...
            (0, vitest_1.expect)(deps.terminalInitStateMachine.markPromptReady).toHaveBeenCalledWith('term-1', 'startOutput');
        ...
        (0, vitest_1.it)('should handle shell initialization failure gracefully', async () => {
            ...
            await handler.handleTerminalInitializationComplete(msg);
            (0, vitest_1.expect)(deps.terminalInitStateMachine.markFailed).toHaveBeenCalledWith('term-1', 'initializeShell');
        ...
        (0, vitest_1.it)('should handle pty output start failure gracefully', async () => {
            ...
            await handler.handleTerminalInitializationComplete(msg);
            (0, vitest_1.expect)(deps.terminalInitStateMachine.markFailed).toHaveBeenCalledWith('term-1', 'startPtyOutput');
        });
    });
    ...
    (0, vitest_1.describe)('initializeTerminal', () => {
        ...
        (0, vitest_1.it)('should send stateUpdate after terminal creation messages', async () => {
            deps.getTerminals.mockReturnValue([]);
            await handler.initializeTerminal();
            (0, vitest_1.expect)(deps.sendMessage).toHaveBeenCalledWith(vitest_1.expect.objectContaining({ command: 'stateUpdate' }));
        });
    ...
    (0, vitest_1.describe)('registerInitializationWatchdogs', () => {
        ...
        (0, vitest_1.it)('should start watchdog for existing terminals when webview is initialized', () => {
            deps.getTerminals.mockReturnValue([{ id: 'term-1' }]);
            deps.isWebViewInitialized.mockReturnValue(true);
            handler.registerInitializationWatchdogs();
            (0, vitest_1.expect)(deps.watchdogCoordinator.recordInitStart).toHaveBeenCalledWith('term-1');
            (0, vitest_1.expect)(deps.watchdogCoordinator.startForTerminal).toHaveBeenCalledWith('term-1', 'ack', 'existingTerminal');
        });
        (0, vitest_1.it)('should add pending terminal when webview is not initialized', () => {
            deps.getTerminals.mockReturnValue([{ id: 'term-1' }]);
            deps.isWebViewInitialized.mockReturnValue(false);
            handler.registerInitializationWatchdogs();
            (0, vitest_1.expect)(deps.watchdogCoordinator.addPendingTerminal).toHaveBeenCalledWith('term-1');
        });
        ...
        (0, vitest_1.it)('should call callback for terminal created event that records init and starts watchdog', () => {
            let createdCallback;
            ...
            (0, vitest_1.expect)(deps.watchdogCoordinator.recordInitStart).toHaveBeenCalledWith('new-term');
            (0, vitest_1.expect)(deps.terminalInitStateMachine.markViewPending).toHaveBeenCalledWith('new-term', 'terminalCreated');
            (0, vitest_1.expect)(deps.terminalInitStateMachine.markPtySpawned).toHaveBeenCalledWith('new-term', 'terminalCreated');
        });
        (0, vitest_1.it)('should call callback for terminal removed event that stops watchdog and resets state', () => {
            let removedCallback;
            ...
            (0, vitest_1.expect)(deps.watchdogCoordinator.stopForTerminal).toHaveBeenCalledWith('term-1', 'terminalRemoved');

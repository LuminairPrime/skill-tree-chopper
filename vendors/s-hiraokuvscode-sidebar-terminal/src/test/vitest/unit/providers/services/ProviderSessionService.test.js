...
function createMockDeps() {
    return {
        ...
        getTerminals: vitest_1.vi.fn().mockReturnValue([{ id: 'terminal-1', name: 'Terminal 1', cwd: '/home' }]),
...
(0, vitest_1.describe)('ProviderSessionService', () => {
    ...
    (0, vitest_1.describe)('saveCurrentSession', () => {
        ...
        (0, vitest_1.it)('should return false on exception', async () => {
            vitest_1.vi.mocked(deps.extensionPersistenceService.saveCurrentSession).mockRejectedValue(new Error('Network error'));
            const result = await service.saveCurrentSession();
            (0, vitest_1.expect)(result).toBe(false);
        });
    });
    (0, vitest_1.describe)('restoreLastSession', () => {
        ...
        (0, vitest_1.it)('should restore terminals from session', async () => {
            ...
            (0, vitest_1.expect)(deps.sendMessage).toHaveBeenCalledWith(vitest_1.expect.objectContaining({ command: 'terminalCreated' }));
            // Should restore scrollback for the first terminal
            (0, vitest_1.expect)(deps.sendMessage).toHaveBeenCalledWith(vitest_1.expect.objectContaining({ command: 'restoreScrollback' }));
        });
        (0, vitest_1.it)('should send fontSettings at top level for terminalCreated messages', async () => {
            vitest_1.vi.mocked(deps.extensionPersistenceService.restoreSession).mockResolvedValue({
        ...
        (0, vitest_1.it)('should wait using TIMING_CONSTANTS.WEBVIEW_INIT_DELAY_MS before restoring scrollback', async () => {
            ...
            try {
                ...
                (0, vitest_1.expect)(setTimeoutSpy).toHaveBeenCalledWith(vitest_1.expect.any(Function), 321);
            }
            finally {
                TimingConstants_1.TIMING_CONSTANTS.WEBVIEW_INIT_DELAY_MS =
                    originalDelay;

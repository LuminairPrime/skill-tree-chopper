...
(0, vitest_1.describe)('TerminalKillService', () => {
  ...
  (0, vitest_1.describe)('killTerminal', () => {
    (0, vitest_1.it)('should kill active terminal without confirmation', async () => {
      ...
      (0, vitest_1.expect)(deps.sendMessage).toHaveBeenNthCalledWith(
        1,
        vitest_1.expect.objectContaining({ command: 'terminalRemoved', terminalId: 'terminal-1' })
      );
      (0, vitest_1.expect)(deps.sendMessage).toHaveBeenNthCalledWith(
        2,
    ...
    (0, vitest_1.it)('should short-circuit duplicate kill attempts while in flight', async () => {
      vitest_1.vi.useFakeTimers();
      vitest_1.vi.mocked(deps.killTerminal).mockImplementation(
        () =>
          new Promise((resolve) => {
    ...
    (0, vitest_1.it)('should send failure response when kill throws', async () => {
      ...
      (0, vitest_1.expect)(deps.sendMessage).not.toHaveBeenCalledWith(
  ...
  (0, vitest_1.describe)('killSpecificTerminal', () => {
    (0, vitest_1.it)('should kill the specified terminal', async () => {
      ...
      (0, vitest_1.expect)(deps.sendMessage).toHaveBeenNthCalledWith(
        1,
        vitest_1.expect.objectContaining({ command: 'terminalRemoved', terminalId: 'terminal-3' })
      );
      (0, vitest_1.expect)(deps.sendMessage).toHaveBeenNthCalledWith(
        2,
        vitest_1.expect.objectContaining({ command: 'stateUpdate' })
    ...
    (0, vitest_1.it)(
      'should short-circuit duplicate killSpecificTerminal requests while in flight',
      async () => {
        vitest_1.vi.useFakeTimers();
        vitest_1.vi.mocked(deps.killTerminal).mockImplementation(
          () =>

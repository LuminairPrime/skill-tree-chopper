...
(0, vitest_1.describe)('SecondaryTerminalProvider', () => {
  ...
  (0, vitest_1.describe)('Message Handling (Handshake)', () => {
    ...
    (0, vitest_1.it)(
      'should route renameTerminal message to terminal command handlers',
      async () => {
        provider.resolveWebviewView(mockWebviewView, {}, {});
        const messageHandler = mockWebviewView.webview.onDidReceiveMessage.mock.calls[0][0];
    ...
    (0, vitest_1.it)(
      'should route updateTerminalHeader message to terminal command handlers',
      async () => {
        provider.resolveWebviewView(mockWebviewView, {}, {});
        const messageHandler = mockWebviewView.webview.onDidReceiveMessage.mock.calls[0][0];
        await messageHandler({
        ...
        await vitest_1.vi.waitFor(() => {
          (0, vitest_1.expect)(mockTerminalManager.updateTerminalHeader).toHaveBeenCalledWith(
            'term-1',
            {
              newName: 'Renamed + Colored',
              indicatorColor: '#FF69B4',

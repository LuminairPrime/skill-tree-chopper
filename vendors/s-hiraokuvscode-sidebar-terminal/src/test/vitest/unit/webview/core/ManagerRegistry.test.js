...
(0, vitest_1.describe)('ManagerRegistry', () => {
    ...
    (0, vitest_1.describe)('initializeAll', () => {
        ...
        (0, vitest_1.it)('should initialize managers in dependency order', async () => {
            const order = [];
            registry.register('dependent', () => createMockManager({
                initializeImpl: () => {
                    order.push('dependent');
                },
            }), { dependsOn: ['dependency'] });
            registry.register('dependency', () => createMockManager({
                initializeImpl: () => {
        ...
        (0, vitest_1.it)('should initialize managers by priority', async () => {
            const order = [];
            registry.register('low', () => createMockManager({
                initializeImpl: () => {
                    order.push('low');
                },
            }), { priority: 1 });
            registry.register('high', () => createMockManager({
                initializeImpl: () => {
                    order.push('high');
                },
            }), { priority: 10 });
            registry.register('medium', () => createMockManager({
                initializeImpl: () => {
                    order.push('medium');
                },
            }), { priority: 5 });
        ...
        (0, vitest_1.it)('should throw when initializing after disposal', async () => {
            registry.disposeAll();
            await (0, vitest_1.expect)(registry.initializeAll()).rejects.toThrow(/Cannot initialize after disposal/);
        });
        (0, vitest_1.it)('should propagate initialization errors', async () => {
            const error = new Error('Init failed');
            registry.register('failing', () => createMockManager({
                initializeImpl: () => {
                    throw error;
                },
            }));
    ...
    (0, vitest_1.describe)('disposeAll', () => {
        ...
        (0, vitest_1.it)('should dispose in reverse order (LIFO)', () => {
            const order = [];
            registry.register('first', () => createMockManager({
                disposeImpl: () => {
                    order.push('first');
                },
            }));
            registry.register('second', () => createMockManager({
                disposeImpl: () => {
                    order.push('second');
                },
            }));
            registry.register('third', () => createMockManager({
                disposeImpl: () => {
                    order.push('third');
                },
            }));
    ...
    (0, vitest_1.describe)('Complex Dependency Scenarios', () => {
        (0, vitest_1.it)('should handle deep dependency chains', async () => {
            const order = [];
            registry.register('level3', () => createMockManager({
                // @ts-expect-error - test mock type
                initializeImpl: () => order.push('level3'),
            }), { dependsOn: ['level2'] });
            registry.register('level2', () => createMockManager({
                // @ts-expect-error - test mock type
                initializeImpl: () => order.push('level2'),
            }), { dependsOn: ['level1'] });
            registry.register('level1', () => createMockManager({
                // @ts-expect-error - test mock type
        ...
        (0, vitest_1.it)('should handle multiple dependencies', async () => {
            const order = [];
            registry.register('final', () => createMockManager({
                // @ts-expect-error - test mock type
                initializeImpl: () => order.push('final'),
            }), { dependsOn: ['dep1', 'dep2'] });
            registry.register('dep1', () => createMockManager({
                // @ts-expect-error - test mock type
                initializeImpl: () => order.push('dep1'),
            }));
            registry.register('dep2', () => createMockManager({
                // @ts-expect-error - test mock type
        ...
        (0, vitest_1.it)('should handle diamond dependency pattern', async () => {
            const order = [];
            registry.register('base', () => createMockManager({
                // @ts-expect-error - test mock type
                initializeImpl: () => order.push('base'),
            }));
            registry.register('left', () => createMockManager({
                // @ts-expect-error - test mock type
                initializeImpl: () => order.push('left'),
            }), { dependsOn: ['base'] });
            registry.register('right', () => createMockManager({
                // @ts-expect-error - test mock type
                initializeImpl: () => order.push('right'),
            }), { dependsOn: ['base'] });
            registry.register('top', () => createMockManager({
                // @ts-expect-error - test mock type
    ...
    (0, vitest_1.describe)('Edge Cases', () => {
        ...
        (0, vitest_1.it)('should handle single manager', async () => {
            const initFn = vi.fn();
            const disposeFn = vi.fn();
            registry.register('single', () => createMockManager({

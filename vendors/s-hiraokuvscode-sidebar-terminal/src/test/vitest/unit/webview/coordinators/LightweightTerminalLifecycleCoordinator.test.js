...
(0, vitest_1.describe)('LightweightTerminalLifecycleCoordinator', () => {
  ...
  (0, vitest_1.beforeEach)(() => {
    ...
    coordinator =
      new LightweightTerminalLifecycleCoordinator_1.LightweightTerminalLifecycleCoordinator(
        dependencies
      );
  });
  ...
  (0, vitest_1.it)(
    'skips duplicate terminal creation requests that are already pending',
    async () => {
      ...
      (0, vitest_1.expect)(
        dependencies.terminalLifecycleManager.createTerminal
      ).not.toHaveBeenCalled();
      (0, vitest_1.expect)(
        dependencies.terminalOperations.markTerminalCreationPending
      ).not.toHaveBeenCalled();
    }
  );
  (0, vitest_1.it)(
    'creates a terminal and performs tab, persistence, and extension post-processing',
    async () => {
      const terminal = await coordinator.createTerminal({
      ...
      (0, vitest_1.expect)(
        dependencies.terminalOperations.markTerminalCreationPending
      ).toHaveBeenCalledWith('terminal-2');
      (0, vitest_1.expect)(
        dependencies.terminalLifecycleManager.createTerminal
      ).toHaveBeenCalledWith('terminal-2', 'Terminal 2', undefined, undefined);
      (0, vitest_1.expect)(dependencies.terminalTabManager.addTab).toHaveBeenCalledWith(
        'terminal-2',
        'Terminal 2',
        vitest_1.expect.anything()
      );
      (0, vitest_1.expect)(dependencies.webViewPersistenceService.addTerminal).toHaveBeenCalledWith(
        'terminal-2',
        vitest_1.expect.anything(),
        { autoSave: true }
      );
      (0, vitest_1.expect)(dependencies.postMessageToExtension).toHaveBeenCalledWith(
        vitest_1.expect.objectContaining({ command: 'createTerminal', terminalId: 'terminal-2' })
      );
      (0, vitest_1.expect)(
        dependencies.terminalOperations.clearTerminalCreationPending
      ).toHaveBeenCalledWith('terminal-2');
    }
  );
  (0, vitest_1.it)('removes a terminal and schedules session persistence refresh', async () => {
    ...
    (0, vitest_1.expect)(
      dependencies.cliAgentStateManager.removeTerminalState
    ).toHaveBeenCalledWith('terminal-3');
    (0, vitest_1.expect)(
      dependencies.webViewPersistenceService.removeTerminal
    ).toHaveBeenCalledWith('terminal-3');
    (0, vitest_1.expect)(dependencies.terminalTabManager.removeTab).toHaveBeenCalledWith(
      'terminal-3'
    );
    (0, vitest_1.expect)(dependencies.terminalLifecycleManager.removeTerminal).toHaveBeenCalledWith(
      'terminal-3'
    );
  ...
  (0, vitest_1.it)(
    'removes terminal performance buffers before lifecycle removal when an instance exists',
    async () => {
      const terminal = { write: vitest_1.vi.fn() };
      dependencies.getTerminalInstance.mockReturnValue({ terminal });
      const result = await coordinator.removeTerminal('terminal-3');
      (0, vitest_1.expect)(result).toBe(true);
      (0, vitest_1.expect)(dependencies.performanceManager.removeTerminal).toHaveBeenCalledWith(
        terminal
      );
      (0, vitest_1.expect)(
        dependencies.terminalLifecycleManager.removeTerminal
      ).toHaveBeenCalledWith('terminal-3');
    }
  );
  (0, vitest_1.it)('updates borders after a successful terminal switch', async () => {
    ...
    (0, vitest_1.expect)(result).toBe(true);
    (0, vitest_1.expect)(
      dependencies.terminalLifecycleManager.switchToTerminal
    ).toHaveBeenCalledWith('terminal-4');
    (0, vitest_1.expect)(dependencies.uiManager.updateTerminalBorders).toHaveBeenCalledWith(
      'terminal-4',
      vitest_1.expect.any(Map)
    );
  });
  (0, vitest_1.it)(
    'prepares display for deletion by exiting fullscreen when multiple terminals exist',
    () => {
      ...
      (0, vitest_1.expect)(dependencies.displayModeManager.setDisplayMode).toHaveBeenCalledWith(
        'split'
      );
    }
  );

...
function createCoordinator() {
    ...
    return {
        coordinator: new LightweightTerminalInitializationCoordinator_1.LightweightTerminalInitializationCoordinator(deps),
        deps,
    };
}
(0, vitest_1.describe)('LightweightTerminalInitializationCoordinator', () => {
    ...
    (0, vitest_1.it)('initializes and wires the existing managers bundle', () => {
        ...
        (0, vitest_1.expect)(initialized.performanceManager.initializePerformance).toHaveBeenCalledWith(deps.managerCoordinator);
        (0, vitest_1.expect)(initialized.uiManager.setActiveBorderMode).toHaveBeenCalledWith('multipleOnly');
        (0, vitest_1.expect)(deps.fontSettingsService.setApplicator).toHaveBeenCalledWith(initialized.uiManager);
        (0, vitest_1.expect)(initialized.terminalTabManager.setCoordinator).toHaveBeenCalledWith(deps.managerCoordinator);
        (0, vitest_1.expect)(initialized.inputManager.initialize).toHaveBeenCalled();
        (0, vitest_1.expect)(initialized.configManager.setFontSettingsService).toHaveBeenCalledWith(deps.fontSettingsService);
        (0, vitest_1.expect)(initialized.messageManager.setCoordinator).toHaveBeenCalledWith(deps.managerCoordinator);
        (0, vitest_1.expect)(deps.findInTerminalManager.setCoordinator).toHaveBeenCalledWith(deps.managerCoordinator);
        (0, vitest_1.expect)(deps.profileManager.setCoordinator).toHaveBeenCalledWith(deps.managerCoordinator);
        (0, vitest_1.expect)(deps.shellIntegrationManager.setCoordinator).toHaveBeenCalledWith(deps.managerCoordinator);
        (0, vitest_1.expect)(deps.scheduleTimeout).toHaveBeenCalledWith(vitest_1.expect.any(Function), 100);
    ...
    (0, vitest_1.it)('registers message, window, and unload event handlers', async () => {
        ...
        (0, vitest_1.expect)(deps.eventHandlerManager.setMessageEventHandler).toHaveBeenCalledWith(vitest_1.expect.any(Function));
        (0, vitest_1.expect)(document.addEventListener).toHaveBeenCalledWith('settings-open-requested', vitest_1.expect.any(Function));
        (0, vitest_1.expect)(window.addEventListener).toHaveBeenCalledWith('focus', handlers.onWindowFocus);
        (0, vitest_1.expect)(window.addEventListener).toHaveBeenCalledWith('blur', handlers.onWindowBlur);
        (0, vitest_1.expect)(deps.eventHandlerManager.onPageUnload).toHaveBeenCalledWith(vitest_1.expect.any(Function));
        ...
        (0, vitest_1.expect)(messageManager.receiveMessage).toHaveBeenCalledWith({ command: 'ping' }, deps.managerCoordinator);
        handlers.onWindowFocus();
        handlers.onWindowBlur();
        (0, vitest_1.expect)(deps.postMessageToExtension).toHaveBeenCalledWith(vitest_1.expect.objectContaining({ command: 'terminalFocused', terminalId: 'terminal-1' }));
        (0, vitest_1.expect)(deps.postMessageToExtension).toHaveBeenCalledWith(vitest_1.expect.objectContaining({ command: 'terminalBlurred', terminalId: 'terminal-1' }));
    });
});

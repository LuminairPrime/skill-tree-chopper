...
(0, vitest_1.describe)('1.0.0 release readiness', () => {
    (0, vitest_1.it)('uses stable release package metadata', () => {
        const packageJson = JSON.parse(node_fs_1.default.readFileSync(node_path_1.default.join(repoRoot, 'package.json'), 'utf8'));
        (0, vitest_1.expect)(packageJson.version).toBe('1.0.0');
        (0, vitest_1.expect)(packageJson.description).toBe('Sidebar terminal for VS Code with AI agent awareness (Claude Code, Copilot, Gemini, Codex), split views, session persistence, and full IME support.');
    ...
    (0, vitest_1.it)('documents the 1.0.0 stable release at the top of the changelog', () => {
        const changelog = node_fs_1.default.readFileSync(node_path_1.default.join(repoRoot, 'CHANGELOG.md'), 'utf8');
        const stableReleaseHeading = /^## \[1\.0\.0\]\(https:\/\/github\.com\/s-hiraoku\/vscode-sidebar-terminal\/compare\/v0\.6\.3\.\.\.v1\.0\.0\) \(\d{4}-\d{2}-\d{2}\)/m;

...
(0, vitest_1.describe)('ExtensionLifecycle', () => {
  ...
  (0, vitest_1.describe)('activate() - successful initialization', () => {
    ...
    (0, vitest_1.it)(
      'should initialize ExtensionPersistenceService and expose it via getter',
      async () => {
        const ctx = createMockContext();
        await lifecycle.activate(ctx);
    ...
    (0, vitest_1.it)(
      'should register webview view provider with retainContextWhenHidden',
      async () => {
        const ctx = createMockContext();
        await lifecycle.activate(ctx);
        (0, vitest_1.expect)(mocks.registerWebviewViewProvider).toHaveBeenCalledWith(
          'secondaryTerminal',
    ...
    (0, vitest_1.it)('should track activation duration when telemetry is available', async () => {
      const ctx = createMockContext();
      await lifecycle.activate(ctx);
      (0, vitest_1.expect)(mocks.telemetryTrackActivation).toHaveBeenCalledWith(
    ...
    (0, vitest_1.it)('should log lifecycle start and completion messages', async () => {
      const ctx = createMockContext();
      await lifecycle.activate(ctx);
      (0, vitest_1.expect)(mocks.loggerLifecycle).toHaveBeenCalledWith(
        'Sidebar Terminal activation started',
        vitest_1.expect.objectContaining({ mode: 'Development', version: '1.0.0' })
      );
      (0, vitest_1.expect)(mocks.loggerLifecycle).toHaveBeenCalledWith(
    ...
    (0, vitest_1.it)('should read version from the extension manifest', async () => {
      ...
      (0, vitest_1.expect)(mocks.loggerLifecycle).toHaveBeenCalledWith(
        'Sidebar Terminal activation started',
        vitest_1.expect.objectContaining({ version: '2.5.0' })
      );
    });
    (0, vitest_1.it)('should default version to "unknown" when extension not found', async () => {
      ...
      (0, vitest_1.expect)(mocks.loggerLifecycle).toHaveBeenCalledWith(
        'Sidebar Terminal activation started',
        vitest_1.expect.objectContaining({ version: 'unknown' })
      );
    });
  });
  // ────────────────────────────────────────────────────────────────────
  ...
  (0, vitest_1.describe)('activate() - graceful failure handling', () => {
    (0, vitest_1.it)('should continue when TelemetryService throws', async () => {
      ...
      const ctx = createMockContext();
      await (0, vitest_1.expect)(lifecycle.activate(ctx)).resolves.toBeUndefined();
      (0, vitest_1.expect)(mocks.loggerWarn).toHaveBeenCalledWith(
        'Telemetry service unavailable; continuing without analytics',
        vitest_1.expect.any(Error)
      );
    ...
    (0, vitest_1.it)(
      'should continue when Phase 8 (decorations/links) services throw',
      async () => {
    ...
    (0, vitest_1.it)(
      'should show error message and resolve when TerminalManager throws',
      async () => {
        mocks.terminalManagerShouldThrow = true;
        const ctx = createMockContext();
        await (0, vitest_1.expect)(lifecycle.activate(ctx)).resolves.toBeUndefined();
        (0, vitest_1.expect)(mocks.loggerError).toHaveBeenCalledWith(
          'Failed to activate Sidebar Terminal extension',
          vitest_1.expect.any(Error)
        );
        (0, vitest_1.expect)(mocks.showErrorMessage).toHaveBeenCalledWith(
          vitest_1.expect.stringContaining('TerminalManager init failed')
        );
      }
    );
    (0, vitest_1.it)(
      'should track error via telemetry when activation fails (if telemetry is available)',
      async () => {
        // Telemetry succeeds but TerminalManager throws
        mocks.terminalManagerShouldThrow = true;
        const ctx = createMockContext();
        await lifecycle.activate(ctx);
        (0, vitest_1.expect)(mocks.telemetryTrackError).toHaveBeenCalledWith(
          vitest_1.expect.any(Error),
          'activation'
        );
      }
    ...
    (0, vitest_1.it)(
      'should still resolve the promise on catastrophic failure (prevents spinner hang)',
      async () => {
        mocks.terminalManagerShouldThrow = true;
        const ctx = createMockContext();
  ...
  (0, vitest_1.describe)('telemetry event listeners', () => {
    ...
    (0, vitest_1.it)(
      'should skip telemetry listener setup when telemetry is unavailable',
      async () => {
        mocks.telemetryShouldThrow = true;
        const ctx = createMockContext();
        await lifecycle.activate(ctx);
  ...
  (0, vitest_1.describe)('getExtensionModeLabel()', () => {
    (0, vitest_1.it)('should label Production mode correctly', async () => {
      ...
      (0, vitest_1.expect)(mocks.loggerLifecycle).toHaveBeenCalledWith(
        'Sidebar Terminal activation started',
        vitest_1.expect.objectContaining({ mode: 'Production' })
      );
    });
    (0, vitest_1.it)('should label Development mode correctly', async () => {
      ...
      (0, vitest_1.expect)(mocks.loggerLifecycle).toHaveBeenCalledWith(
        'Sidebar Terminal activation started',
        vitest_1.expect.objectContaining({ mode: 'Development' })
      );
    });
    (0, vitest_1.it)('should label Test mode correctly', async () => {
      ...
      (0, vitest_1.expect)(mocks.loggerLifecycle).toHaveBeenCalledWith(
        'Sidebar Terminal activation started',
        vitest_1.expect.objectContaining({ mode: 'Test' })
      );
    });
  });
});

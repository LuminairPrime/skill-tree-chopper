        ...
        getConfiguration: vitest_1.vi.fn().mockImplementation((section) => ({
            get: vitest_1.vi.fn().mockImplementation((key, defaultValue) => {
                if (section === 'secondaryTerminal') {
                    if (key === 'enablePersistentSessions')
                        return true;
                    if (key === 'persistentSessionScrollback')
                        return 1000;
                    if (key === 'persistentSessionStorageLimit')
                        return 20;
                    if (key === 'persistentSessionExpiryDays')
                        return 7;
                }
                return defaultValue;
            }),
...
(0, vitest_1.describe)('Terminal History Restoration', () => {
    ...
    (0, vitest_1.describe)('RED Phase - Failing Tests (Expected Behavior)', () => {
        (0, vitest_1.it)('should fail: message flow for session restoration should be established', async () => {
            // 1. Arrange
            ...
            const { ExtensionPersistenceService } = await Promise.resolve().then(() => require('../../../../services/persistence/ExtensionPersistenceService'));
            ...
            (0, vitest_1.expect)(mockSidebarProvider.sendMessageToWebview).toHaveBeenCalledWith(vitest_1.expect.objectContaining({
        ...
        (0, vitest_1.it)('should fail: terminal serialization should capture actual content', async () => {
            // 1. Arrange
            const mockTerminalManager = {
            ...
            (0, vitest_1.expect)(mockContext.workspaceState.update).toHaveBeenCalledWith('terminal-session-unified', vitest_1.expect.objectContaining({
                terminals: vitest_1.expect.arrayContaining([
        ...
        (0, vitest_1.it)('should fail: session data should be saved to Extension storage', async () => {
            ...
            // Assert
            (0, vitest_1.expect)(mockContext.workspaceState.update).toHaveBeenCalledWith('terminal-session-unified', vitest_1.expect.objectContaining({ scrollbackData: vitest_1.expect.objectContaining({ t1: ['data'] }) }));
        });
        (0, vitest_1.it)('should fail: session data should be retrieved and sent to WebView', async () => {
            ...
            (0, vitest_1.expect)(mockSidebarProvider.sendMessageToWebview).toHaveBeenCalledWith(vitest_1.expect.objectContaining({
                command: 'restoreTerminalSessions',
                terminals: vitest_1.expect.arrayContaining([
                    vitest_1.expect.objectContaining({ terminalId: 'new-1', scrollbackData: ['history'] }),
                ]),
            }));
        });
        (0, vitest_1.it)('should fail: async terminal readiness should trigger scrollback restoration', async () => {
            ...
            (0, vitest_1.expect)(mockSidebarProvider.sendMessageToWebview).toHaveBeenCalledWith(vitest_1.expect.objectContaining({ command: 'restoreTerminalSessions' }));
        });
    });
    (0, vitest_1.describe)('Real User Scenario Tests (Should Fail Initially)', () => {
        (0, vitest_1.it)('should fail: CLI Agent commands should be preserved across VS Code restarts', async () => {
            // 1. Arrange - Initial State
            const _mockStorage = new Map();
            // Mock Context for First Session (Save)
            const context1 = {
                globalState: { get: vitest_1.vi.fn(), update: vitest_1.vi.fn(), keys: vitest_1.vi.fn().mockReturnValue([]) },
                workspaceState: {
            ...
            (0, vitest_1.expect)(sidebarProvider2.sendMessageToWebview).toHaveBeenCalledWith(vitest_1.expect.objectContaining({
                command: 'restoreTerminalSessions',
                terminals: vitest_1.expect.arrayContaining([
                    vitest_1.expect.objectContaining({
        ...
        (0, vitest_1.it)('should fail: multiple terminals should all restore with history', async () => {
            ...
            const mockTerminalManager = {
                getTerminals: vitest_1.vi.fn().mockReturnValue([]),
                createTerminal: vitest_1.vi.fn().mockReturnValueOnce('t1-new').mockReturnValueOnce('t2-new'),
                setActiveTerminal: vitest_1.vi.fn(),
                reorderTerminals: vitest_1.vi.fn(),
                renameTerminal: vitest_1.vi.fn().mockReturnValue(true),
                updateTerminalHeader: vitest_1.vi.fn().mockReturnValue(true),

...
(0, vitest_1.describe)('UnifiedMessageDispatcher', () => {
    ...
    (0, vitest_1.describe)('dispose - event listener cleanup', () => {
        (0, vitest_1.it)('should remove the message event listener on dispose', async () => {
            ...
            (0, vitest_1.expect)(addEventListenerSpy).toHaveBeenCalledWith('message', vitest_1.expect.any(Function));
            // Get the handler that was registered
            const registeredHandler = addEventListenerSpy.mock.calls.find((call) => call[0] === 'message')?.[1];
            // Dispose should remove the event listener
            dispatcher.dispose();
            (0, vitest_1.expect)(removeEventListenerSpy).toHaveBeenCalledWith('message', registeredHandler);
        });
        ...
        (0, vitest_1.it)('should handle dispose when initialize was not called', () => {
            ...
            const messageRemoveCalls = removeEventListenerSpy.mock.calls.filter((call) => call[0] === 'message');
        ...
        (0, vitest_1.it)('should not call removeEventListener twice on double dispose', async () => {
            await dispatcher.initialize();
            dispatcher.dispose();
            const removeCallCount = removeEventListenerSpy.mock.calls.filter((call) => call[0] === 'message').length;
            // Second dispose should not add more removeEventListener calls
            dispatcher.dispose();
            const newRemoveCallCount = removeEventListenerSpy.mock.calls.filter((call) => call[0] === 'message').length;

...
(0, vitest_1.describe)('SessionCommandHandler', () => {
  ...
  (0, vitest_1.it)('should handle sessionRestoreStarted command', async () => {
    ...
    (0, vitest_1.expect)(mockContext.log).toHaveBeenCalledWith(
      'info',
      vitest_1.expect.stringContaining('Session restore started')
    );
  });
  (0, vitest_1.it)('should handle sessionRestoreProgress command', async () => {
    ...
    (0, vitest_1.expect)(mockContext.log).toHaveBeenCalledWith(
      'debug',
      vitest_1.expect.stringContaining('1/2')
    );
  });
  (0, vitest_1.it)('should handle sessionRestoreCompleted command', async () => {
    ...
    (0, vitest_1.expect)(mockContext.log).toHaveBeenCalledWith(
      'info',
      vitest_1.expect.stringContaining('5 terminals restored')
    );
  });
  ...
  (0, vitest_1.it)('should handle sessionSaved command', async () => {
    ...
    (0, vitest_1.expect)(mockContext.log).toHaveBeenCalledWith(
      'info',
      vitest_1.expect.stringContaining('Session saved: 3 terminals')
    );
  });
  (0, vitest_1.it)('should handle sessionCleared command', async () => {
    ...
    (0, vitest_1.expect)(mockContext.log).toHaveBeenCalledWith(
      'info',
      vitest_1.expect.stringContaining('Session cleared')
    );
  });
  (0, vitest_1.it)('should handle terminalRestoreError command', async () => {
    await handler.handle(
      { command: 'terminalRestoreError', terminalId: 't1', error: 'dead' },
      mockContext
    );
    (0, vitest_1.expect)(mockContext.log).toHaveBeenCalledWith(
      'error',
      vitest_1.expect.stringContaining('Terminal restore error for t1'),
      'dead'
    );
  });
  (0, vitest_1.it)(
    'should continue if terminals missing in sessionRestore (validateRequired is guard only)',
    async () => {

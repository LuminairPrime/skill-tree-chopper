...
function createCoordinator(terminals, activeTerminalManager, cliAgentService) {
    ...
    return { coordinator: new TerminalIOCoordinator_1.TerminalIOCoordinator(terminals, atm, cas), atm, cas };
}
(0, vitest_1.describe)('TerminalIOCoordinator - PTY recovery', () => {
    (0, vitest_1.it)('retries using terminal.pty when primary is undefined and first write fails', () => {
        ...
        const coordinator = new TerminalIOCoordinator_1.TerminalIOCoordinator(terminals, activeTerminalManager, mockCliAgentService);
...
(0, vitest_1.describe)('TerminalIOCoordinator - CLI agent input pipeline', () => {
    (0, vitest_1.it)('does not connect on keystrokes alone but connects when Enter submits the command', () => {
        const ptyWrite = vitest_1.vi.fn();
        const terminal = createMockTerminal({
            id: 'term-1',
        ...
        const coordinator = new TerminalIOCoordinator_1.TerminalIOCoordinator(terminals, activeTerminalManager, cliAgentService);
...
(0, vitest_1.describe)('TerminalIOCoordinator - resolveTerminalId (via sendInput)', () => {
    ...
    (0, vitest_1.it)('falls back to first available when both explicit and active are invalid', () => {
        const ptyWrite = vitest_1.vi.fn();
        const terminal = createMockTerminal({
            id: 'first',
            name: 'First',
...
(0, vitest_1.describe)('TerminalIOCoordinator - attemptPtyRecovery (via sendInput)', () => {
    ...
    (0, vitest_1.it)('returns false when only ptyProcess exists and it fails (no pty alternative)', () => {
        const ptyProcessWrite = vitest_1.vi.fn(() => {

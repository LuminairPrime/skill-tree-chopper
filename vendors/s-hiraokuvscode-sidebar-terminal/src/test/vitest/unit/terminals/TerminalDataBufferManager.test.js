...
(0, vitest_1.describe)('TerminalDataBufferManager', () => {
    (0, vitest_1.it)('strips CSI 3 J (erase scrollback) even when split across chunks', () => {
        ...
        const bufferManager = new TerminalDataBufferManager_1.TerminalDataBufferManager(terminals, emitter, cliAgentService);
        const received = [];
    ...
    (0, vitest_1.it)('runs termination detection when an agent is active', () => {
        ...
        bufferManager.flushBuffer('terminal-1');
        (0, vitest_1.expect)(cliAgentService.handleOutputChunk).toHaveBeenCalledWith('terminal-1', 'user@host:~$ ');
        bufferManager.dispose();
    });
    (0, vitest_1.it)('skips termination detection when no agent is active', () => {
        ...
        bufferManager.flushBuffer('terminal-1');
        (0, vitest_1.expect)(cliAgentService.handleOutputChunk).toHaveBeenCalledWith('terminal-1', 'plain shell output');
        bufferManager.dispose();
    });
    ...
    (0, vitest_1.it)('delegates each flush to a single handleOutputChunk call', () => {
        ...
        (0, vitest_1.expect)(cliAgentService.handleOutputChunk).toHaveBeenCalledWith('terminal-1', 'Gemini CLI v0.1.0\r\ngemini > ');
        bufferManager.dispose();
    });
});

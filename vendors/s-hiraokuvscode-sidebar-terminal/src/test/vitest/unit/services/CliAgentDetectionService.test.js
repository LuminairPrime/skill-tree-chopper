...
(0, vitest_1.describe)('🧪 CLI Agent Detection Service - Comprehensive Test Suite', () => {
    let detectionService;
    let stateManager;
    // Test event tracking
    let statusChangeEvents = [];
    (0, vitest_1.beforeEach)(() => {
        statusChangeEvents = [];
        // Create fresh instances for each test
        // @ts-expect-error - test mock type
        stateManager = new CliAgentStateStore_1.CliAgentStateStore();
    ...
    vitest_1.describe.skip('🎯 Real Claude Code Output Pattern Tests', () => {
        ...
        realClaudeOutputs.forEach((output, index) => {
            (0, vitest_1.it)(`should detect Claude startup from real output #${index + 1}: "${output}"`, () => {
                // ACT
                const result = detectionService.detectFromOutput('term1', output);
                // ASSERT
                (0, vitest_1.expect)(result).not.toBeNull();
                if (result) {
                    (0, vitest_1.expect)(result.type).toBe('claude');
                ...
                (0, vitest_1.expect)(statusChangeEvents).toHaveLength(1);
                if (statusChangeEvents.length > 0) {
                    (0, vitest_1.expect)(statusChangeEvents[0]?.status).toBe('connected');
                    (0, vitest_1.expect)(statusChangeEvents[0]?.type).toBe('claude');
                }
            });
        });
        realClaudeNonStartupOutputs.forEach((output, index) => {
            (0, vitest_1.it)(`should NOT detect Claude startup from non-startup output #${index + 1}: "${output}"`, () => {
                // ACT
                const result = detectionService.detectFromOutput('term1', output);
                // ASSERT
                ...
                (0, vitest_1.expect)(statusChangeEvents).toHaveLength(0);
            });
        });
    });
    vitest_1.describe.skip('🎯 Real Gemini CLI Output Pattern Tests', () => {
        ...
        realGeminiOutputs.forEach((output, index) => {
            (0, vitest_1.it)(`should detect Gemini startup from real output #${index + 1}: "${output}"`, () => {
                // ACT
                const result = detectionService.detectFromOutput('term1', output);
                // ASSERT
                (0, vitest_1.expect)(result).not.toBeNull();
                if (result) {
                    (0, vitest_1.expect)(result.type).toBe('gemini');
                    (0, vitest_1.expect)(result.confidence).toBeGreaterThan(0.8);
                    (0, vitest_1.expect)(result.source).toBe('output');
                }
                // Check state change
                const state = detectionService.getAgentState('term1');
                (0, vitest_1.expect)(state.status).toBe('connected');
                (0, vitest_1.expect)(state.agentType).toBe('gemini');
                // Check event was fired
                (0, vitest_1.expect)(statusChangeEvents).toHaveLength(1);
                if (statusChangeEvents.length > 0) {
                    (0, vitest_1.expect)(statusChangeEvents[0]?.status).toBe('connected');
                    (0, vitest_1.expect)(statusChangeEvents[0]?.type).toBe('gemini');
                }
            });
        });
        realGeminiNonStartupOutputs.forEach((output, index) => {
            (0, vitest_1.it)(`should NOT detect Gemini startup from non-startup output #${index + 1}: "${output}"`, () => {
                // ACT
                const result = detectionService.detectFromOutput('term1', output);
                // ASSERT
                (0, vitest_1.expect)(result).toBeNull();
    ...
    (0, vitest_1.describe)('🔄 Status Transition Tests (Critical for Bug Prevention)', () => {
        (0, vitest_1.it)('should handle complete Claude lifecycle: none → connected → disconnected → none', () => {
            ...
            (0, vitest_1.expect)(state1.status).toBe('disconnected');
            (0, vitest_1.expect)(state1.agentType).toBe('claude');
            (0, vitest_1.expect)(state2.status).toBe('connected');
            (0, vitest_1.expect)(state2.agentType).toBe('gemini');
            (0, vitest_1.expect)(statusChangeEvents).toHaveLength(3); // connected(claude), disconnected(claude), connected(gemini)
            ...
            (0, vitest_1.expect)(finalState1.status).toBe('none');
            (0, vitest_1.expect)(finalState1.agentType).toBeNull();
            (0, vitest_1.expect)(finalState2.status).toBe('connected'); // Should remain connected
            if (statusChangeEvents.length > 0) {
                (0, vitest_1.expect)(statusChangeEvents[statusChangeEvents.length - 1]?.status).toBe('none');
            }
        });
        (0, vitest_1.it)('should handle Gemini complete lifecycle: none → connected → none', () => {
            ...
            const terminationResult = detectionService.detectTermination('term1', 'Agent powering down. Goodbye!\nuser@hostname:~/project$ ');
            // ASSERT 2: Termination detected
            (0, vitest_1.expect)(terminationResult.isTerminated).toBe(true);
            (0, vitest_1.expect)(terminationResult.reason).toBe('Explicit termination pattern');
            // Check final state is none
            state = detectionService.getAgentState('term1');
            (0, vitest_1.expect)(state.status).toBe('none');
        });
        (0, vitest_1.it)('🚨 REGRESSION TEST: should NOT cause connected/disconnected flickering', () => {
    ...
    (0, vitest_1.describe)('🤝 Multiple Agent Management Tests', () => {
        (0, vitest_1.it)('should handle switching between Claude and Gemini agents', () => {
            // ACT 1: Start Claude
            detectionService.detectFromOutput('term1', 'Welcome to Claude Code!');
            ...
            (0, vitest_1.expect)(claudeState.status).toBe('disconnected');
            (0, vitest_1.expect)(claudeState.agentType).toBe('claude');
            (0, vitest_1.expect)(geminiState.status).toBe('connected');
            (0, vitest_1.expect)(geminiState.agentType).toBe('gemini');
            // Check events sequence
            (0, vitest_1.expect)(statusChangeEvents).toHaveLength(3);
            (0, vitest_1.expect)(statusChangeEvents[0]).toMatchObject({
                terminalId: 'term1',
                status: 'connected',
                type: 'claude',
            });
            // Implementation fires new connection event BEFORE disconnecting previous
            (0, vitest_1.expect)(statusChangeEvents[1]).toMatchObject({
                terminalId: 'term2',
                status: 'connected',
                type: 'gemini',
            });
            (0, vitest_1.expect)(statusChangeEvents[2]).toMatchObject({
                terminalId: 'term1',
                status: 'disconnected',
                type: 'claude',
            });
        });
        (0, vitest_1.it)('should handle agent switching via toggle button', () => {
            // ARRANGE: Start both agents
            detectionService.detectFromOutput('term1', 'Welcome to Claude Code!');
            detectionService.detectFromOutput('term2', 'Welcome to Gemini');
            // Verify initial state: Gemini connected, Claude disconnected
            (0, vitest_1.expect)(detectionService.getAgentState('term1').status).toBe('disconnected');
            (0, vitest_1.expect)(detectionService.getAgentState('term2').status).toBe('connected');
            ...
            (0, vitest_1.expect)(switchResult.success).toBe(true);
            (0, vitest_1.expect)(switchResult.newStatus).toBe('connected');
            (0, vitest_1.expect)(switchResult.agentType).toBe('claude');
            // Verify final state: Claude connected, Gemini disconnected
            (0, vitest_1.expect)(detectionService.getAgentState('term1').status).toBe('connected');
            (0, vitest_1.expect)(detectionService.getAgentState('term2').status).toBe('disconnected');
        });
        (0, vitest_1.it)('should reconnect disconnected terminal with its own detected agent type', () => {
            detectionService.detectFromInput('term1', 'codex\r');
            detectionService.detectFromInput('term2', 'gemini\r');
            (0, vitest_1.expect)(detectionService.getAgentState('term1').status).toBe('disconnected');
            (0, vitest_1.expect)(detectionService.getAgentState('term1').agentType).toBe('codex');
            (0, vitest_1.expect)(detectionService.getAgentState('term2').status).toBe('connected');
            (0, vitest_1.expect)(detectionService.getAgentState('term2').agentType).toBe('gemini');
            vitest_1.vi.advanceTimersByTime(3000);
            const switchResult = detectionService.switchAgentConnection('term1');
            (0, vitest_1.expect)(switchResult.success).toBe(true);
            (0, vitest_1.expect)(switchResult.newStatus).toBe('connected');
            (0, vitest_1.expect)(switchResult.agentType).toBe('codex');
            (0, vitest_1.expect)(detectionService.getAgentState('term1').status).toBe('connected');
            (0, vitest_1.expect)(detectionService.getAgentState('term1').agentType).toBe('codex');
            (0, vitest_1.expect)(detectionService.getAgentState('term2').status).toBe('disconnected');
        });
        (0, vitest_1.it)('should not switch when terminal has no detected AI agent', () => {
            ...
            (0, vitest_1.expect)(switchResult.newStatus).toBe('none');
            (0, vitest_1.expect)(switchResult.agentType).toBeNull();
            (0, vitest_1.expect)(switchResult.reason).toContain('No detected AI agent');
        });
        (0, vitest_1.it)('should prevent promotion of disconnected agents via output re-processing', () => {
    ...
    (0, vitest_1.describe)('🔚 Termination Detection Tests', () => {
        ...
        realShellPrompts.forEach((prompt, index) => {
            ...
            (0, vitest_1.it)(`should detect termination from shell prompt #${index + 1}: "${prompt}"`, () => {
                // ACT
                ...
                (0, vitest_1.expect)(result.detectedLine).toContain(prompt.trim());
            });
        });
        ...
        realExitCommands.forEach((command, index) => {
            (0, vitest_1.it)(`should NOT detect termination from bare exit command #${index + 1}: "${command}"`, () => {
                // ACT
        ...
        (0, vitest_1.it)('should detect termination from real agent output: [Process completed]', () => {
        ...
        (0, vitest_1.it)('should detect termination from real agent output: Agent powering down. Goodbye!', () => {
            const result = detectionService.detectTermination('term1', 'Agent powering down. Goodbye!');
            (0, vitest_1.expect)(result.isTerminated).toBe(true);
            (0, vitest_1.expect)(result.reason).toBe('Explicit termination pattern');
        });
        (0, vitest_1.it)('should handle agent termination and state cleanup', () => {
            // ARRANGE: Verify agent is connected
            (0, vitest_1.expect)(detectionService.getAgentState('term1').status).toBe('connected');
            // ACT: Detect termination
            const terminationResult = detectionService.detectTermination('term1', 'user@hostname:~/project$ ');
            (0, vitest_1.expect)(terminationResult.isTerminated).toBe(true);
            ...
            (0, vitest_1.expect)(finalState.status).toBe('none');
            (0, vitest_1.expect)(finalState.agentType).toBeNull();
            // Check event was fired
            (0, vitest_1.expect)(statusChangeEvents).toHaveLength(1);
            if (statusChangeEvents.length > 0) {
                (0, vitest_1.expect)(statusChangeEvents[0]?.status).toBe('none');
            }
        });
    });
    // =================== ERROR HANDLING AND EDGE CASES ===================
    (0, vitest_1.describe)('⚠️ Error Handling and Edge Cases', () => {
        (0, vitest_1.it)('should handle empty and null data gracefully', () => {
            // ACT & ASSERT: Should not throw errors
            (0, vitest_1.expect)(() => detectionService.detectFromOutput('term1', '')).not.toThrow();
            (0, vitest_1.expect)(() => detectionService.detectFromOutput('term1', '   ')).not.toThrow();
            (0, vitest_1.expect)(() => detectionService.detectFromOutput('term1', '\n\r\n')).not.toThrow();
            // Should return null for empty data
            (0, vitest_1.expect)(detectionService.detectFromOutput('term1', '')).toBeNull();
            (0, vitest_1.expect)(detectionService.detectFromOutput('term1', '   ')).toBeNull();
        });
        (0, vitest_1.it)('should handle ANSI escape sequences in terminal output', () => {
        ...
        (0, vitest_1.it)('should handle terminal removal cleanup', () => {
        ...
        (0, vitest_1.it)('should handle concurrent agent operations', () => {
            ...
            (0, vitest_1.expect)(state1.status).toBe('none');
            (0, vitest_1.expect)(['connected', 'disconnected']).toContain(state2.status);
            (0, vitest_1.expect)(['connected', 'disconnected']).toContain(state3.status);
            // At least one should be connected
            const connectedStates = [state1, state2, state3].filter((s) => s.status === 'connected');
            (0, vitest_1.expect)(connectedStates.length).toBeGreaterThanOrEqual(0);
        });
    });
    // =================== INPUT DETECTION TESTS ===================
    (0, vitest_1.describe)('⌨️ Input Detection Tests', () => {
        ...
        startupCommands.forEach((command, index) => {
            (0, vitest_1.it)(`should detect agent startup from input command #${index + 1}: "${command}"`, () => {
                ...
                if (result) {
                    ...
                    (0, vitest_1.expect)(result.type).toBe(expectedType);
                }
            });
        });
        (0, vitest_1.it)('should ignore non-agent input commands', () => {
        ...
        (0, vitest_1.it)('should detect wrapped and prefixed launcher commands', () => {
            (0, vitest_1.expect)(detectionService.detectFromInput('term1', 'FOO=1 codex\r')?.type).toBe('codex');
            (0, vitest_1.expect)(detectionService.detectFromInput('term1', 'npx @openai/codex@latest\r')?.type).toBe('codex');
            (0, vitest_1.expect)(detectionService.detectFromInput('term1', 'pnpm dlx @google/gemini-cli\r')?.type).toBe('gemini');
            (0, vitest_1.expect)(detectionService.detectFromInput('term1', 'yarn dlx @anthropic-ai/claude-code\r')?.type).toBe('claude');
            (0, vitest_1.expect)(detectionService.detectFromInput('term1', 'bunx opencode\r')?.type).toBe('opencode');
            (0, vitest_1.expect)(detectionService.detectFromInput('term1', 'gh copilot suggest\r')?.type).toBe('copilot');
        });
    });
    // =================== PERFORMANCE AND CACHING TESTS ===================
    (0, vitest_1.describe)('⚡ Performance and Caching Tests', () => {
        vitest_1.it.skip('should use debouncing to prevent excessive detection calls', () => {
            ...
            (0, vitest_1.expect)(nullResults.length).toBeGreaterThan(0);
        });
        (0, vitest_1.it)('should cache detection results for identical data', () => {
            ...
            (0, vitest_1.expect)(result1).toBeNull(); // Not a detection
            (0, vitest_1.expect)(result2).toBeNull(); // Cached result
        });
        (0, vitest_1.it)('should handle large output data efficiently', () => {
    ...
    (0, vitest_1.describe)('🔗 Integration Tests', () => {
        (0, vitest_1.it)('should maintain state consistency across all operations', () => {
            ...
            const terminationResult = detectionService.detectTermination('term1', '[Process completed]\nuser@hostname:~$ ');
            (0, vitest_1.expect)(terminationResult.isTerminated).toBe(true);
            stateManager.setAgentTerminated('term1');
            (0, vitest_1.expect)(detectionService.getAgentState('term1').status).toBe('none');
            ...
            (0, vitest_1.expect)(detectionService.getAgentState('term2').status).toBe('none');
            // FINAL ASSERT: All terminals should be in 'none' state
            (0, vitest_1.expect)(detectionService.getAgentState('term1').status).toBe('none');
            (0, vitest_1.expect)(detectionService.getAgentState('term2').status).toBe('none');
        });
        (0, vitest_1.it)('🚨 BUG REPRODUCTION: Agent status becomes "none" when it should stay "connected"', () => {
            ...
            const finalState = detectionService.getAgentState('term1');
            (0, vitest_1.expect)(finalState.status).toBe('connected'); // BUG DETECTED: Agent status changed to "none" when it should remain "connected"
            (0, vitest_1.expect)(finalState.agentType).toBe('claude');
            // Should have no additional status change events
            (0, vitest_1.expect)(statusChangeEvents).toHaveLength(0); // BUG DETECTED: Unexpected status change events fired during normal output
        });
        (0, vitest_1.it)('should terminate connected agent when Ctrl+C is followed by shell prompt', () => {
            detectionService.detectFromOutput('term1', 'Welcome to Claude Code!');
            ...
            (0, vitest_1.expect)(terminationResult.isTerminated).toBe(true);
            (0, vitest_1.expect)(terminationResult.reason).toBe('Interrupt followed by shell prompt');
            (0, vitest_1.expect)(detectionService.getAgentState('term1').status).toBe('none');
        });
        (0, vitest_1.it)('should terminate connected agent when Ctrl+C is pressed twice quickly', () => {
            detectionService.detectFromOutput('term1', 'Welcome to Claude Code!');
            detectionService.detectFromInput('term1', '\x03');
            (0, vitest_1.expect)(detectionService.getAgentState('term1').status).toBe('connected');
        ...
        (0, vitest_1.it)('should terminate connected agent when Ctrl+C is followed by decorated zsh prompt', () => {
            detectionService.detectFromOutput('term1', 'Welcome to Claude Code!');
            detectionService.detectFromOutput('term1', 'Claude is thinking...');
            detectionService.detectFromInput('term1', '\x03');
            const terminationResult = detectionService.detectTermination('term1', '^C\n➜ myproject git:(main) ✗ ');
        ...
        (0, vitest_1.it)('should terminate connected agent on shell integration SIGINT completion', () => {
            detectionService.detectFromOutput('term1', 'Welcome to Claude Code!');
            const terminationResult = detectionService.detectTermination('term1', '\x1b]633;C;130\x07');
            (0, vitest_1.expect)(terminationResult.isTerminated).toBe(true);
            (0, vitest_1.expect)(terminationResult.reason).toBe('Shell integration command finished');
            (0, vitest_1.expect)(detectionService.getAgentState('term1').status).toBe('none');
        });
    });
    // =================== HEARTBEAT AND MONITORING TESTS ===================
    (0, vitest_1.describe)('💓 Heartbeat and Monitoring Tests', () => {
        (0, vitest_1.it)('should keep heartbeat startup idempotent', () => {
            const setTimeoutSpy = vitest_1.vi.spyOn(globalThis, 'setTimeout');
            const clearTimeoutSpy = vitest_1.vi.spyOn(globalThis, 'clearTimeout');
            detectionService.startHeartbeat();
            detectionService.startHeartbeat();
            (0, vitest_1.expect)(setTimeoutSpy).toHaveBeenCalledTimes(2);
            (0, vitest_1.expect)(clearTimeoutSpy).toHaveBeenCalledTimes(1);
        });
        (0, vitest_1.it)('should provide agent state refresh functionality', () => {
            ...
            (0, vitest_1.expect)(refreshResult).toBe(true);
        });
        (0, vitest_1.it)('should handle heartbeat validation without side effects', () => {
            // ARRANGE: Start agent
            detectionService.detectFromOutput('term1', 'Welcome to Claude Code!');
            ...
            const finalState = detectionService.getAgentState('term1');
            (0, vitest_1.expect)(finalState).toEqual(initialState);
        });
    });
    (0, vitest_1.describe)('🧭 Refactored detection pipeline tests', () => {
        (0, vitest_1.it)('accumulates input chunks until Enter submits the command', () => {
            ...
            detectionService.handleInputChunk('term1', 'i');
            (0, vitest_1.expect)(detectionService.getAgentState('term1')).toEqual({
                status: 'none',
                agentType: null,
            });
        ...
        (0, vitest_1.it)('keeps agent state connected when waiting-like output is received', () => {
            const result = detectionService.handleOutputChunk('term1', 'Gemini CLI v0.1.0\r\ngemini > ');
            (0, vitest_1.expect)(result?.detection).toMatchObject({
                type: 'gemini',
                source: 'output',
            });
            (0, vitest_1.expect)(result?.state.status).toBe('connected');
            (0, vitest_1.expect)(statusChangeEvents).toHaveLength(1);
        });
        (0, vitest_1.it)('keeps agent state stable when the user submits input after waiting-like output', () => {
...
//# sourceMappingURL=CliAgentDetectionService.test.js.map
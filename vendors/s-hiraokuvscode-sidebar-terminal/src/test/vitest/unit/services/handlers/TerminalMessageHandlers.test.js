...
(0, vitest_1.describe)('TerminalMessageHandlers', () => {
    ...
    (0, vitest_1.it)('creates terminals with the requested options', async () => {
        ...
        // When
        await (0, vitest_1.expect)(handler.handle({
            profile: 'zsh',
    ...
    (0, vitest_1.it)('wraps create-terminal failures with handler context', async () => {
        // Given
        const handler = new TerminalMessageHandlers_1.CreateTerminalHandler(dependencies);
        vitest_1.vi.mocked(dependencies.terminalManager.createTerminal).mockRejectedValueOnce(new Error('boom'));
        // When / Then
        await (0, vitest_1.expect)(handler.handle({})).rejects.toThrow('Terminal creation failed: Error: boom');
    });
    (0, vitest_1.it)('requires a terminal id before deleting', async () => {
        ...
        // When / Then
        await (0, vitest_1.expect)(invokeWithPartialPayload({})).rejects.toThrow("Required field 'terminalId' is missing or null");
    });
    (0, vitest_1.it)('passes input and terminal id in the runtime order expected by terminalManager', () => {
        // Given
        const handler = new TerminalMessageHandlers_1.TerminalInputHandler(dependencies);
        // When
        (0, vitest_1.expect)(handler.handle({ terminalId: 'terminal-1', input: 'pwd' })).toEqual({ success: true });
        // Then
        (0, vitest_1.expect)(dependencies.terminalManager.sendInput).toHaveBeenCalledWith('pwd', 'terminal-1');
    });
    (0, vitest_1.it)('rejects resize requests with non-positive dimensions', () => {
        ...
        // When / Then
        (0, vitest_1.expect)(() => handler.handle({
    ...
    (0, vitest_1.it)('delegates valid resize requests to terminalManager', () => {
        ...
        // Then
        (0, vitest_1.expect)(dependencies.terminalManager.resize).toHaveBeenCalledWith(120, 30, 'terminal-1');
    });
    ...
    (0, vitest_1.it)('wraps settings update errors', async () => {
        ...
        vitest_1.vi.mocked(dependencies.configService.updateSettings).mockRejectedValueOnce(new Error('nope'));
        // When / Then
        await (0, vitest_1.expect)(handler.handle({ settings: { theme: 'dark' } })).rejects.toThrow('Settings update failed: Error: nope');
    });
    ...
    (0, vitest_1.it)('uses the active terminal working directory when creating a split terminal', async () => {
        // Given
        ...
        // Then
        (0, vitest_1.expect)(dependencies.terminalManager.getWorkingDirectory).toHaveBeenCalledWith('terminal-1');
        (0, vitest_1.expect)(dependencies.terminalManager.createTerminal).toHaveBeenCalledWith({
            workingDirectory: '/tmp/worktree',
        });
    });
    (0, vitest_1.it)('falls back to safeProcessCwd when the active terminal cwd cannot be resolved', async () => {
        // Given
        const handler = new TerminalMessageHandlers_1.SplitTerminalHandler(dependencies);
        vitest_1.vi.mocked(dependencies.terminalManager.getWorkingDirectory).mockRejectedValueOnce(new Error('cwd unavailable'));
        // When
    ...
    (0, vitest_1.it)('creates and registers the full handler set', () => {
        // Given
        const handlers = TerminalMessageHandlers_1.TerminalMessageHandlerFactory.createAllHandlers(dependencies);
        ...
        // When
        TerminalMessageHandlers_1.TerminalMessageHandlerFactory.registerAllHandlers(router, dependencies);
        // Then

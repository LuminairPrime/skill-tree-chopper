...
(0, vitest_1.describe)('ExtensionPersistenceService', () => {
    ...
    (0, vitest_1.beforeEach)(() => {
        ...
        service = new ExtensionPersistenceService_1.ExtensionPersistenceService(context, terminalManager);
    });
    ...
    (0, vitest_1.it)('prefetchScrollbackForSave updates cache when extraction returns data', async () => {
        const requestStub = vitest_1.vi
            .spyOn(service, 'requestImmediateScrollbackExtraction')
    ...
    (0, vitest_1.it)('reorders restored terminals to match saved session order', async () => {
        ...
        const waitStub = vitest_1.vi.spyOn(service, 'waitForTerminalsReady').mockResolvedValue(undefined);
        ...
        (0, vitest_1.expect)(terminalManager.reorderTerminals.mock.calls[0][0]).toEqual(['new-1', 'new-2', 'new-3']);
    ...
    (0, vitest_1.describe)('indicatorColor persistence', () => {
        (0, vitest_1.it)('saves indicatorColor in session data when present', async () => {
            ...
            await service.saveCurrentSession({ preferCache: true });
            // Find the save call
            const saveCalls = workspaceState.update.mock.calls.filter((call) => call[0] === 'terminal-session-unified' && call[1] != null);
    ...
    (0, vitest_1.describe)('terminal name and indicatorColor restoration', () => {
        (0, vitest_1.it)('restores terminal name via renameTerminal after creation', async () => {
            ...
            await service.batchRestoreTerminals(sessionData);
            (0, vitest_1.expect)(terminalManager.renameTerminal).toHaveBeenCalledWith('new-1', 'My Custom Name');
            (0, vitest_1.expect)(terminalManager.renameTerminal).toHaveBeenCalledWith('new-2', 'Terminal 2');
        });
        (0, vitest_1.it)('restores indicatorColor via updateTerminalHeader after creation', async () => {
            terminalManager.createTerminal.mockReturnValueOnce('new-1').mockReturnValueOnce('new-2');
            vitest_1.vi.spyOn(service, 'waitForTerminalsReady').mockResolvedValue(undefined);
            vitest_1.vi.spyOn(service, 'requestScrollbackRestoration').mockResolvedValue(undefined);
            const sessionData = {
            ...
            (0, vitest_1.expect)(terminalManager.updateTerminalHeader).not.toHaveBeenCalledWith('new-2', vitest_1.expect.anything());
        });

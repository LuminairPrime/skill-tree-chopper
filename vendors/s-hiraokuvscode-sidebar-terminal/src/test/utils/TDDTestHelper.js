...
exports.TDDTestSuite = exports.TestDataFactory = exports.RestoreAssertionHelper = exports.MockManager = exports.PerformanceTestHelper = exports.TDDWorkflowManager = void 0;
...
class PerformanceTestHelper {
    ...
    static validatePerformance(duration, operation, testName) {
        ...
        // アサーションとしても使用可能
        (0, chai_1.expect)(duration).to.be.lessThan(threshold, `Performance test failed: ${testName} took ${duration}ms (max: ${threshold}ms)`);
    }
}
...
class RestoreAssertionHelper {
     ...
     */
    static validateTerminalManagerCalls(mockTerminalManager, expectedCreateCalls, expectedSetActiveCalls, testName) {
        (0, chai_1.expect)(mockTerminalManager.createTerminal.callCount).to.equal(expectedCreateCalls);
        if (expectedSetActiveCalls > 0) {
            (0, chai_1.expect)(mockTerminalManager.setActiveTerminal.callCount).to.equal(expectedSetActiveCalls);
        }
     ...
     * Scrollback復元の検証
     */
    static async validateScrollbackRestore(mockSidebarProvider, expectedTerminalId, expectedLines, delayMs = 2000) {
        // Scrollback復元の遅延を待つ
...
class TDDTestSuite {
    ...
    async runTDDCycle(testName, testImplementation) {
        ...
        try {
            const { result, duration } = await PerformanceTestHelper.measureExecutionTime(testImplementation, testName);

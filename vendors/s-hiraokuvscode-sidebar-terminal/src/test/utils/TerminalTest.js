    ...
    assertTerminalReceivedData(terminalId, expectedData) {
        ...
        if (typeof expectedData === 'string') {
            if (!output.includes(expectedData)) {
                throw new Error(`Expected terminal ${terminalId} to receive "${expectedData}", ` + `but got: "${output}"`);
            }
        }
        else {
            if (!expectedData.test(output)) {
                throw new Error(`Expected terminal ${terminalId} output to match ${expectedData}, ` +
                    `but got: "${output}"`);
            }
        }
    }

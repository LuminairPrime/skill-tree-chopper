...
test_1.test.describe('Theme Variations @visual-regression', () => {
   ...
   */
  (0, test_1.test)(
    'should render full layout correctly in high contrast theme @P2',
    async ({ page }) => {
   ...
   */
  (0, test_1.test)(
    'should render terminal container correctly in dark theme @P1',
    async ({ page }) => {
      await vrtHelper.setTheme('dark');
   ...
   */
  (0, test_1.test)(
    'should render terminal container correctly in light theme @P1',
    async ({ page }) => {
      await vrtHelper.setTheme('light');
      await vrtHelper.setTerminalBorderState(true, 1);
      await vrtHelper.injectTerminalContent(
        'user@host:~$ echo "Light theme test"\nLight theme test'
      );
      const terminalBody = page.locator('#terminal-body');
...
test_1.test.describe('Theme Consistency @visual-regression', () => {
   ...
   */
  (0, test_1.test)(
    'should render status indicators visibly in dark theme @P1',
    async ({ page }) => {
      await vrtHelper.setTheme('dark');
      const content = [
  ...
  (0, test_1.test)(
    'should render status indicators visibly in light theme @P1',
    async ({ page }) => {
      await vrtHelper.setTheme('light');
      const content = [
        `${helpers_1.ANSI.BOLD}${helpers_1.ANSI.GREEN}✓${helpers_1.ANSI.RESET} Success`,
      ...
      await vrtHelper.injectTerminalContent(content);
      const terminalOutput = page.locator('.terminal-output');
      await (0, test_1.expect)(terminalOutput).toHaveScreenshot(
        'theme-light-status-indicators.png'
      );
    }
  );
  (0, test_1.test)(
    'should render status indicators visibly in high contrast theme @P2',
    async ({ page }) => {
      await vrtHelper.setTheme('high-contrast');
      const content = [
        `${helpers_1.ANSI.BOLD}${helpers_1.ANSI.GREEN}✓${helpers_1.ANSI.RESET} Success`,
      ...
      const terminalOutput = page.locator('.terminal-output');
      await (0, test_1.expect)(terminalOutput).toHaveScreenshot(
        'theme-high-contrast-status-indicators.png'
      );
    }
  );
  /**
...
test_1.test.describe('Focus and Border States @visual-regression', () => {
   ...
   */
  (0, test_1.test)(
    'should render focus border correctly in high contrast theme @P2',
    async ({ page }) => {
      await vrtHelper.setTheme('high-contrast');
      await vrtHelper.setTerminalBorderState(true, 1);
      const container = page.locator('.terminal-container[data-terminal-id="1"]');

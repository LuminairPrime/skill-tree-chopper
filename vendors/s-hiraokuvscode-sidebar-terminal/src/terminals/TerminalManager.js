...
class TerminalManager {
  ...
  constructor(cliAgentService) {
    ...
    this._validationService = new TerminalValidationService_1.TerminalValidationService({
      maxTerminals: config.maxTerminals,
    });
    this._bufferManager = new CircularBufferManager_1.CircularBufferManager(
      (terminalId, data) => this._dataEmitter.fire({ terminalId, data }),
      {
    ...
    this._dataBufferManager = new TerminalDataBufferManager_1.TerminalDataBufferManager(
      this._terminals,
      this._dataEmitter,
      this._cliAgentService
    );
    this._stateCoordinator = new TerminalStateCoordinator_1.TerminalStateCoordinator(
      this._terminals,
    ...
    this._ioCoordinator = new TerminalIOCoordinator_1.TerminalIOCoordinator(
      this._terminals,
      this._activeTerminalManager,
      this._cliAgentService
    );
    this._processCoordinator = new TerminalProcessCoordinator_1.TerminalProcessCoordinator(
      this._terminals,
      this._shellIntegrationService,
      this._stateUpdateEmitter,
      (terminalId, data) => this._dataBufferManager.bufferData(terminalId, data)
    );
    this._lifecycleManager = new TerminalLifecycleManager_1.TerminalLifecycleManager(
      ...
      this._exitEmitter,
      (terminal) => this._setupTerminalEvents(terminal),
      () => this._stateCoordinator.notifyStateUpdate(),
      (terminalId) => this._performCleanup(terminalId)
    );
  }
  ...
  getConnectedAgentType() {
    ...
    const supportedAgentTypes = ['claude', 'gemini', 'codex', 'copilot', 'opencode'];

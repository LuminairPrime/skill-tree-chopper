    ...
    spawnTerminal(request) {
        ...
        for (const cwd of candidateCwds) {
            ...
            for (const shell of attemptedShells) {
                try {
                    (0, logger_1.terminal)(`🔧 [SPAWNER] Attempting PTY spawn for ${request.terminalId} (shell=${shell}, cwd=${cwd})`);
                    ...
                    (0, logger_1.terminal)(`✅ [SPAWNER] PTY spawned successfully for ${request.terminalId} (pid=${ptyProcess.pid}, shell=${shell}, cwd=${cwd})`);
                    return { ptyProcess };
                }
                catch (error) {
                    lastError = error;
                    const err = error;
                    (0, logger_1.terminal)(`❌ [SPAWNER] PTY spawn failed for shell=${shell} cwd=${cwd}: ${err?.message || error}`);

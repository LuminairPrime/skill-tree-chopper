...
class TerminalLifecycleManager {
    constructor(_terminals, _terminalNumberManager, _profileService, _terminalSpawner, _cliAgentService, _terminalCreatedEmitter, _terminalRemovedEmitter, _exitEmitter, _setupEventsCallback, _notifyStateUpdateCallback, _cleanupTerminalDataCallback) {
    ...
    async createTerminalWithProfile(profileName, overrides) {
        if (!this._terminalNumberManager.canCreate(this._terminals)) {
            (0, common_1.showWarningMessage)('Maximum number of terminals reached. Please close some terminals before creating new ones.');
            return '';
        }
    ...
    createTerminal(overrides) {
        ...
        if (!canCreateResult) {
            ...
            if (this._terminals.size !== 0) {
                (0, common_1.showWarningMessage)(`${constants_1.ERROR_MESSAGES.MAX_TERMINALS_REACHED} (${config.maxTerminals})`);
                return '';
            }
        }
        const terminalNumber = this._terminalNumberManager.findAvailableNumber(this._terminals);
        ...
        try {
            ...
            (0, common_1.showErrorMessage)(constants_1.ERROR_MESSAGES.TERMINAL_CREATION_FAILED, error instanceof Error ? error.message : String(error));
            throw error;
        }
    }

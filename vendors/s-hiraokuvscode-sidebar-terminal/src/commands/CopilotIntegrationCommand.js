...
class CopilotIntegrationCommand {
    ...
    async handleActivateCopilot() {
        try {
            ...
            if (!this.isGitHubCopilotIntegrationEnabled()) {
                (0, logger_1.extension)('🔧 [DEBUG] GitHub Copilot integration is disabled by user setting');
                await vscode.window.showInformationMessage('GitHub Copilot integration is disabled. Enable it in Terminal Settings.');
                return;
            }
            // アクティブエディタの確認
            const fileInfo = this.getActiveFileInfo();
            if (!fileInfo) {
                (0, logger_1.extension)('⚠️ [DEBUG] No active editor found, activating Copilot without file reference');
            ...
            (0, logger_1.extension)('✅ [DEBUG] Successfully activated GitHub Copilot Chat with file reference');
        }
        catch (error) {
            (0, logger_1.extension)('❌ [ERROR] Error in handleActivateCopilot:', error);
            await vscode.window.showErrorMessage(`Failed to activate GitHub Copilot Chat: ${String(error)}`);
        }
    }
    /**
    ...
    getActiveFileInfo() {
        ...
        if (!selection.isEmpty) {
            ...
            (0, logger_1.extension)(`🔍 [DEBUG] Selection detected for Copilot: L${startLine}-L${endLine}`);
        }

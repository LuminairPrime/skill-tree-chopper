...
class FileReferenceCommand {
  ...
  handleSendAtMention() {
    try {
      ...
      connectedAgents.forEach((agent) => {
        if (!this.terminalManager.getTerminal(agent.terminalId)) {
          (0, logger_1.extension)(
            `⚠️ [WARN] Stale terminalId ${agent.terminalId} for ${agent.agentType}, skipping`
          );
          return;
        }
      ...
      (0, logger_1.extension)(
        `✅ [DEBUG] File reference sent to ${connectedAgents.length} CLI agents`
      );
    } catch (error) {
    ...
    try {
      ...
      if (this.terminalManager.isTerminalFocused()) {
        ...
        if (activeId) {
          this.terminalManager.sendInput(text, activeId);
          (0, logger_1.extension)(
            `📤 [DEBUG] Sent ${openFiles.length} file references to focused terminal`
          );
          void vscode.window.setStatusBarMessage(
      ...
      connectedAgents.forEach((agent) => {
        ...
        this.terminalManager.sendInput(text, agent.terminalId);
        (0, logger_1.extension)(
          `📤 [DEBUG] Sent ${openFiles.length} file references to ${agent.agentType}`
        );
        sentAgents.push(agent);
      });
      if (sentAgents.length === 0) {
  ...
  getConnectedAgents() {
    ...
    if (connectedAgents.length === 0) {
      (0, logger_1.extension)(
        '⚠️ [FALLBACK] No connected agents found, attempting state refresh...'
      );

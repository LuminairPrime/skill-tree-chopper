...
function registerPhase2Services(container, eventBus) {
  ...
  container.register(
    IBufferManagementService_1.IBufferManagementService,
    () => new BufferManagementService_1.BufferManagementService(eventBus),
    DIContainer_1.ServiceLifetime.Singleton
  );
  // Register TerminalStateService (Phase 2 Week 2)
  container.register(
    ITerminalStateService_1.ITerminalStateService,
    () => new TerminalStateService_1.TerminalStateService(eventBus),
    DIContainer_1.ServiceLifetime.Singleton
  );
...
async function registerPhase3Plugins(container, eventBus) {
  try {
    ...
    const configService = new PluginConfigurationService_1.PluginConfigurationService(
      pluginManager
    );

...
class ScrollbackCoordinator {
    ...
    async requestScrollbackData(terminalId, maxLines = 1000) {
        ...
        (0, logger_1.provider)(`📋 [SCROLLBACK-COORDINATOR] Requesting scrollback data for terminal ${terminalId} (requestId: ${requestId})`);
        return new Promise((resolve, reject) => {
            // Set timeout for request
            const timeout = setTimeout(() => {
                (0, logger_1.provider)(`⏰ [SCROLLBACK-COORDINATOR] Timeout for terminal ${terminalId} scrollback request`);
    ...
    async requestMultipleScrollbackData(terminalIds, maxLines = 1000) {
        (0, logger_1.provider)(`📋 [SCROLLBACK-COORDINATOR] Requesting scrollback data for ${terminalIds.length} terminals`);
        ...
        await Promise.all(terminalIds.map(async (terminalId) => {
            try {
                ...
                if (scrollbackData && scrollbackData.length > 0) {
                    ...
                    (0, logger_1.provider)(`✅ [SCROLLBACK-COORDINATOR] Collected ${scrollbackData.length} lines for terminal ${terminalId}`);
                }
            }
            catch (error) {
                (0, logger_1.provider)(`❌ [SCROLLBACK-COORDINATOR] Failed to collect scrollback for terminal ${terminalId}:`, error);
            }
        }));
        (0, logger_1.provider)(`📋 [SCROLLBACK-COORDINATOR] Collected scrollback data for ${Object.keys(scrollbackDataMap).length}/${terminalIds.length} terminals`);
    ...
    handleScrollbackDataResponse(message) {
        ...
        if (!pendingRequest) {
            (0, logger_1.provider)(`⚠️ [SCROLLBACK-COORDINATOR] No pending request found for requestId: ${requestId}`);
            return;
        }
        // Clear timeout
        ...
        this._pendingScrollbackRequests.delete(requestId);
        if (message.error) {
            (0, logger_1.provider)(`⚠️ [SCROLLBACK-COORDINATOR] Scrollback extraction error for terminal ${message.terminalId}: ${message.error}`);
            pendingRequest.resolve([]); // Resolve with empty array on error
        }
        else {
            (0, logger_1.provider)(`✅ [SCROLLBACK-COORDINATOR] Scrollback data received for terminal ${message.terminalId}: ${message.scrollbackData?.length || 0} lines`);

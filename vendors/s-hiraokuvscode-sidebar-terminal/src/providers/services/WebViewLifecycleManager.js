...
class WebViewLifecycleManager {
  ...
  setWebviewHtml(webviewView, htmlContent, isPanelMove = false) {
    try {
      ...
      (0, logger_1.provider)(
        '🎆 [LIFECYCLE] WebView HTML length after setting:',
        webviewView.webview.html.length
      );
    } catch (error) {
  ...
  logPerformanceMetrics() {
    ...
    (0, logger_1.provider)(
      `📊 [METRICS] Listener target met: ${meetsListenerTarget ? '✅' : '❌'}`
    );
    (0, logger_1.provider)(
      `📊 [METRICS] Panel movement target met: ${meetsPanelMovementTarget ? '✅' : '❌'}`
    );
    (0, logger_1.provider)(
      `📊 [METRICS] Initialization target met: ${meetsInitializationTarget ? '✅' : '❌'}`
    );
  }
  /**

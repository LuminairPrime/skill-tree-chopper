...
class WebViewCommunicationService {
  ...
  async sendMessageToWebview(message) {
    (0, logger_1.provider)(
      `📤 [COMMUNICATION] Public sendMessageToWebview called: ${message.command}`
    );
  ...
  async sendVersionInfo() {
    try {
      ...
      if (this._view) {
        ...
        (0, logger_1.provider)(
          `📤 [COMMUNICATION] Sent version info to WebView: ${formattedVersion}`
        );
      }
    } catch (error) {
  ...
  async sendInitializationComplete(terminalCount) {
    try {
      ...
      (0, logger_1.provider)(
        `📤 [COMMUNICATION] Sent initialization complete (${terminalCount} terminals)`
      );
    } catch (error) {
  ...
  async requestPanelLocationDetection() {
    try {
      ...
      (0, logger_1.provider)(
        '❌ [COMMUNICATION] Failed to request panel location detection:',
        error
      );
    }
  }

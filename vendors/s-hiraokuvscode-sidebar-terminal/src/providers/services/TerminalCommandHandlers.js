...
class TerminalCommandHandlers {
    ...
    async handleFocusTerminal(message) {
        ...
        try {
            ...
            if (newActive === message.terminalId) {
                (0, logger_1.provider)(`✅ [HANDLER] Active terminal successfully updated to: ${message.terminalId}`);
            }
            else {
                (0, logger_1.provider)(`❌ [HANDLER] Active terminal update failed. Expected: ${message.terminalId}, Got: ${newActive}`);
            }
        }
        catch (error) {
    ...
    async handleRequestInitialTerminal(_message) {
        ...
        try {
            ...
            if (currentTerminals.length === 0) {
                ...
                (0, logger_1.provider)(`📊 [HANDLER] Terminals already exist (${currentTerminals.length}), skipping creation`);
            }
            await this.sendStateUpdate();
        }
        catch (error) {
    ...
    handleTerminalResize(message) {
        if (!(0, type_guards_1.hasTerminalId)(message) || !(0, type_guards_1.hasResizeParams)(message)) {
    ...
    async handleRenameTerminal(message) {
        ...
        try {
            ...
            if (!renamed) {
                (0, logger_1.provider)(`⚠️ [HANDLER] renameTerminal failed for terminalId=${message.terminalId}`);
                return;
            }
            await this.sendStateUpdate();
            (0, logger_1.provider)(`✅ [HANDLER] Terminal renamed: ${message.terminalId} -> ${nextName.trim()}`);
        }
        catch (error) {
    ...
    async handleUpdateTerminalHeader(message) {
        ...
        try {
            ...
            if (!updated) {
                (0, logger_1.provider)(`⚠️ [HANDLER] updateTerminalHeader failed for terminalId=${message.terminalId}`);
                return;
            }
            await this.sendStateUpdate();
            (0, logger_1.provider)(`✅ [HANDLER] Terminal header updated: ${message.terminalId} (name=${updates.newName ?? 'unchanged'}, color=${updates.indicatorColor ?? 'unchanged'})`);
        }
        catch (error) {
    ...
    async handleOpenTerminalLink(message) {
        (0, logger_1.provider)(`🔗 [HANDLER] openTerminalLink received: type=${message.linkType} url=${message.url ?? ''} file=${message.filePath ?? ''} terminal=${message.terminalId ?? ''}`);
    ...
    async handleClipboardRequest(message) {
        ...
        try {
            ...
            (0, logger_1.provider)(`📋 [HANDLER] Clipboard content length: ${clipboardText.length} characters`);
    ...
    async handleCopyToClipboard(message) {
        ...
        try {
            ...
            (0, logger_1.provider)(`📋 [HANDLER] Copied ${text.length} chars from terminal ${message.terminalId ?? 'unknown'}`);
        }
        catch (error) {
    ...
    async handlePasteImage(message) {
        ...
        try {
            (0, logger_1.provider)(`🖼️ [HANDLER] Processing image paste for terminal ${message.terminalId}`);
    ...
    async handleSwitchAiAgent(message) {
        ...
        (0, logger_1.provider)(`📎 [HANDLER] switchAiAgent received: terminalId=${terminalId}, action=${action}`);
        if (!terminalId) {
        ...
        try {
            ...
            (0, logger_1.provider)(`⏻ [HANDLER] switchAiAgent result: success=${result.success}, newStatus=${result.newStatus}, agentType=${result.agentType}`);
            ...
            if (result.success) {
                (0, logger_1.provider)(`✅ [HANDLER] AI Agent switch succeeded: ${terminalId} -> ${result.newStatus}`);
            }
            else {
                (0, logger_1.provider)(`⚠️ [HANDLER] AI Agent switch failed: ${terminalId}, reason: ${result.reason}`);
            }
        }
        catch (error) {
    ...
    async handlePasteText(message) {
        ...
        try {
            (0, logger_1.provider)(`📋 [HANDLER] Processing text paste for terminal ${message.terminalId}`);
    ...
    async initializeTerminals() {
        ...
        for (const terminal of terminals) {
            const displayModeOverride = 'consumeCreationDisplayModeOverride' in this.deps.terminalManager &&
    ...
    getTerminalHeaderUpdates(message) {
        ...
        const newName = typeof rawName === 'string' && rawName.trim().length > 0 ? rawName.trim() : undefined;

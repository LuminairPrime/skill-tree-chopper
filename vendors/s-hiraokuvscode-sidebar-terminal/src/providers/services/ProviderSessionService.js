...
class ProviderSessionService {
    ...
    async saveCurrentSession() {
        ...
        try {
            ...
            if (response.success) {
                (0, logger_1.provider)(`✅ [PERSISTENCE] Session saved successfully: ${terminals.length} terminals`);
                return true;
            }
            else {
    ...
    async restoreLastSession() {
        ...
        try {
            ...
            (0, logger_1.provider)(`📦 [PERSISTENCE] Found ${sessionResult.terminals.length} terminals to restore`);
            ...
            for (const terminalData of sessionResult.terminals) {
                try {
                    ...
                    (0, logger_1.provider)(`✅ [PERSISTENCE] Restored terminal: ${terminalData.name} (${newTerminalId})`);
                }
                catch (error) {
                    (0, logger_1.provider)(`❌ [PERSISTENCE] Failed to restore terminal ${terminalData.name}:`, error);
                }
            }
            if (restoredTerminals.length > 0) {
                ...
                await new Promise((resolve) => setTimeout(resolve, TimingConstants_1.TIMING_CONSTANTS.WEBVIEW_INIT_DELAY_MS));
                // Restore scrollback
                for (const mapping of terminalMappings) {
                    if (mapping.terminalData.scrollback &&

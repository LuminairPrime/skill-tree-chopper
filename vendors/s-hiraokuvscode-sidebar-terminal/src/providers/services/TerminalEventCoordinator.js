...
class TerminalEventCoordinator {
    constructor(_terminalManager, _sendMessage, _sendFullCliAgentStateSync, _terminalIdMapping, _initializationState) {
    ...
    setupTerminalEventListeners() {
        ...
        const dataDisposable = this._terminalManager.onData((event) => {
            if (!event.data) {
                (0, logger_1.provider)('⚠️ [EVENT-COORDINATOR] Empty data received from terminal:', event.terminalId);
                return;
            }
            const webviewTerminalId = this._resolveWebviewTerminalId(event.terminalId);
            const outputAllowed = !this._initializationState || this._initializationState.isOutputAllowed(event.terminalId);
            (0, logger_1.provider)('🔍 [EVENT-COORDINATOR] Terminal output received:', event.data.length, 'chars, Extension ID:', event.terminalId, '→ WebView ID:', webviewTerminalId, 'ready:', outputAllowed, 'preview:', JSON.stringify(event.data.substring(0, 50)));
            if (!outputAllowed) {
        ...
        const createdDisposable = this._terminalManager.onTerminalCreated((terminal) => {
            (0, logger_1.provider)('🆕 [EVENT-COORDINATOR] Terminal created:', terminal.id, terminal.name);
            const displayModeOverride = 'consumeCreationDisplayModeOverride' in this._terminalManager &&
    ...
    flushBufferedOutput(extensionTerminalId) {
        ...
        (0, logger_1.provider)(`📤 [EVENT-COORDINATOR] Flushing buffered output for ${webviewTerminalId}: ${combined.length} chars (${buffer.length} chunks)`);
    ...
    _bufferOutput(webviewTerminalId, chunk) {
        ...
        (0, logger_1.provider)(`⏸️ [EVENT-COORDINATOR] Buffering output for ${webviewTerminalId} (chunks=${existing.length}, length=${chunk.length})`);
    }
    ...
    setupCliAgentStatusListeners() {
        ...
        const claudeStatusDisposable = this._terminalManager.onCliAgentStatusChange((event) => {
            try {
                ...
                (0, logger_1.provider)('❌ [EVENT-COORDINATOR] CLI Agent status change processing failed:', error);
            }
        });
    ...
    setupConfigurationChangeListeners() {
        const configChangeDisposable = vscode.workspace.onDidChangeConfiguration((event) => {
            ...
            if (event.affectsConfiguration('terminal.integrated.fontSize') ||
            ...
            if (this._pendingSettingsUpdate || this._pendingFontSettingsUpdate) {
                ...
                this._configChangeDebounceTimer = setTimeout(() => {
                    ...
                    this._pendingSettingsUpdate = false;
                    this._pendingFontSettingsUpdate = false;
                    (0, logger_1.provider)('⚙️ [EVENT-COORDINATOR] Configuration changed (settings:', shouldUpdateSettings, ', fonts:', shouldUpdateFontSettings, ')');
                    ...
                    if (shouldUpdateSettings) {
                        ...
                        if (settingsKey !== this._lastSentSettingsKey) {
                            ...
                            this._sendMessage({
                                ...
                                settings,
                            }).catch((err) => (0, logger_1.provider)('❌ [EVENT-COORDINATOR] Failed to send settings update:', err));
                    ...
                    if (shouldUpdateFontSettings) {
                        ...
                        if (fontSettingsKey !== this._lastFontSettingsKey) {
                            ...
                            this._sendMessage({
                                ...
                                fontSettings,
                            }).catch((err) => (0, logger_1.provider)('❌ [EVENT-COORDINATOR] Failed to send font settings update:', err));
                            (0, logger_1.provider)('⚙️ [EVENT-COORDINATOR] Sent font settings update to WebView');
                        }
                        else {
                            (0, logger_1.provider)(`⏭️ [EVENT-COORDINATOR] Skipping duplicate font settings update`);
                        }
                    }

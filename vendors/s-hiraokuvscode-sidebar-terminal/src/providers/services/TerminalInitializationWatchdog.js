...
class TerminalInitializationWatchdog {
    ...
    start(terminalId, reason, overrideOptions) {
        ...
        (0, logger_1.provider)(`⏳ [WATCHDOG] Started for ${terminalId} (reason=${reason}, initialDelay=${mergedOptions.initialDelayMs}ms, maxAttempts=${mergedOptions.maxAttempts})`);
    ...
    scheduleNext(terminalId, entry) {
        ...
        entry.timer = setTimeout(() => {
            ...
            (0, logger_1.provider)(`⚠️ [WATCHDOG] Attempt #${nextAttempt} for ${terminalId} (delay=${delay}ms, final=${isFinalAttempt})`);

...
class WatchdogCoordinator {
  constructor(deps, ackOptions, promptOptions) {
    ...
    this._watchdog = new TerminalInitializationWatchdog_1.TerminalInitializationWatchdog(
  ...
  startForTerminal(terminalId, phase, source, overrideOptions) {
    ...
    (0, logger_1.provider)(
      `⏳ [WATCHDOG] Started for ${terminalId} (phase=${phase}, source=${source})`
    );
  }
  /**
  ...
  handleTimeout(terminalId, attempt, isFinalAttempt) {
    ...
    if (!this._safeModeTerminals.has(terminalId)) {
      ...
      try {
        ...
        (0, logger_1.provider)(
          `❌ [FALLBACK] Safe mode initialization failed for ${terminalId}:`,
          error
        );
      }
    }
  ...
  recordMetric(metric, terminalId) {
    ...
    this._safeModeNotified.delete(terminalId);
    (0, logger_1.provider)(
      `📊 [METRIC] terminal.init.${metric} (terminal=${terminalId}, duration=${duration}ms)`
    );

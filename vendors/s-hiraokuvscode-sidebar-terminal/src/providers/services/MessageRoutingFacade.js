...
class MessageRoutingFacade {
    ...
    registerHandler(command, handler, category, description) {
        ...
        (0, logger_1.provider)(`✅ [ROUTING] Registered handler for '${command}'${category ? ` (${category})` : ''}`);
    }
    /**
    ...
    registerHandlers(handlers) {
        for (const registration of handlers) {
            this.registerHandler(registration.command, registration.handler, registration.category, registration.description);
        }
    }
    /**
    ...
    validateHandlers(requiredCommands) {
        ...
        if (missing.length > 0) {
            (0, logger_1.provider)(`❌ [ROUTING] Missing handlers for critical commands: ${missing.join(', ')}`);
            (0, logger_1.provider)('📋 [ROUTING] Currently registered commands:', this._router.getRegisteredCommands());
        }
        else {

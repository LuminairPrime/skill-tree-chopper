...
class PanelLocationService {
  ...
  async initialize(_webviewView) {
    ...
    (0, logger_1.provider)(
      '📍 [PANEL-DETECTION] Panel location service initialized (detection deferred to WebView)'
    );
  }
  /**
  ...
  async handlePanelLocationReport(location, onLocationChange) {
    ...
    if (previousLocation !== effectiveLocation) {
      ...
      if (shouldUpdateContext) {
        ...
        (0, logger_1.provider)(
          '📍 [DEBUG] Context key updated with NEW panel location:',
          effectiveLocation
        );
      } else {
      ...
      if (onLocationChange) {
        (0, logger_1.provider)(
          `🔄 [RELAYOUT] Location changed: ${previousLocation} → ${effectiveLocation}`
        );
  ...
  async requestPanelLocationDetection() {
    ...
    this._detectionDebounceTimer = setTimeout(async () => {
      try {
        (0, logger_1.provider)(
          '📍 [PANEL-DETECTION] Requesting panel location detection from WebView (debounced)'
        );
        ...
        (0, logger_1.provider)(
          '⚠️ [PANEL-DETECTION] Error requesting panel location detection:',
          error
        );
  ...
  determineSplitDirection() {
    (0, logger_1.provider)(
      '🔀 [SPLIT] ==================== DETERMINE SPLIT DIRECTION ===================='
    );
    ...
    (0, logger_1.provider)(
      `🔀 [SPLIT] Mapping logic: ${panelLocation} === 'panel' ? 'horizontal' : 'vertical'`
    );
  ...
  getCurrentPanelLocation() {
    ...
    (0, logger_1.provider)(
      `📍 [PANEL-DETECTION] Dynamic split direction enabled: ${isDynamicSplitEnabled}`
    );
    if (!isDynamicSplitEnabled) {
      (0, logger_1.provider)(
        '📍 [PANEL-DETECTION] ❌ Dynamic split direction is DISABLED, defaulting to sidebar'
      );
      (0, logger_1.provider)(
        '📍 [PANEL-DETECTION] =========================================================================='
      );
    ...
    (0, logger_1.provider)(
      `📍 [PANEL-DETECTION] Manual panel location setting: ${manualPanelLocation}`
    );
    if (manualPanelLocation !== 'auto') {
      (0, logger_1.provider)(
        `📍 [PANEL-DETECTION] ✅ Using MANUAL panel location: ${manualPanelLocation}`
      );
      (0, logger_1.provider)(
        '📍 [PANEL-DETECTION] =========================================================================='
      );
    ...
    (0, logger_1.provider)(
      `📍 [PANEL-DETECTION] AUTO mode - using cached value: ${this._cachedPanelLocation}`
    );
    (0, logger_1.provider)(
      '📍 [PANEL-DETECTION] =========================================================================='
    );
    ...
    this._disposables.push(
      vscode.workspace.onDidChangeConfiguration((event) => {
        if (event.affectsConfiguration('secondaryTerminal.panelLocation')) {
          (0, logger_1.provider)(
            '📍 [PANEL-DETECTION] Panel location setting changed - requesting detection'
          );

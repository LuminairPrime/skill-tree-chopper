...
class TerminalInitLifecycleHandler {
    ...
    async handleTerminalReady(message) {
        ...
        if (currentState < TerminalInitializationStateMachine_1.TerminalInitializationState.ViewReady) {
            ...
            (0, logger_1.provider)(`🔄 [PROVIDER] terminalReady promoted state to ViewReady for ${terminalId}`);
        }
    ...
    async sendInitializationComplete(terminalCount) {
        (0, logger_1.provider)(`📤 [PROVIDER] Sending initialization complete: ${terminalCount} terminals`);
    ...
    async handleTerminalInitializationComplete(message) {
        ...
        if (phase === 'prompt' &&
            ...
            (0, logger_1.provider)(`⏭️ [PROVIDER] Ignoring duplicate terminalInitializationComplete for ${terminalId}`);
            return;
        }
        (0, logger_1.provider)(`✅ [PROVIDER] Terminal ${terminalId} initialization confirmed by WebView`);
        ...
        if (!terminal || !terminal.ptyProcess) {
            ...
            if (attempts > 5) {
                (0, logger_1.provider)(`❌ [PROVIDER] Terminal ${terminalId} still unavailable after ${attempts} retries`);
                this._pendingInitRetries.delete(terminalId);
                return;
            }
            (0, logger_1.provider)(`⏳ [PROVIDER] Terminal ${terminalId} not ready (attempt=${attempts}). Retrying terminalInitializationComplete handler...`);
    ...
    ensureMultipleTerminals() {
        ...
        try {
            ...
            if (currentTerminals < 1) {
                ...
                (0, logger_1.provider)(`✅ [ENSURE] Sufficient terminals already exist: ${currentTerminals}`);
            }
        }
        catch (error) {

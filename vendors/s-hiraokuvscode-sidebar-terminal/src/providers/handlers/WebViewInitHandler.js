...
class WebViewInitHandler {
    ...
    resolveInitialTheme(settingsTheme) {
        ...
        const hasThemeKind = typeof vscode.ColorThemeKind !== 'undefined' && typeof activeThemeKind === 'number';
        if (!hasThemeKind) {
            return normalizedTheme;
        }
        if (activeThemeKind === vscode.ColorThemeKind.Light ||
            activeThemeKind === vscode.ColorThemeKind.HighContrastLight) {
            return 'light';
        }
        if (activeThemeKind === vscode.ColorThemeKind.Dark ||
    ...
    handleWebviewReady(_message) {
        ...
        if (this._isInitialized) {
            (0, logger_1.provider)('🔄 [TERMINAL-INIT] WebView already initialized, skipping duplicate initialization');
            return;
        }
        (0, logger_1.provider)('🎯 [TERMINAL-INIT] WebView ready - sending extensionReady confirmation');
        // Send extensionReady
        ...
        (0, logger_1.provider)('⏳ [HANDSHAKE] Waiting for webviewInitialized before starting terminal initialization');
    }
    /**
    ...
    async handleWebviewInitialized(_message) {
        ...
        (0, logger_1.provider)('🎯 [TERMINAL-INIT] WebView fully initialized - starting terminal initialization');
        (0, logger_1.provider)(`🔍 [TERMINAL-INIT] _pendingPanelMoveReinit: ${this._pendingPanelMoveReinit}`);
        ...
        (0, logger_1.provider)(`📤 [TERMINAL-INIT] Sending settings to WebView FIRST (theme: ${settings.theme})`);
    ...
    async reinitializeWebviewAfterPanelMove() {
        try {
            ...
            (0, logger_1.provider)('❌ [PANEL-MOVE] Failed to reinitialize WebView after panel move:', error);
            try {
                await this.deps.initializeTerminal();
            }
            catch {
    ...
    async initializeWithFontSettings() {
        try {
            ...
            const fontSettings = this.deps.getCurrentFontSettings();
            (0, logger_1.provider)('📤 [TERMINAL-INIT] Step 2: Sending font settings BEFORE terminal creation');
            await this.deps.sendMessage({
                command: 'fontSettingsUpdate',
                fontSettings,
            });
            ...
            (0, logger_1.provider)('📤 [TERMINAL-INIT] Step 3: Starting terminal initialization with font settings ready');
    ...
    async sendMessage(message) {
        if (!this._isInitialized && message.command !== 'extensionReady') {
            ...
            (0, logger_1.provider)(`⏳ [PROVIDER] Queuing message until webviewReady: ${message.command}`);
            return;
        }

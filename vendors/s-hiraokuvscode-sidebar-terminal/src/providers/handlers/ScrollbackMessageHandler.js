...
class ScrollbackMessageHandler {
    ...
    async handlePushScrollbackData(message) {
        ...
        if (!persistenceService) {
            (0, logger_1.provider)('⚠️ [PROVIDER] Received pushScrollbackData but persistence service is unavailable');
            return;
        }
        const handler = persistenceService.handlePushedScrollbackData;
        if (typeof handler !== 'function') {
            (0, logger_1.provider)('⚠️ [PROVIDER] Persistence service does not support pushScrollbackData');
            return;
        }
        try {
    ...
    async handleScrollbackDataCollected(message) {
        ...
        if (persistenceService) {
            ...
            if (typeof handler === 'function') {
                ...
                (0, logger_1.provider)(`✅ [PROVIDER] scrollbackDataCollected forwarded to persistence service (requestId=${requestId || 'none'})`);
                return;
            }
        }
    ...
    async handleScrollbackRefreshRequest(message) {
        const persistenceService = this.deps.getExtensionPersistenceService();
        if (!persistenceService) {
            (0, logger_1.provider)('⚠️ [PROVIDER] Received requestScrollbackRefresh but persistence service is unavailable');
            return;
        }
        const handler = persistenceService.handleScrollbackRefreshRequest;
        if (typeof handler !== 'function') {
            (0, logger_1.provider)('⚠️ [PROVIDER] Persistence service does not support handleScrollbackRefreshRequest');
            return;
        }
        try {

...
const defaultServiceFactory = (context, terminalManager) => new ExtensionPersistenceService_1.ExtensionPersistenceService(context, terminalManager);
const defaultHandlerFactory = (service) => new PersistenceMessageHandler_1.PersistenceMessageHandler(service);
class PersistenceOrchestrator {
    constructor(options) {
        this.options = options;
        this.logger = options.logger ?? logger_1.provider;
        this.persistenceService = (options.serviceFactory || defaultServiceFactory)(options.extensionContext, options.terminalManager);
    ...
    async handlePersistenceMessage(message, sendMessage) {
        await this.routePersistenceMessage(message, sendMessage ?? this.sendMessageImpl, (webviewMessage) => webviewMessage);
    }
    ...
    async restoreLastSession() {
        ...
        try {
            ...
            if (result.success) {
                ...
                this.logger(`✅ [PERSISTENCE] Session restored successfully: ${restoredCount}/${restoredCount + skippedCount} terminals`);
            }
            else {

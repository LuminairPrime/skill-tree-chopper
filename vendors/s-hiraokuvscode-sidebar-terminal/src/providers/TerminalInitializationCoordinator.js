...
class TerminalInitializationCoordinator {
  ...
  async restoreTerminalsIfNeeded(currentTerminalCount) {
    ...
    (0, logger_1.terminal)(
      '📦 [TERMINAL-INIT] No existing terminals - attempting coordinated restoration...'
    );
    ...
    if (!restorationSucceeded) {
      ...
      (0, logger_1.terminal)(
        '🎯 [TERMINAL-INIT] Restoration succeeded - notifying WebView about restored terminals'
      );
      await this.actions.initializeTerminal();
    }
  }
    ...
    if (this.extensionPersistenceService) {
      try {
        ...
        if (result.success && result.restoredCount && result.restoredCount > 0) {
          (0, logger_1.terminal)(
            `✅ [EXT-RESTORE] Successfully restored ${result.restoredCount} terminals`
          );
  ...
  logExistingTerminals(currentTerminalCount) {
    ...
    existingTerminals.forEach((terminal, index) => {
      (0, logger_1.terminal)(
        `📋 [TERMINAL-INIT] Existing terminal ${index + 1}: ${terminal.name} (${terminal.id})`
      );
    });
  }

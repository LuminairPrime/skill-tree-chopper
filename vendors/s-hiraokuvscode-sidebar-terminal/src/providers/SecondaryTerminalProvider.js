...
class SecondaryTerminalProvider {
  constructor(_extensionContext, _terminalManager, _extensionPersistenceService, telemetryService) {
    ...
    this._terminalInitStateMachine =
      new TerminalInitializationStateMachine_1.TerminalInitializationStateMachine();
    ...
    this._panelLocationService = new PanelLocationService_1.PanelLocationService((message) =>
      this._communicationService.sendMessage(message)
    );
    this._linkResolver = new TerminalLinkResolver_1.TerminalLinkResolver((terminalId) =>
      this._terminalManager.getTerminal(terminalId)
    );
    this._scrollbackCoordinator = new ScrollbackCoordinator_1.ScrollbackCoordinator(
    ...
    if (this._extensionPersistenceService) {
      this._persistenceHandler = new PersistenceMessageHandler_1.PersistenceMessageHandler(
        this._extensionPersistenceService
      );
      (0, logger_1.provider)('💾 [PROVIDER] Terminal persistence services initialized');
    } else {
      (0, logger_1.provider)(
        '⚠️ [PROVIDER] ExtensionPersistenceService not provided, persistence disabled'
      );
    }
    // Initialize terminal initialization coordinator
    this._initializationCoordinator =
      new TerminalInitializationCoordinator_1.TerminalInitializationCoordinator(
    ...
    this._settingsService = new SettingsSyncService_1.SettingsSyncService(
      async () => await this._initializeTerminal()
    );
    this._cleanupService = new ResourceCleanupService_1.ResourceCleanupService();
    this._lifecycleManager = new WebViewLifecycleManager_1.WebViewLifecycleManager(
      this._extensionContext,
      this._htmlGenerationService
    );
    this._messageRouter = new MessageRoutingFacade_1.MessageRoutingFacade();
    this._orchestrator = new InitializationOrchestrator_1.InitializationOrchestrator(
      this._initializationCoordinator,
    ...
    this._watchdogCoordinator = new WatchdogCoordinator_1.WatchdogCoordinator(
      {
        ...
        initializeShellForTerminal: (id, pty, safe) =>
          this._terminalManager.initializeShellForTerminal(id, pty, safe),
    ...
    this._terminalInitLifecycleHandler =
      new TerminalInitLifecycleHandler_1.TerminalInitLifecycleHandler({
        getTerminal: (id) => this._terminalManager.getTerminal(id),
        getTerminals: () => this._terminalManager.getTerminals(),
        ...
        // Set later when event coordinator is initialized
        safeProcessCwd: common_1.safeProcessCwd,
      });
  ...
  _registerThemeChangeListener() {
    const disposable = vscode.window.onDidChangeActiveColorTheme((colorTheme) => {
      ...
      if (themeMode !== 'auto') {
        (0, logger_1.provider)(
          `🎨 [THEME] Theme change detected but mode is '${themeMode}', ignoring`
        );
        return;
      }
      const isDark =
  ...
  resolveWebviewView(webviewView, _context, _token) {
    ...
    if (this._lifecycleManager.isBodyRendered()) {
      (0, logger_1.provider)(
        '⏭️ [PROVIDER] Body already rendered - checking if WebView needs reinitialization'
      );
    ...
    try {
      ...
      (0, logger_1.provider)(
        '✅ [PROVIDER] Body rendering complete, _bodyRendered flag set to true'
      );
  ...
  _resetForNewView(webviewView) {
    ...
    this._eventCoordinator = new TerminalEventCoordinator_1.TerminalEventCoordinator(
      this._terminalManager,
      this._communicationService.sendMessage.bind(this._communicationService),
  ...
  _registerWebviewMessageListener(webviewView) {
    ...
    if (this._lifecycleManager.isMessageListenerRegistered()) {
      (0, logger_1.provider)(
        '⏭️ [PROVIDER] Message listener already registered for current WebView'
      );
      return;
    }
  ...
  _registerVisibilityListener(webviewView) {
    (0, logger_1.provider)(
      '🔧 [PROVIDER] Step 3: Setting up consolidated visibility listener (VS Code pattern)...'
    );
  ...
  _initializeWebviewContent(webviewView) {
    ...
    (0, logger_1.provider)(
      `🎨 [PROVIDER] Initial theme for HTML: ${initialTheme} (settings: ${settingsTheme})`
    );
  ...
  _handleUnroutedMessage(message) {
    (0, logger_1.provider)(
      `⚠️ [PROVIDER] No handler registered for command '${message.command}', invoking fallback`
    );
    switch (message.command) {
      ...
      default:
        (0, logger_1.provider)(
          `⚠️ [PROVIDER] No fallback available for command: ${message.command}`
        );
        break;
    }
  }
  ...
  _setupPanelLocationChangeListener(_webviewView) {
    ...
    const settingsDisposable = vscode.workspace.onDidChangeConfiguration((event) => {
      ...
      if (this._settingsMessageHandler.isSettingsChangeAffectingWebView(event)) {
        (0, logger_1.provider)(
          '⚙️ [PROVIDER] Settings changed, sending updated settings to WebView...'
        );
  ...
  async _handleLegacyPersistenceMessage(message) {
    (0, logger_1.provider)(
      '⚠️ [PERSISTENCE] Legacy persistence message received - converting to new format'
    );
  ...
  dispose() {
    ...
    if (this._extensionPersistenceService) {
      this._extensionPersistenceService
        ...
        .catch((error) =>
          (0, logger_1.provider)(`⚠️ [PERSISTENCE] Cleanup during dispose failed: ${error}`)
        );
    }
  ...
  setPhase8Services(decorationsService, linksService) {
    ...
    (0, logger_1.provider)(
      '🎨 [PROVIDER] Phase 8 services (Decorations & Links) connected to provider'
    );
    const view = this._lifecycleManager.getView();
    if (view) {
      this._sendMessage({
        ...
        (0, logger_1.provider)('❌ [PROVIDER] Failed to send Phase 8 capabilities:', error)
      );
    }
  }
}

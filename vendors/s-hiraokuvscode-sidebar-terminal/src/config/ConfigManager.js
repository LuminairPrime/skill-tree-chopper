...
class ConfigManager {
    ...
    try {
      if (vscode?.workspace?.onDidChangeConfiguration) {
        vscode.workspace.onDidChangeConfiguration((event) => {
          if (
            event.affectsConfiguration(shared_1.CONFIG_SECTIONS.SIDEBAR_TERMINAL) ||
  ...
  getShellForPlatform(customShell) {
    ...
    switch (process.platform) {
      case SystemConstants_1.TERMINAL_CONSTANTS.PLATFORMS.WINDOWS:
        return (
          this.getConfig(section, shared_1.CONFIG_KEYS.SHELL_WINDOWS, '') ||
          process.env['COMSPEC'] ||
          'cmd.exe'
        );
      case SystemConstants_1.TERMINAL_CONSTANTS.PLATFORMS.DARWIN:
        return (
          this.getConfig(section, shared_1.CONFIG_KEYS.SHELL_OSX, '') ||
          process.env['SHELL'] ||
          '/bin/zsh'
        );
      default:
        return (
          this.getConfig(section, shared_1.CONFIG_KEYS.SHELL_LINUX, '') ||
          process.env['SHELL'] ||
          '/bin/bash'
        );
    }
  ...
  getTerminalProfilesConfig() {
    ...
    return {
      ...
      inheritVSCodeProfiles: this.getConfig(
        section,
        shared_1.CONFIG_KEYS.INHERIT_VSCODE_PROFILES,
        true
      ),
    };
  }
  ...
  getDefaultTerminalProfile() {
    ...
    const defaultKey =
      defaultKeyMap[process.platform] || shared_1.CONFIG_KEYS.DEFAULT_PROFILE_LINUX;
  ...
  isProfileAutoDetectionEnabled() {
    ...
    return this.getConfig(
      shared_1.CONFIG_SECTIONS.SIDEBAR_TERMINAL,
      shared_1.CONFIG_KEYS.ENABLE_PROFILE_AUTO_DETECTION,
      true
    );
  }
  getCacheInfo() {

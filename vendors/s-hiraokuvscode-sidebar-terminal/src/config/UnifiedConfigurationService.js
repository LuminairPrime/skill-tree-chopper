...
class UnifiedConfigurationService {
  ...
  initialize() {
    ...
    try {
      ...
      if (typeof vscode !== 'undefined' && vscode?.workspace) {
        ...
        (0, logger_1.terminal)(
          '⚠️ [UnifiedConfig] VS Code API not available, running in limited mode'
        );
      }
    } catch (error) {
  ...
  get(section, key, defaultValue) {
    ...
    try {
      ...
      if (!validation.isValid) {
        (0, logger_1.terminal)(
          `⚠️ [UnifiedConfig] Validation failed for ${fullKey}:`,
          validation.errors
        );
  ...
  getExtensionTerminalConfig() {
    ...
    return {
      maxTerminals: this.get(
        section,
        shared_1.CONFIG_KEYS.MAX_TERMINALS,
        SystemConstants_1.TERMINAL_CONSTANTS.DEFAULT_MAX_TERMINALS
      ),
  ...
  getCompleteTerminalSettings() {
    ...
    return {
      ...
      theme: this.get(
        ...
        shared_1.CONFIG_KEYS.THEME,
        'auto'
      ),
      cursorBlink: this.get(
        shared_1.CONFIG_SECTIONS.SIDEBAR_TERMINAL,
        shared_1.CONFIG_KEYS.CURSOR_BLINK,
        true
      ),
      confirmBeforeKill: this.get(
        shared_1.CONFIG_SECTIONS.SIDEBAR_TERMINAL,
        shared_1.CONFIG_KEYS.CONFIRM_BEFORE_KILL,
        false
      ),
      protectLastTerminal: this.get(
        shared_1.CONFIG_SECTIONS.SIDEBAR_TERMINAL,
        shared_1.CONFIG_KEYS.PROTECT_LAST_TERMINAL,
        true
      ),
      minTerminalCount: this.get(
        shared_1.CONFIG_SECTIONS.SIDEBAR_TERMINAL,
        shared_1.CONFIG_KEYS.MIN_TERMINAL_COUNT,
        1
      ),
      altClickMovesCursor: this.get(
        shared_1.CONFIG_SECTIONS.TERMINAL_INTEGRATED,
        shared_1.CONFIG_KEYS.ALT_CLICK_MOVES_CURSOR,
        false
      ),
      multiCursorModifier: this.get(
        shared_1.CONFIG_SECTIONS.EDITOR,
        shared_1.CONFIG_KEYS.MULTI_CURSOR_MODIFIER,
        'ctrlCmd'
      ),
  ...
  getShellForPlatform(customShell) {
    ...
    switch (process.platform) {
      case SystemConstants_1.TERMINAL_CONSTANTS.PLATFORMS.WINDOWS:
        return (
          this.get(section, shared_1.CONFIG_KEYS.SHELL_WINDOWS, '') ||
          process.env['COMSPEC'] ||
          'cmd.exe'
        );
      case SystemConstants_1.TERMINAL_CONSTANTS.PLATFORMS.DARWIN:
        return (
          this.get(section, shared_1.CONFIG_KEYS.SHELL_OSX, '') ||
          process.env['SHELL'] ||
          '/bin/zsh'
        );
      default:
        return (
          this.get(section, shared_1.CONFIG_KEYS.SHELL_LINUX, '') ||
          process.env['SHELL'] ||
          '/bin/bash'
        );
    }
  ...
  isFeatureEnabled(featureName) {
    switch (featureName) {
      ...
      case 'altClickMovesCursor':
        return (
          this.get(shared_1.CONFIG_SECTIONS.TERMINAL_INTEGRATED, 'altClickMovesCursor', true) &&
  ...
  _registerConfigurationSchemas() {
    ...
    this._registry.register(
      `${shared_1.CONFIG_SECTIONS.SIDEBAR_TERMINAL}.${shared_1.CONFIG_KEYS.MAX_TERMINALS}`,
      {
        type: 'number',
        default: 5,
        minimum: 1,
    ...
    // Theme
    this._registry.register(
      `${shared_1.CONFIG_SECTIONS.SIDEBAR_TERMINAL}.${shared_1.CONFIG_KEYS.THEME}`,
      {
        type: 'string',
        default: 'auto',
  ...
  _initializeConfigurationWatcher() {
    try {
      if (vscode?.workspace?.onDidChangeConfiguration) {
        const disposable = vscode.workspace.onDidChangeConfiguration((event) => {
          ...
          if (hasChanges) {
            ...
            (0, logger_1.terminal)(
              `⚙️ [UnifiedConfig] Configuration changed: ${changedKeys.length} keys affected`
            );
          }
        });
      ...
      (0, logger_1.terminal)(
        '⚠️ [UnifiedConfig] Could not initialize configuration watcher:',
        error
      );
    }
  }

...
class ConfigurationMigrator {
        ...
        try {
            ...
            migrationResults.forEach((migrationResult, index) => {
                ...
                if (migrationResult.status === 'fulfilled') {
                    ...
                    (0, logger_1.terminal)(`❌ [ConfigMigrator] ${serviceNames[index]} migration failed:`, migrationResult.reason);
                }
            });
            ...
            if (result.success) {
                (0, logger_1.terminal)(`✅ [ConfigMigrator] Migration completed successfully. ${result.migratedKeys.length} keys migrated`);
                ...
                (0, logger_1.terminal)(`❌ [ConfigMigrator] Migration completed with errors: ${result.errors.length} errors`);
            }
            return result;
        }
        catch (error) {
    ...
    async validateCurrentConfiguration() {
        ...
        (0, logger_1.terminal)(`🔍 [ConfigMigrator] Validation result: ${validation.isValid ? 'VALID' : 'INVALID'}`);
        if (!validation.isValid) {
    ...
    async _migrateFromWebViewConfigManager() {
        ...
        try {
            ...
            for (const setting of webViewSettings) {
                try {
                    const value = this._unifiedService.get(shared_1.CONFIG_SECTIONS.SIDEBAR_TERMINAL, setting, undefined);
                    if (value !== undefined) {

...
exports.DEFAULT_VALIDATION_RULES = exports.MessageValidator = exports.MessageValidationError = void 0;
    ...
    validateRequired(message, fields) {
        for (const field of fields) {
            if (!(field in message) || message[field] === undefined) {
                throw new MessageValidationError(`Missing required field: ${field}`, field, message.command);
            }
        }
    }
    /**
    ...
    validateType(value, expectedType, fieldName) {
        ...
        if (expectedType === 'array') {
            if (!Array.isArray(value)) {
                throw new MessageValidationError(`Field '${fieldName}' must be an array, got ${actualType}`);
            }
            return true;
        }
        if (actualType !== expectedType) {
            throw new MessageValidationError(`Field '${fieldName}' must be of type ${expectedType}, got ${actualType}`);
        }
        return true;
    }
    /**

...
exports.UnifiedMessageDispatcher = exports.MessagePriority = void 0;
...
(function (MessagePriority) {
    MessagePriority[MessagePriority["CRITICAL"] = 100] = "CRITICAL";
    MessagePriority[MessagePriority["HIGH"] = 75] = "HIGH";
    MessagePriority[MessagePriority["NORMAL"] = 50] = "NORMAL";
    MessagePriority[MessagePriority["LOW"] = 25] = "LOW";
 ...
 */
class UnifiedMessageDispatcher {
     ...
     */
    async dispatchToHandler(message, context) {
     ...
     */
    async sendMessage(message, priority = MessagePriority.NORMAL) {
     ...
     */
    async sendTerminalInteractionEvent(type, terminalId, data, priority = MessagePriority.NORMAL) {
        const message = {
     ...
     */
    async sendInput(input, terminalId, priority = MessagePriority.HIGH) {
        if (!this.coordinator) {
     ...
     * Send resize command
     */
    async sendResize(cols, rows, terminalId, priority = MessagePriority.HIGH) {
        if (!this.coordinator) {
            throw new Error('Coordinator not available');
        }
        const message = {
     ...
     */
    async requestSettings(priority = MessagePriority.NORMAL) {
        const message = {
            command: 'getSettings',
            timestamp: Date.now(),
        };
     ...
     * Update settings
     */
    async updateSettings(settings, priority = MessagePriority.NORMAL) {
        const message = {
     ...
     */
    async requestNewTerminal(priority = MessagePriority.HIGH) {
        const message = {
     ...
     */
    async sendDeleteTerminalMessage(terminalId, requestSource, priority = MessagePriority.HIGH) {
        const message = {
     ...
     */
    mapPriorityToQueueLevel(priority) {
...
exports.UnifiedMessageDispatcher = UnifiedMessageDispatcher;
//# sourceMappingURL=UnifiedMessageDispatcher.js.map
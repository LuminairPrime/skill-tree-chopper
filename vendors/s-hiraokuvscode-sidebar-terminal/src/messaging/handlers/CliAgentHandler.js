...
exports.CliAgentHandler = void 0;
const UnifiedMessageDispatcher_1 = require("../UnifiedMessageDispatcher");
const BaseMessageHandler_1 = require("./BaseMessageHandler");
class CliAgentHandler extends BaseMessageHandler_1.BaseMessageHandler {
    constructor() {
        super(['cliAgentStatusUpdate', 'cliAgentFullStateSync', 'switchAiAgentResponse'], UnifiedMessageDispatcher_1.MessagePriority.HIGH);
    }
     ...
     */
    async handleCliAgentStatusUpdate(message, context) {
        context.logger.info('CLI Agent Status Update received');
        const cliAgentStatus = message.cliAgentStatus;
        if (cliAgentStatus) {
            const statusData = cliAgentStatus;
            context.logger.info(`Processing status update: ${statusData.status} for ${statusData.activeTerminalName} (ID: ${statusData.terminalId})`);
            try {
                ...
                context.coordinator.updateCliAgentStatus(terminalId, mappedStatus, statusData.agentType || null);
                context.logger.info(`CLI Agent status updated successfully: ${mappedStatus} for terminal ${terminalId}`);
            }
            catch (error) {
     ...
     */
    async handleCliAgentFullStateSync(message, context) {
        ...
        if (terminalStates) {
            context.logger.info(`Processing full state sync: CONNECTED=${String(connectedAgentId)} (${String(connectedAgentType)}), DISCONNECTED=${String(disconnectedCount)}`);
            try {
                for (const [terminalId, stateInfo] of Object.entries(terminalStates)) {
                    const typedStateInfo = stateInfo;
                    context.logger.debug(`Updating terminal ${terminalId}`, typedStateInfo);
                    try {
                        context.coordinator.updateCliAgentStatus(terminalId, typedStateInfo.status, typedStateInfo.agentType);
                        context.logger.debug(`Applied state: Terminal ${terminalId} -> ${typedStateInfo.status} (${typedStateInfo.agentType})`);
                    }
                    catch (error) {
     ...
     */
    async handleSwitchAiAgentResponse(message, context) {
    ...
    mapLegacyStatus(legacyStatus) {
        switch (legacyStatus.toLowerCase()) {
            ...
            case 'terminated':
                return 'none';
            default:
                this.logActivity(undefined, `Unknown legacy status: ${legacyStatus}, defaulting to 'none'`);
                return 'none';
        }
    }
}
exports.CliAgentHandler = CliAgentHandler;
//# sourceMappingURL=CliAgentHandler.js.map
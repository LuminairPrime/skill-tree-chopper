...
exports.SessionDataTransformer = void 0;
...
class SessionDataTransformer {
    ...
    static isValidSessionData(data) {
        ...
        const session = data;
        return (Array.isArray(session.terminals) &&
     ...
     */
    static isSessionExpired(sessionData, expiryDays = 7) {
     ...
     */
    static calculateStorageSize(sessionData) {
        try {
     ...
     */
    static isStorageLimitExceeded(sessionData, limitMB = 20) {
     ...
     */
    static getCleanupRecommendations(sessionData, config = {}) {
        ...
        if (storageInfo.exceeded) {
            reasons.push(`Storage limit exceeded (${storageInfo.currentSizeMB}MB / ${storageInfo.limitMB}MB)`);
            shouldCleanup = true;
        }
        // Check warning threshold
        if (storageInfo.percentageUsed >= warnThresholdPercent && !storageInfo.exceeded) {
            reasons.push(`Storage usage high (${storageInfo.percentageUsed}% of ${storageInfo.limitMB}MB)`);
        }
        return {
            shouldCleanup,
            reason: reasons,
     ...
     */
    static optimizeSessionStorage(sessionData, targetSizeMB = 18 // 90% of 20MB default limit
    ) {
     ...
     */
    static normalizeTerminalData(terminal) {
        return {
            id: terminal.id || '',
     ...
     */
    static createEmptyResult(message = 'No session data') {
        return {
            success: true,
            restoredCount: 0,
            skippedCount: 0,
     ...
     */
    static createSuccessResult(restoredCount, skippedCount = 0, terminals) {
        return {
            success: true,
            restoredCount,
            skippedCount,
     ...
     */
    static createFailureResult(error) {
     ...
     */
    static validateSessionForRestore(sessionData) {
        const issues = [];
        const warnings = [];
        ...
        if (sessionData.scrollbackData) {
            Object.entries(sessionData.scrollbackData).forEach(([termId, data]) => {
                if (Array.isArray(data)) {
                    if (data.length === 200) {
                        warnings.push(`Terminal ${termId} has exactly 200 lines (possible old format truncation)`);
                    }
     ...
     */
    static createMigrationProgress(totalTerminals, processedTerminals, migrated) {
...
exports.SessionDataTransformer = SessionDataTransformer;

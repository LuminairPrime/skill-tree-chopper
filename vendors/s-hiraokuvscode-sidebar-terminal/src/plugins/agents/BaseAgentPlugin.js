...
class BaseAgentPlugin {
    ...
    dispose() {
        if (this._state === IPlugin_1.PluginState.Active) {
            this.deactivate().catch((error) => {
                (0, logger_1.terminal)(`⚠️ [PLUGIN] Error during disposal of ${this.metadata.name}:`, error);
            });
        }
    ...
    onAgentActivated(terminalId) {
        (0, logger_1.terminal)(`🤖 [PLUGIN] ${this.metadata.name} activated in terminal: ${terminalId}`);
    }
    onAgentDeactivated(terminalId) {
        (0, logger_1.terminal)(`🤖 [PLUGIN] ${this.metadata.name} deactivated in terminal: ${terminalId}`);
    }
    /**

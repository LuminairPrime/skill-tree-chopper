...
class IntegratedSecondaryTerminalProvider {
    constructor(context, terminalManager) {
        ...
        this.persistenceService = new ExtensionPersistenceService_1.ExtensionPersistenceService(this.context, this.terminalManager);
        // Initialize persistence message handler with clean separation
        this.persistenceMessageHandler = (0, PersistenceMessageHandler_1.createPersistenceMessageHandler)(this.persistenceService);
    ...
    async checkAndRestoreSession() {
        ...
        if (sessionInfo && sessionInfo.exists) {
            (0, logger_1.log)(`🔄 [PROVIDER] Previous session found: ${sessionInfo.terminals?.length} terminals`);
            ...
            if (restoreResult.success) {
                (0, logger_1.log)(`✅ [PROVIDER] Session restored: ${restoreResult.restoredCount} terminals`);
            }
            else {
                (0, logger_1.log)(`⚠️ [PROVIDER] Session restore failed: ${restoreResult.error instanceof Error ? restoreResult.error.message : restoreResult.error}`);
            }
        }
        else {
    ...
    setupEventListeners(webviewView) {
        webviewView.webview.onDidReceiveMessage((message) => this.handleWebviewMessage(message), undefined, this.disposables);

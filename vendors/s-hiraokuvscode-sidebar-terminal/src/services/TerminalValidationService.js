...
class TerminalValidationService {
    constructor(config = {}) {
        ...
        this.config = {
            maxTerminals: terminalConfig.maxTerminals || SystemConstants_1.TERMINAL_CONSTANTS.MAX_TERMINAL_COUNT,
            minTerminals: 1,
        ...
        this._terminalNumberManager = new TerminalNumberManager_1.TerminalNumberManager(this.config.maxTerminals);
    ...
    validateOperation(terminalId, terminals, operation) {
        ...
        (0, logger_1.terminal)(`✅ [VALIDATION] Operation '${operation}' validated for terminal: ${terminalId}`);
        return OperationResultHandler_1.OperationResultHandler.success();
    }
    /**
     * ターミナルIDの検証
    ...
    validateTerminalMapHealth(terminals) {
        ...
        if (!isHealthy) {
            (0, logger_1.terminal)(`⚠️ [VALIDATION] Terminal map health check failed: ${issues.length} issues`);
        }

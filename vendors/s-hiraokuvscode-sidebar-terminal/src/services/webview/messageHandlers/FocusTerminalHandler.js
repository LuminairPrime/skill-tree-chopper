...
class FocusTerminalHandler extends BaseMessageHandler_1.BaseMessageHandler {
    ...
    async handle(message, context) {
        ...
        try {
            ...
            (0, logger_1.provider)(`🔍 [FocusTerminal] Current active: ${currentActive} → Requested: ${message.terminalId}`);
            ...
            if (newActive === message.terminalId) {
                (0, logger_1.provider)(`✅ [FocusTerminal] Successfully focused terminal: ${message.terminalId}`);
            }
            else {
                (0, logger_1.provider)(`❌ [FocusTerminal] Failed to focus terminal. Expected: ${message.terminalId}, Got: ${newActive}`);
            }
        }
        catch (error) {

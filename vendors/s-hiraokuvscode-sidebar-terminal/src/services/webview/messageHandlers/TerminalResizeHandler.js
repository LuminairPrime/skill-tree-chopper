...
class TerminalResizeHandler extends BaseMessageHandler_1.BaseMessageHandler {
  constructor() {
    super(...arguments);
    this.supportedCommands = [constants_1.TERMINAL_CONSTANTS?.COMMANDS?.RESIZE || 'resize'];
  }
  async handle(message, context) {
    ...
    try {
      ...
      (0, logger_1.provider)(
        `✅ [TerminalResize] Successfully resized terminal to ${message.cols}x${message.rows}`
      );
    } catch (error) {

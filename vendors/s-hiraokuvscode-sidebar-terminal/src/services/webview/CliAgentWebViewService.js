...
class CliAgentWebViewService {
  ...
  async handleSwitchAiAgent(terminalId, action, context) {
    (0, logger_1.provider)(
      `🔌 [CliAgent] Processing AI Agent switch: ${terminalId} (action: ${action})`
    );
    try {
      ...
      if (result.success) {
        (0, logger_1.provider)(
          `✅ [CliAgent] Switch succeeded: ${terminalId}, new status: ${result.newStatus}`
        );
        ...
        (0, logger_1.provider)(
          `⚠️ [CliAgent] Switch failed: ${terminalId}, reason: ${result.reason}`
        );

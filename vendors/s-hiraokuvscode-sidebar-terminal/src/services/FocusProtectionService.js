...
class FocusProtectionService {
    constructor(deps) {
        ...
        this._disposables.add(vscode.window.onDidChangeActiveTerminal((terminal) => {
        ...
        this._disposables.add(vscode.window.onDidOpenTerminal((terminal) => {
            if (!(0, logger_1.isDebugEnabled)())
                return;
            (0, logger_1.provider)('🔍 [FOCUS_PROTECTION] onDidOpenTerminal fired:', this._describeTerminal(terminal));
        }));
    ...
    _hadRecentFocus() {
        if (this._deps.isTerminalFocused())
            return true;
    ...
    _hadRecentInteraction() {
        return (Date.now() - this._lastInteractionTime < FocusProtectionService.RECENT_INTERACTION_WINDOW_MS);
    }
    _shouldRestoreAfterBlur() {
        ...
        if (!this._cliAgentConnected && !this._hadRecentInteraction())
            return false;
    ...
    _describeTerminal(terminal) {
        ...
        // rejection.
        Promise.resolve(terminal.processId).then((pid) => (0, logger_1.provider)('🔍 [FOCUS_PROTECTION] terminal processId resolved:', {
            name: terminal.name,
            pid,
        }), () => undefined);
        const info = {
    ...
    _onActiveTerminalChanged(terminal) {
        ...
        if ((0, logger_1.isDebugEnabled)()) {
            if (terminal) {
                (0, logger_1.provider)('🔍 [FOCUS_PROTECTION] onDidChangeActiveTerminal fired:', this._describeTerminal(terminal));
            }
            else {

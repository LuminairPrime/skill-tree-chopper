...
class TerminalDataBufferService {
  constructor(config = {}) {
    ...
    this._config = {
      flushInterval:
        ...
        SystemConstants_1.PERFORMANCE_CONSTANTS.OUTPUT_BUFFER_FLUSH_INTERVAL_MS, // ~60fps default
      maxBufferSize:
        config.maxBufferSize ?? SystemConstants_1.PERFORMANCE_CONSTANTS.MAX_BUFFER_CHUNK_COUNT,
      cliAgentFlushInterval:
        config.cliAgentFlushInterval ??
        SystemConstants_1.PERFORMANCE_CONSTANTS.CLI_AGENT_FAST_FLUSH_INTERVAL_MS, // ~250fps for CLI agents
    };
  ...
  bufferData(terminalId, data) {
    try {
      ...
      if (
        data.length >= SystemConstants_1.PERFORMANCE_CONSTANTS.IMMEDIATE_FLUSH_THRESHOLD_BYTES ||
  ...
  flushBuffer(terminalId) {
    try {
      ...
      if (buffer && buffer.length > 0) {
        ...
        (0, logger_1.terminal)(
          `🚰 [DataBuffer] Flushing ${combinedData.length} chars for terminal ${terminalId}`
        );

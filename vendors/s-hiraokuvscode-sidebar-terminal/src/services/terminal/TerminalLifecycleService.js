...
class TerminalLifecycleService {
  ...
  async createTerminal(options = {}) {
    ...
    try {
      ...
      (0, logger_1.terminal)(
        `🚀 [LifecycleService] Creating terminal ${terminalId} with options:`,
        options
      );
      ...
      (0, logger_1.terminal)(
        `✅ [LifecycleService] Terminal created successfully: ${terminalId} (${terminalName})`
      );
      return (0, shared_1.success)(terminal);
    } catch (error) {
      (0, logger_1.terminal)(
        `❌ [LifecycleService] Failed to create terminal ${terminalId}:`,
        error
      );
      return (0, shared_1.failureFromDetails)({
  ...
  async disposeTerminal(terminal) {
    try {
      (0, logger_1.terminal)(
        `🗑️ [LifecycleService] Disposing terminal ${terminal.id} (${terminal.name})`
      );
      // Kill PTY process
      if (terminal.pty) {
        terminal.pty.kill();
        ...
        (0, logger_1.terminal)(
          `🔨 [LifecycleService] Force killing terminal process ${terminal.id}`
        );
      ...
      if (terminal.number) {
        (0, logger_1.terminal)(
          `🔢 [LifecycleService] Terminal number ${terminal.number} will be released by caller`
        );
      }
      // Clean up shell integration (method not available in current implementation)
      if (this._shellIntegrationService) {
        (0, logger_1.terminal)(
          `🧹 [LifecycleService] Shell integration cleanup skipped for terminal ${terminal.id}`
        );
      }
      ...
      (0, logger_1.terminal)(
        `❌ [LifecycleService] Error disposing terminal ${terminal.id}:`,
        error
      );
      return (0, shared_1.failureFromDetails)({
  ...
  resizeTerminal(terminal, cols, rows) {
    if (!terminal.pty) {
      (0, logger_1.terminal)(
        `⚠️ [LifecycleService] Cannot resize terminal without PTY ${terminal.id}`
      );
      return (0, shared_1.failureFromDetails)({
    ...
    return (0, shared_1.tryCatch)(
      () => {
        ...
        (0, logger_1.terminal)(
          `📏 [LifecycleService] Resized terminal ${terminal.id} to ${cols}x${rows}`
        );
      },
      (error) => ({
        code: shared_1.ErrorCode.TERMINAL_PROCESS_FAILED,
  ...
  sendInputToTerminal(terminal, data) {
    if (!terminal.pty) {
      (0, logger_1.terminal)(
        `⚠️ [LifecycleService] Cannot send input to terminal without PTY ${terminal.id}`
      );
      return (0, shared_1.failureFromDetails)({
        code: shared_1.ErrorCode.TERMINAL_NOT_FOUND,
    ...
    return (0, shared_1.tryCatch)(
      () => {
        ...
        (0, logger_1.terminal)(
          `⌨️ [LifecycleService] Sent ${data.length} chars to terminal ${terminal.id}`
        );
      },
      (error) => ({
        code: shared_1.ErrorCode.TERMINAL_PROCESS_FAILED,
    ...
    try {
      if (requestedProfile) {
        try {
          ...
          (0, logger_1.terminal)(
            `⚠️ [LifecycleService] Error getting profile ${requestedProfile}:`,
            error
          );
        }
      }
  ...
  async createPtyProcess(options) {
    try {
      ...
      (0, logger_1.terminal)(
        `🔧 [LifecycleService] Creating PTY process: ${shell} ${args.join(' ')}`
      );
      ...
      (0, logger_1.terminal)(
        `✅ [LifecycleService] PTY process created with PID: ${ptyProcess.pid}`
      );
  ...
  initializeShellIntegration(terminal, safeMode) {
    try {
      ...
      if (!this._shellIntegrationService) {
        ...
        (0, logger_1.terminal)(
          `⚠️ [LifecycleService] Shell integration service initialization skipped`
        );
      }
      ...
      (0, logger_1.terminal)(
        `🔗 [LifecycleService] Shell integration attachment skipped for terminal ${terminal.id}`
      );
    } catch (error) {
      (0, logger_1.terminal)(
  ...
  dispose() {
    ...
    try {
      ...
      (0, logger_1.terminal)(
        '❌ [LifecycleService] Error disposing terminal lifecycle service:',
        error
      );
    }
  }

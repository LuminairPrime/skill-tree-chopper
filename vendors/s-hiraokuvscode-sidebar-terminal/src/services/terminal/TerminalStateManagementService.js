...
class TerminalStateManagementService {
    ...
    addTerminal(terminal) {
        try {
            ...
            (0, logger_1.terminal)(`➕ [StateManager] Terminal added to state: ${terminal.id} (${terminal.name})`);
            // Emit events
    ...
    removeTerminal(terminalId) {
        try {
            ...
            if (!terminal) {
                (0, logger_1.terminal)(`⚠️ [StateManager] Attempted to remove non-existent terminal: ${terminalId}`);
                return;
            }
            ...
            if (terminal.number) {
                (0, logger_1.terminal)(`🔢 [StateManager] Terminal number ${terminal.number} marked for release for ${terminalId}`);
            }
    ...
    setActiveTerminal(terminalId) {
        try {
            const terminal = this._terminals.get(terminalId);
            if (!terminal) {
                (0, logger_1.terminal)(`⚠️ [StateManager] Cannot set non-existent terminal as active: ${terminalId}`);
                return false;
            }
            ...
            (0, logger_1.terminal)(`✅ [StateManager] Set active terminal: ${terminalId} (${terminal.name})`);
            this._notifyStateUpdate();
            return true;
        }
        catch (error) {
    ...
    releaseTerminalNumber(number) {
        (0, logger_1.terminal)(`🔢 [StateManager] Terminal number ${number} release requested (handled externally)`);
    }
    /**
    ...
    dispose() {
        ...
        try {
            ...
            (0, logger_1.terminal)('❌ [StateManager] Error disposing terminal state management service:', error);
        }
    }
}

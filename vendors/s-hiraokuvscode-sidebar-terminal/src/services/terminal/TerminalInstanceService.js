...
class TerminalInstanceService {
    constructor() {
        ...
        this._maxTerminals = config.maxTerminals || SystemConstants_1.TERMINAL_CONSTANTS.MAX_TERMINAL_COUNT;
        this._terminalNumberManager = new TerminalNumberManager_1.TerminalNumberManager(this._maxTerminals);
    ...
    async createTerminal(options = {}) {
        ...
        try {
            ...
            (0, logger_1.terminal)(`🚀 [InstanceService] Creating terminal ${terminalId} with options:`, options);
            ...
            (0, logger_1.terminal)(`🔧 [InstanceService] Terminal config: shell=${shell}, args=[${shellArgs.join(', ')}], cwd=${cwd}`);
            // Create PTY process
            ...
            (0, logger_1.terminal)(`✅ [InstanceService] Terminal created successfully: ${terminalId} (${terminalName})`);
            return terminal;
        }
        catch (error) {
            (0, logger_1.terminal)(`❌ [InstanceService] Failed to create terminal ${terminalId}:`, error);
            throw error;
        }
        finally {
    ...
    async disposeTerminal(terminal) {
        try {
            (0, logger_1.terminal)(`🗑️ [InstanceService] Disposing terminal ${terminal.id} (${terminal.name})`);
            // Kill PTY process
            if (terminal.pty) {
                try {
                    terminal.pty.kill();
            ...
            (0, logger_1.terminal)(`❌ [InstanceService] Error disposing terminal ${terminal.id}:`, error);
            throw error;
        }
    }
    /**
    ...
    resizeTerminal(terminal, cols, rows) {
        try {
            if (!terminal.pty) {
                (0, logger_1.terminal)(`⚠️ [InstanceService] Cannot resize terminal without PTY ${terminal.id}`);
                return;
            }
            terminal.pty.resize(cols, rows);
            (0, logger_1.terminal)(`📏 [InstanceService] Resized terminal ${terminal.id} to ${cols}x${rows}`);
        }
        catch (error) {
    ...
    sendInputToTerminal(terminal, data) {
        try {
            if (!terminal.pty) {
                (0, logger_1.terminal)(`⚠️ [InstanceService] Cannot send input to terminal without PTY ${terminal.id}`);
                return;
            }
            terminal.pty.write(data);
            (0, logger_1.terminal)(`⌨️ [InstanceService] Sent ${data.length} chars to terminal ${terminal.id}`);
        }
        catch (error) {
            (0, logger_1.terminal)(`❌ [InstanceService] Error sending input to terminal ${terminal.id}:`, error);
            throw error;
        }
    }
    /**
        ...
        try {
            if (requestedProfile) {
                try {
                    ...
                    (0, logger_1.terminal)(`⚠️ [InstanceService] Profile service not available, using default for: ${requestedProfile}`);
                }
                catch (error) {
                    (0, logger_1.terminal)(`⚠️ [InstanceService] Error getting profile ${requestedProfile}:`, error);
                }
            }
    ...
    async createPtyProcess(options) {
        try {
            ...
            (0, logger_1.terminal)(`🔧 [InstanceService] Creating PTY process: ${shell} ${args.join(' ')}`);
            ...
            (0, logger_1.terminal)(`✅ [InstanceService] PTY process created with PID: ${ptyProcess.pid}`);
            return ptyProcess;
        }
        catch (error) {
    ...
    initializeShellIntegration(terminal, safeMode) {
        try {
            ...
            if (!this._shellIntegrationService) {
                ...
                (0, logger_1.terminal)(`⚠️ [InstanceService] Shell integration service initialization skipped`);
            }
            // Skip shell integration in safe mode
            if (safeMode) {
                (0, logger_1.terminal)(`⚠️ [InstanceService] Skipping shell integration for safe mode terminal ${terminal.id}`);
                return;
            }
            // this._shellIntegrationService.attachToTerminal(terminal);
            // Method not available in current implementation
            (0, logger_1.terminal)(`🔗 [InstanceService] Shell integration attachment skipped for terminal ${terminal.id}`);
        }
        catch (error) {
            (0, logger_1.terminal)(`⚠️ [InstanceService] Failed to initialize shell integration for terminal ${terminal.id}:`, error);
    ...
    dispose() {
        ...
        try {
            ...
            (0, logger_1.terminal)('❌ [InstanceService] Error disposing terminal lifecycle service:', error);
        }
    }
}

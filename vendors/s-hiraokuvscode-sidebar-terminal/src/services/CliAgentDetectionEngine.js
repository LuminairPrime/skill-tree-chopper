...
class CliAgentDetectionEngine {
  ...
  detectFromInput(terminalId, input) {
    ...
    if (agentType) {
      ...
      (0, logger_1.terminal)(
        `✅ [INPUT-DETECTION] Detected ${agentType} from input: "${trimmedInput}"`
      );
      return result;
    }
  ...
  processOutputLine(terminalId, line) {
    ...
    if (agentType) {
      ...
      (0, logger_1.terminal)(
        `🚀 [OUTPUT-DETECTION] Detected ${agentType} startup: "${fullyCleanLine}"`
      );
      return {
        agentType,
        isDetected: true,
  ...
  detectStrictTermination(terminalId, cleanLine, agentType) {
    ...
    if (this.patternRegistry.isShellPrompt(cleanLine)) {
      ...
      if (recentInterrupt) {
        this.resetInterruptTracking(terminalSignals);
        (0, logger_1.terminal)(
          `✅ [TERMINATION] Interrupt followed by shell prompt: "${cleanLine}"`
        );
        return {
          isTerminated: true,
          confidence: 0.95,
      ...
      if (!looksLikeAIOutput && cleanLine.length <= 50) {
        ...
        if (isValid) {
          ...
          (0, logger_1.terminal)(
            `⚠️ [TERMINATION] Shell prompt ignored (recent AI activity): "${cleanLine}"`
          );
        }
      }
    }
  ...
  clearTerminalCache(terminalId) {
    ...
    (0, logger_1.terminal)(
      `🧹 [CACHE-CLEAR] Cleared ${keysToDelete.length} entries for terminal ${terminalId}`
    );
  }
  /**

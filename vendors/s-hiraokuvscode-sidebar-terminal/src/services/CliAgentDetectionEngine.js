...
exports.CliAgentDetectionEngine = void 0;
const logger_1 = require("../utils/logger");
const CliAgentPatternRegistry_1 = require("./CliAgentPatternRegistry");
const LRUCache_1 = require("../utils/LRUCache");
/**
 * Unified CLI Agent Detection Engine
 */
class CliAgentDetectionEngine {
    constructor() {
        this.terminalSignalState = new Map();
        // Configuration
        this.CACHE_TTL_MS = 5000; // 5 seconds
        this.AI_ACTIVITY_TIMEOUT_MS = 10000; // 10 seconds
        this.TERMINATION_GRACE_PERIOD_MS = 2000; // 2 seconds
        this.RECENT_INPUT_WINDOW_MS = 15000; // 15 seconds
        this.SHELL_COMPLETION_WINDOW_MS = 4000; // 4 seconds
        this.INTERRUPT_PROMPT_WINDOW_MS = 5000; // 5 seconds
        this.DOUBLE_INTERRUPT_WINDOW_MS = 3000; // 3 seconds
        this.INTERRUPT_ECHO_DEDUP_MS = 250; // 250ms
        this.patternRegistry = new CliAgentPatternRegistry_1.CliAgentPatternRegistry();
        this.detectionCache = new LRUCache_1.LRUCache(100);
    }
    /**
    ...
    detectFromInput(terminalId, input) {
        ...
        (0, logger_1.terminal)(`🎯 [INPUT-DETECTION] Processing input: "${trimmedInput}"`);
        ...
        if (agentType) {
            ...
            (0, logger_1.terminal)(`✅ [INPUT-DETECTION] Detected ${agentType} from input: "${trimmedInput}"`);
            return result;
        }
    ...
    detectFromOutput(terminalId, data) {
        try {
            ...
            (0, logger_1.terminal)('ERROR: Output detection processing failed:', error);
     ...
     * @returns Detection result
     */
    processOutputLine(terminalId, line) {
        ...
        if (agentType) {
            const now = Date.now();
            const terminalSignals = this.getTerminalSignalState(terminalId);
            const hasRecentInputContext = terminalSignals.lastInputAgent === agentType &&
                now - terminalSignals.lastInputAt <= this.RECENT_INPUT_WINDOW_MS;
            const hasRecentShellContext = terminalSignals.shellExecutingAgent === agentType &&
                now - terminalSignals.lastShellSignalAt <= this.RECENT_INPUT_WINDOW_MS;
            (0, logger_1.terminal)(`🚀 [OUTPUT-DETECTION] Detected ${agentType} startup: "${fullyCleanLine}"`);
            return {
                agentType,
                isDetected: true,
     ...
     */
    detectTermination(terminalId, data, currentAgentType) {
        try {
            const shellTermination = this.detectShellCommandCompletion(terminalId, data, currentAgentType);
            if (shellTermination) {
     ...
     */
    detectImmediateInterruptTermination(terminalId, currentAgentType) {
        if (!currentAgentType) {
            return null;
        }
        const terminalSignals = this.getTerminalSignalState(terminalId);
        const recentInterrupt = terminalSignals.lastInterruptAt > 0 &&
     ...
     */
    detectStrictTermination(terminalId, cleanLine, agentType) {
        const terminalSignals = this.getTerminalSignalState(terminalId);
        ...
        const now = Date.now();
        const claudeContextLikelyActive = agentType === 'claude' ||
        ...
        if (this.patternRegistry.isTerminationPattern(cleanLine, agentType)) {
            this.resetInterruptTracking(terminalSignals);
            (0, logger_1.terminal)(`✅ [TERMINATION] Explicit termination detected: "${cleanLine}"`);
            return {
                isTerminated: true,
                confidence: 1.0,
                detectedLine: cleanLine,
        ...
        if (this.patternRegistry.isShellPrompt(cleanLine)) {
            ...
            if (recentInterrupt) {
                this.resetInterruptTracking(terminalSignals);
                (0, logger_1.terminal)(`✅ [TERMINATION] Interrupt followed by shell prompt: "${cleanLine}"`);
                return {
                    isTerminated: true,
                    confidence: 0.95,
                    detectedLine: cleanLine,
            ...
            const looksLikeAIOutput = this.patternRegistry.isAgentActivity(cleanLine) ||
            ...
            if (!looksLikeAIOutput && cleanLine.length <= 50) {
                ...
                if (isValid) {
                    this.resetInterruptTracking(terminalSignals);
                    (0, logger_1.terminal)(`✅ [TERMINATION] Shell prompt detected: "${cleanLine}"`);
                    return {
                        isTerminated: true,
                        confidence: 0.6,
                        detectedLine: cleanLine,
                    ...
                    (0, logger_1.terminal)(`⚠️ [TERMINATION] Shell prompt ignored (recent AI activity): "${cleanLine}"`);
                }
            }
        }
        ...
        if (timeSinceLastAIOutput > 30000) {
            ...
            if (cleanLine.length <= 30 &&
                ...
                (0, logger_1.terminal)(`✅ [TERMINATION] Timeout-based detection: "${cleanLine}"`);
                return {
                    isTerminated: true,
                    confidence: 0.5,
                    detectedLine: cleanLine,
     ...
     */
    validateTerminationSignal(terminalId, baseConfidence) {
        ...
        if (timeSinceLastAIOutput < this.AI_ACTIVITY_TIMEOUT_MS) {
            (0, logger_1.terminal)(`🔍 [VALIDATION] Recent AI activity (${timeSinceLastAIOutput}ms ago), requiring higher confidence`);
     ...
     */
    updateAIActivityTimestamp(terminalId, line) {
    ...
    getTerminalSignalState(terminalId) {
    ...
    detectShellCommandStart(terminalId, data) {
        const commandExecutedPattern = /\x1b\]633;B;([^\x07]*)\x07/g;
        const terminalSignals = this.getTerminalSignalState(terminalId);
        let detectedResult = null;
        let match = commandExecutedPattern.exec(data);
        while (match) {
    ...
    detectShellCommandCompletion(terminalId, data, currentAgentType) {
        const commandFinishedPattern = /\x1b\]633;C(?:;(-?\d+))?\x07/g;
        const terminalSignals = this.getTerminalSignalState(terminalId);
        const match = commandFinishedPattern.exec(data);
        if (!match) {
            return null;
        }
        let candidateAgent = terminalSignals.shellExecutingAgent;
        const now = Date.now();
        const exitCode = match[1] ? Number(match[1]) : undefined;
        if (!candidateAgent && currentAgentType) {
            const recentInputForCurrentAgent = terminalSignals.lastInputAgent === currentAgentType &&
                now - terminalSignals.lastInputAt <= this.RECENT_INPUT_WINDOW_MS;
        ...
        const shellConfidence = now - terminalSignals.lastInputAt <= this.SHELL_COMPLETION_WINDOW_MS ? 1.0 : 0.95;
        return {
            isTerminated: true,
    ...
    registerInterruptSignal(terminalSignals, source) {
        const now = Date.now();
        const timeSinceLastInterrupt = now - terminalSignals.lastInterruptAt;
        const withinDoubleInterruptWindow = terminalSignals.lastInterruptAt > 0 &&
            timeSinceLastInterrupt <= this.DOUBLE_INTERRUPT_WINDOW_MS;
        if (withinDoubleInterruptWindow) {
            const isLikelyEchoFromInput = source === 'output' &&
    ...
    resetInterruptTracking(terminalSignals) {
     ...
     */
    createNegativeResult(source, reason) {
        return {
            agentType: null,
            isDetected: false,
            confidence: 0,
    ...
    clearTerminalCache(terminalId) {
        ...
        try {
            ...
            this.detectionCache.forEach((_value, key) => {
            ...
            (0, logger_1.terminal)(`🧹 [CACHE-CLEAR] Cleared entire cache (terminal ${terminalId})`);
            return;
        }
        // Delete collected keys
        keysToDelete.forEach((key) => this.detectionCache.delete(key));
        (0, logger_1.terminal)(`🧹 [CACHE-CLEAR] Cleared ${keysToDelete.length} entries for terminal ${terminalId}`);
    }
    /**
...
exports.CliAgentDetectionEngine = CliAgentDetectionEngine;
//# sourceMappingURL=CliAgentDetectionEngine.js.map
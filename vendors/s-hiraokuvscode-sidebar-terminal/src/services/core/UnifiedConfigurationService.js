...
class UnifiedConfigurationService {
  ...
  getSidebarTerminalConfig() {
    ...
    const settings = {
      ...
      maxTerminals: this._getWithDefault(
        config,
        'maxTerminals',
        SystemConstants_1.TERMINAL_CONSTANTS.MAX_TERMINAL_COUNT
      ),
  ...
  async update(section, key, value, target) {
    try {
      ...
      (0, logger_1.terminal)(
        `⚙️ [UnifiedConfig] Updated ${section}.${key}: ${oldValue} → ${value}`
      );
    } catch (error) {
  ...
  validateConfiguration() {
    ...
    try {
      ...
      if (
        config.maxTerminals < 1 ||
        config.maxTerminals > SystemConstants_1.TERMINAL_CONSTANTS.MAX_TERMINAL_COUNT
      ) {
        warnings.push(
          `maxTerminals should be between 1 and ${SystemConstants_1.TERMINAL_CONSTANTS.MAX_TERMINAL_COUNT} for optimal performance`
        );
      }
  ...
  async resetToDefaults(section) {
    try {
      ...
      (0, logger_1.terminal)(
        `⚙️ [UnifiedConfig] Reset configuration for section: ${targetSection}`
      );
    } catch (error) {

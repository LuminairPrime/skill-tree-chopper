...
class TerminalDecorationsService {
    ...
    loadSettings() {
        ...
        const vscodeDecorations = config.get('terminal.integrated.shellIntegration.decorationsEnabled', true);
    ...
    addDecoration(decoration) {
        ...
        (0, logger_1.terminal)(`🎨 [DECORATIONS] Added ${decoration.status} decoration for terminal ${decoration.terminalId}: ${decoration.command || 'unknown'}`);
    ...
    completeCommand(terminalId, commandId, exitCode) {
        ...
        if (decoration) {
            ...
            (0, logger_1.terminal)(`🎨 [DECORATIONS] Completed command ${commandId} with exit code ${exitCode}`);
            this._decorationEmitter.fire({
                terminalId,

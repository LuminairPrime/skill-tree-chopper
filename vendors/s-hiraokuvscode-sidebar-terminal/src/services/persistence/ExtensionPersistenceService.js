...
class ExtensionPersistenceService {
  ...
  async saveCurrentSession(options) {
    ...
    try {
      ...
      const storageCheck = session_types_1.SessionDataTransformer.isStorageLimitExceeded(
        sessionData,
  ...
  async restoreSession(forceRestore = false) {
    ...
    try {
      ...
      const validation =
        session_types_1.SessionDataTransformer.validateSessionForRestore(sessionData);
      if (!validation.valid) {
      ...
      // Check expiry
      if (
        session_types_1.SessionDataTransformer.isSessionExpired(
          sessionData,
          config.persistentSessionExpiryDays
        )
      ) {

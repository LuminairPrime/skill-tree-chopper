...
class CliAgentStateManager {
  ...
  setConnectedAgent(terminalId, type, terminalName) {
    ...
    if (previousConnectedId && previousConnectedId !== terminalId && previousType) {
      ...
      (0, logger_1.terminal)(
        `📝 [STATE-MANAGER] Terminal ${previousConnectedId} moved to DISCONNECTED`
      );
    }
    (0, logger_1.terminal)(
  ...
  promoteDisconnectedAgentToConnected(terminalId) {
    ...
    if (previousConnectedId && previousConnectedId !== terminalId && previousType) {
      ...
      (0, logger_1.terminal)(
        `📝 [PROMOTION] Terminal ${previousConnectedId} moved to DISCONNECTED`
      );
    }
    (0, logger_1.terminal)(
  ...
  setAgentTerminated(terminalId) {
    (0, logger_1.terminal)(
      `🔄 [STATE-MANAGER-DEBUG] setAgentTerminated called for terminal ${terminalId}`
    );
    (0, logger_1.terminal)(
    ...
    if (this._connectedAgentTerminalId === terminalId) {
      ...
      wasConnected = true;
      (0, logger_1.terminal)(
        `🔄 [STATE-MANAGER] Connected terminal ${terminalId} (${agentType}) terminated`
      );
    }
  ...
  removeTerminalCompletely(terminalId) {
    ...
    if (wasConnected || wasDisconnected) {
      ...
      (0, logger_1.terminal)(
        `🗑️ [STATE-MANAGER] Terminal ${terminalId} completely removed (${agentType} agent)`
      );
  ...
  promoteLatestDisconnectedAgent() {
    ...
    if (latestAgent) {
      ...
      (0, logger_1.terminal)(
        `📊 [AUTO-PROMOTION] Remaining DISCONNECTED agents: ${this._disconnectedAgents.size}`
      );
    }
  }
  ...
  forceReconnectAgent(terminalId, agentType, terminalName) {
    (0, logger_1.terminal)(
      `🔄 [MANUAL-RESET] Force reconnecting ${agentType} agent in terminal ${terminalId}`
    );
    ...
    if (wasConnected) {
      (0, logger_1.terminal)(
        `🔄 [MANUAL-RESET] Terminal ${terminalId} was CONNECTED, clearing connected state`
      );
      this._connectedAgentTerminalId = null;
      this._connectedAgentType = null;
    }
  ...
  clearDetectionError(terminalId) {
    ...
    if (this._connectedAgentTerminalId === terminalId) {
      ...
      hadState = true;
      (0, logger_1.terminal)(
        `🧹 [MANUAL-RESET] Cleared CONNECTED state for terminal ${terminalId}`
      );
    }
    ...
    if (this._disconnectedAgents.has(terminalId)) {
      ...
      hadState = true;
      (0, logger_1.terminal)(
        `🧹 [MANUAL-RESET] Cleared DISCONNECTED state for terminal ${terminalId}`
      );
    }
    ...
    if (hadState) {
      ...
      (0, logger_1.terminal)(
        `✅ [MANUAL-RESET] Reset terminal ${terminalId} to 'none' state (was: ${previousType})`
      );
      return true;
    } else {
  ...
  validateConnectedAgentState() {
    ...
    (0, logger_1.terminal)(
      `💓 [HEARTBEAT] Validating connected agent state: terminal ${terminalId} (${agentType})`
    );

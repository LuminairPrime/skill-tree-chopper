...
class CommonUtilityService {
    ...
    async withRetry(operation, config = {}) {
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                ...
                if (attempt > 1) {
                    (0, logger_1.terminal)(`✅ [CommonUtility] Operation succeeded on attempt ${attempt}/${maxAttempts}`);
                }
                return result;
            }
            catch (error) {
                ...
                if (attempt === maxAttempts) {
                    (0, logger_1.terminal)(`❌ [CommonUtility] Operation failed after ${maxAttempts} attempts:`, lastError);
                    break;
                }
                const delay = exponentialBackoff ? delayMs * Math.pow(2, attempt - 1) : delayMs;
                (0, logger_1.terminal)(`⚠️ [CommonUtility] Attempt ${attempt}/${maxAttempts} failed, retrying in ${delay}ms:`, lastError.message);
                if (onRetry) {
    ...
    async safeExecute(operation, errorHandler, context) {
        try {
            ...
            const err = error;
            (0, logger_1.terminal)(`❌ [CommonUtility] Safe execution failed${context ? ` in ${context}` : ''}:`, err);
            if (errorHandler) {
                try {
    ...
    validate(data, rules, context) {
        ...
        if (context && (errors.length > 0 || warnings.length > 0)) {
            (0, logger_1.terminal)(`🔍 [CommonUtility] Validation result for ${context}: ${errors.length} errors, ${warnings.length} warnings`);
        }
        return {
            isValid,
            errors,
        ...
        const cleanupPromises = resources.map(async (resource, index) => {
            try {
                if (resource && typeof resource.dispose === 'function') {
                    ...
                    (0, logger_1.terminal)(`✅ [CommonUtility] Cleaned up resource ${index + 1}/${resources.length}`);
                }
            }
            catch (error) {
                (0, logger_1.terminal)(`⚠️ [CommonUtility] Error cleaning up resource ${index + 1}:`, error);
            }
        });
        try {
            await Promise.race([
            ...
            (0, logger_1.terminal)(`✅ [CommonUtility] All ${resources.length} resources cleaned up successfully`);
        }
        catch (error) {
    ...
    createCancelablePromise(promise) {
        ...
        const cancelablePromise = new Promise((resolve, reject) => {
            promise.then((value) => (isCanceled ? reject(new Error('Operation canceled')) : resolve(value)), (error) => (isCanceled ? reject(new Error('Operation canceled')) : reject(error)));
        });
        return {

...
vscode.window.showInformationMessage('Delete this file?', 'Yes', 'No').then((selection) => {
  if (selection === 'Yes') {
    // Delete file
  }
});
...
workspaceEdit.replace(uri2, new vscode.Range(1, 0, 1, 10), 'replacement');
...
class MyHoverProvider implements vscode.HoverProvider {
  provideHover(
    ...
    return new vscode.Hover([`**${word}**`, 'Detailed information about ' + word]);
  }
}
...
class MyCodeActionProvider implements vscode.CodeActionProvider {
  provideCodeActions(
    ...
    const fix = new vscode.CodeAction('Fix this issue', vscode.CodeActionKind.QuickFix);
...
context.subscriptions.push(
  vscode.languages.registerCodeActionsProvider('javascript', new MyCodeActionProvider()),
);
```
...
context.subscriptions.push(
  vscode.languages.registerDefinitionProvider('javascript', new MyDefinitionProvider()),
);
```

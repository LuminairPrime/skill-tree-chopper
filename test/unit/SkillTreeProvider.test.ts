...
describe('SkillTreeProvider', () => {
    ...
    it('returns false when a folder cannot be read while checking for skill.md', async () => {
        ...
        await expect(hasSkillMd(vscode.Uri.file(path.resolve('virtual', 'missing')))).resolves.toBe(false);
    });
    ...
    it('returns workspace and global roots only for valid skill directories', async () => {
        ...
        configureVirtualFileSystem({
            [workspaceRoot]: [['.cursor', vscode.FileType.Directory], ['.forge', vscode.FileType.Directory]],
            ...
            [path.join(workspaceRoot, '.cursor', 'skills', 'alpha-skill')]: [['skill.md', vscode.FileType.File]],
            [path.join(workspaceRoot, '.cursor', 'skills', 'empty-folder')]: [['notes.md', vscode.FileType.File]],
            [path.join(workspaceRoot, '.cursor', 'skills', '.archived')]: [['zeta-skill', vscode.FileType.Directory]],
            [path.join(workspaceRoot, '.cursor', 'skills', '.archived', 'zeta-skill')]: [['SKILL.md', vscode.FileType.File]],
            ...
            [path.join(homeRoot, '.claude', 'skills', 'global-skill')]: [['skill.md', vscode.FileType.File]]
        });
    ...
    const provider = new SkillTreeProvider(outputChannel, homeRoot);
        ...
        expect(workspaceSkillsContainer?.children?.map((node) => node.label)).toEqual(['alpha-skill', 'zeta-skill']);
        ...
        expect(globalAgentRoot?.children?.[0].children?.map((node) => node.label)).toEqual(['global-skill']);
    });
});